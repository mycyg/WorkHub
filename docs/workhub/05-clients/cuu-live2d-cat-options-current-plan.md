...
| QA 业务场景注入 | `apps/desktop-webview/src/cuu-qa-scenarios.ts` / `WORKHUB_CUU_QA_SCENARIO` / `WORKHUB_CUU_QA_LOCALE` | 允许真实 Tauri pet window 录 `launcher` / `clarify` / `approval` / `search` / `sync` / `done` / `offline` / `run-stream` / `run-failure` / `permission-401` / `permission-403` / `stream-offline` / `reload-session` / `reload-active-run` / `reload-terminal-run` / `settings-menu` / `settings-menu-model-switch`；R3.10 已补 `launcher` 双语 locale 注入与 WebView2 CDP 点击，R3.12 已补真实 run-stream completion，R3.13.1 已补真实 run-failure terminal，R3.13.2 已补真实 permission/offline error states，R3.13.3 已补 webview boot session/run restore 单测，R3.14 已补 launcher spec readback smoke，R3.15 已补真实 reload restore capture，R3.16 已补 `clarify/search/sync/done/offline/approval` 当前脚本版本真实 Tauri 业务状态矩阵，R3.17 已补 settings/menu matrix |
...
- R3.13.3 的刷新恢复源码验收必须覆盖 session question card、active AgentRun card 和 terminal AgentRun card；`@workhub/desktop-webview` 单测必须证明刷新后仍保留 `payload_ref`、replay action、active run stream 订阅路径和 `cuuStart.restored` 双语文案。
- R3.14 的 launcher metadata 验收必须覆盖 `cuu_launcher_spec` JSON、`delivery_kind`、`risk_hint`、`default_acceptance`、WorkItem acceptance readback 与 `qa:cuu-r3-launcher-smoke` / `qa:cuu-r3-dev-server-smoke`。同轮新增文本边界规则，后续真实 capture 要继续确认 title/message/chip/action/progress 不超出 bubble。
- R3.15 的 reload restore 验收必须覆盖 session question、active AgentRun、terminal AgentRun 三类真实 Tauri `pet` window capture；zh-CN/en-US 证据必须同时看 contact sheet/GIF/MP4、DOM report、motion diff report 和 `right_edge_clip_gate`，并人工确认长标题、Run progress、Budget、按钮、chip 不超出框。
- R3.16 的业务状态矩阵验收必须覆盖 `clarify/search/sync/done/offline/approval` 六类真实 Tauri `pet` window capture；证据必须同时看 contact sheet/GIF/MP4、DOM report、motion diff report 和 `right_edge_clip_gate`，并人工确认按钮、chip、evidence refs、英文 offline 文案不超出框。该矩阵只证明真实窗口状态渲染与文本边界，不替代真实审批/检索/同步后端端到端链路。
- R3.17 的 settings/menu 验收必须覆盖 settings matrix 八组合、zh-CN/en-US 右键菜单和黑猫切白猫模型切换；settings matrix 必须满足 `first_frame_bounds_gate.passed=true`，菜单/模型切换必须满足 `settings_menu_layout_gate.passed=true`。本轮修复了比例 canvas 在 scale 下的边界问题和模型切换短提示文字残片问题。
...
3. R3 后恢复：`launcher/en-US` 已升级为真实 Tauri 32 帧 capture，真实 API dev-server launcher-to-run smoke 已落，zh-CN/en-US run-stream completion capture 已落，zh-CN/en-US run-failure terminal capture 已落，zh-CN/en-US 401/403/offline capture 已落，webview boot session/run 恢复已落，launcher chip metadata 进入 WorkItem spec 已落，真实 reload session/active/terminal capture 已落，`clarify/search/sync/done/offline/approval` 当前脚本版本真实业务状态矩阵已落，settings matrix 与右键菜单边界已落；下一步补 pass-through 真恢复、主窗 settings 截图与跨平台 capture。
4. R3 后恢复：持续输出 contact sheet、GIF/MP4、DOM dump、diff report 到本轮审计目录；R3.10 launcher 证据已落 `docs/workhub/05-clients/assets/audit/2026-06-10-cuu-r3-10-sidecar/hijiki/launcher-en-US/`，R3.12 run-stream 证据已落 `docs/workhub/05-clients/assets/audit/2026-06-10-cuu-r3-run-stream/hijiki/`，R3.13.1 run-failure 证据已落 `docs/workhub/05-clients/assets/audit/2026-06-10-cuu-r3-run-failure/hijiki/`，R3.13.2 error-state 证据已落 `docs/workhub/05-clients/assets/audit/2026-06-10-cuu-r3-error-states/hijiki/`，R3.15 reload restore 证据已落 `docs/workhub/05-clients/assets/audit/2026-06-10-cuu-r3-reload-restore/hijiki/`，R3.16 business matrix 证据已落 `docs/workhub/05-clients/assets/audit/2026-06-10-cuu-r3-business-matrix/hijiki/`，R3.17 settings/menu 证据已落 `docs/workhub/05-clients/assets/audit/2026-06-10-cuu-r3-settings-matrix/hijiki/` 与 `docs/workhub/05-clients/assets/audit/2026-06-10-cuu-r3-settings-menu-recovery/hijiki/`。
