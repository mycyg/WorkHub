...
| `POST /merge-proposals/{mpid}/apply` | 对 `ai_fusion` 物化融合稿；text/spec 写 accepted ledger / Drive version / audit，ready + executable `structured_record` 写 WorkItem 标量字段和 `field_merge` audit；未选择时把本次点击写为 `chosen_*`，已选其它候选时拒绝 | 「采用 AI 融合稿」 |
