...
export type AgentMemoryConflictProposal = {
  ...
  resolution_options: Array<{
    id: "keep_current" | "accept_incoming" | "discard_both" | "edit_memory";
    label: string;
  }>;
};
...
export function preferenceMemoryCandidatesFromRun(input: {
  ...
  return explicitMemorySignalsFromRun(result).map((signal) => ({
    ...base,
    category: signal.category,
    key: signal.key,
    valueMd: signal.valueMd,
    confidence: signal.confidence
  }));
}

type ExplicitMemorySignal = {
  category: UserMemoryCategory;
  key: string;
  valueMd: string;
  confidence: number;
...
function runMemoryText(result: AgentLoopResult) {
  const chunks: string[] = [];
  if (result.finalText) {
    chunks.push(result.finalText);
  }
  if (result.reason) {
    chunks.push(result.reason);
  }
  for (const step of result.steps) {
    for (const block of step.assistant) {
...
function explicitMemorySignalsFromRun(result: AgentLoopResult): ExplicitMemorySignal[] {
  const counters: Record<UserMemoryCategory, number> = {
    preference: 0,
    correction: 0,
    recurring_context: 0
  };
  const signals: ExplicitMemorySignal[] = [];
  for (const rawLine of runMemoryText(result).split(/\r?\n/u)) {
    const line = rawLine.trim().replace(/^[-*]\s*/u, "");
    const match = /^(用户偏好|偏好|以后请|用户要求|用户纠正|纠正|常用上下文)[：:]\s*(.+)$/u.exec(line);
    const valueMd = match?.[2]?.trim();
    if (!match?.[1] || !valueMd) {
      continue;
    }
    const category: UserMemoryCategory = match[1] === "用户纠正" || match[1] === "纠正"
      ? "correction"
      : match[1] === "常用上下文"
        ? "recurring_context"
        : "preference";
    ...
    const prefix = category === "preference"
      ...
      : category === "correction"
        ? "explicit_correction"
        : "explicit_recurring_context";
    signals.push({
      category,
      key: `${prefix}_${counters[category]}`,
      valueMd: neutralizeFenceTags(valueMd),
      confidence: category === "correction" ? 0.9 : category === "recurring_context" ? 0.82 : 0.86
    });
  }
  return signals;
}
...
function buildMemoryConflictProposal(input: {
  ...
  return {
    ...
    attention: {
      ...
      actions: [
        ...
        },
        {
          ...
          style: "danger",
          method: "POST",
          href: `/api/memory-conflicts/${input.fallbackId}/resolve/discard_both`
        },
        {
          id: "edit_memory",
          label: "合并成一条",
          style: "secondary",
          method: "POST",
          href: `/api/memory-conflicts/${input.fallbackId}/resolve/edit_memory`
        }
      ],
      cuu_state: "worried",
    ...
    resolution_options: [
      ...
      { id: "discard_both", label: "两条都不要" },
      { id: "edit_memory", label: "手动编辑" }
    ]
  };
}

function memoryConflictProposalFromRow(row: MemoryConflictRow): AgentMemoryConflictProposal {
...
export async function promoteMemory(input: PromoteMemoryInput): Promise<PromoteMemoryResult> {
  ...
  if (mergeResult.status === "conflict") {
    const conflictRepository = input.memoryConflictRepository ?? getDefaultMemoryConflictRepository();
    const conflictRow = await conflictRepository.createOrUpdateOpen({
      workspaceId: input.workspaceId,
      userId: context.sourceActorUserId,
    ...
    const memoryConflict = memoryConflictProposalFromRow(conflictRow);
