...
- **生命周期**：**✅ 停用 + 改密已完成**。
  ...
  - **邀请（out-of-band）**：**✅ 全部完成（地基 + 路由）**。地基：`user_invites` 表（email citext / token_hash 唯一 / invited_by / role / workspace_id / expires_at / accepted_at+accepted_user_id 墓碑 / 软删撤销 / pending partial index）+ 迁移 0026 + `invites` 仓库（create/findActiveByTokenHash/accept 墓碑防重用/revoke/listPendingForEmail），真 PG 往返。路由：`POST /api/auth/invites`（管理员；鉴权先行 401→isAdmin 403→生成 token 一次性返回，服务端只存 sha256，7 天有效）+ `POST /api/auth/invites/accept`（公开入口；findActiveByTokenHash→建 user+createCredential(email_verified_at=now trust-on-invite)+默认成员(invite.workspaceId 或默认, invite.role)+accept 墓碑+mintSession→201）。AuthDependencies.invites? OPTIONAL + 注入。route-auth-posture 把 /invites/accept 列公开白名单、/invites 管理员保护 fails-closed。auth.test.ts +2 端到端测 + OpenAPI 补 4 条（password/deactivate/invites/accept）。
  - **⏭️ 待做**：offboard（昵称改名释放唯一）/ OIDC 抽象（接 0 provider）/ 前端 onboarding 屏 / Phase 5 切换。
