...
| `apps/desktop-webview/src/assets/cuu/*` | Cuu generated asset pack | 已有 10 个业务状态 clip 的绿幕源图、透明 alpha 图和 motion pack atlas |
...
| Cuu 绿幕资产 | 已有 10 clip motion pack，`CuuMotionHint.sprite_state` 业务状态已 full coverage，均保留绿幕源图、透明 alpha 与 `cuu.sprite.json` | 继续补齐 idle / interaction 微动作专用 clip，完成 GPT Image 18 动作绿幕多帧素材、抠图裁切、anchor 对齐、full coverage atlas |
...
- **当前活体行为**：`packages/cuu/src/idle-scheduler.ts` 已提供基础 scheduler，`pet-surface.ts` 会在没有 active card 时 tick 并更新 `data-cuu-idle-action`；`pet-window-bridge.ts` 已把 pointer hover / drag / release 喂给 scheduler，并会调用 Tauri window `startDragging` 或 Rust command fallback；`pet_commands.rs` 已固定 `set_pet_window_mode`、`start_pet_window_drag`、`save_pet_window_position`、`sample_pet_cursor_near`；`main.rs` 已注册这些 command，并把 mode resize/position/show、drag、save-position 执行到真实 Tauri window API。当前还未接真实跨窗口鼠标采样、位置持久化和 full coverage atlas。
...
- **视觉资产（新增硬约束）**：P1 不再接受抽象图标或 procedural CSS 作为完成标准；当前已有 10 clip 绿幕 motion pack，用于证明业务动作可进入运行时。生产必须继续生成绿幕抠图后的 Cuu PNG/WebP sprite atlas，补齐 blink、tail、sleep、wake、drag、tap、wave 等微动作后才算 18 动作完成。
- **两态**：
