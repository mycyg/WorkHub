...
- 已有 6 clip 真实小猫绿幕 motion pack seed，但还没有 18 动作 full coverage atlas、Rive 文件或 Live2D rig。
...
- `apps/desktop-webview/src/assets/cuu/source-green/{idle_breathe,thinking_tail,asking_approval_bounce,carrying_document_step,celebrating_jump,searching_evidence_peek}/`：GPT Image 绿幕 sprite sheets，保留原始绿幕源图。
- `apps/desktop-webview/src/assets/cuu/alpha/{idle_breathe,thinking_tail,asking_approval_bounce,carrying_document_step,celebrating_jump,searching_evidence_peek}/`：本地 chroma-key + despill + edge-contract 后的透明 PNG。
- `apps/desktop-webview/src/assets/cuu/atlas/cuu-p1-motion-pack.png`：P1 motion pack atlas，当前覆盖 6 个核心 clip。
...
- 测试已覆盖：每个 `CuuMotionHint.sprite_state` 都有对应 procedural clip；atlas manifest 可校验真实 motion pack，`thinking_tail` / `asking_approval_bounce` 等已命中真实 clip，未补状态仍可 fallback；idle scheduler 可 deterministic 触发呼吸、尾巴、眨眼、睡觉、醒来、拖动、释放和点击反馈；Rust pet window command plan 与 pet window bridge 可解析 body/card 模式、Tauri-like command 和拖拽 fallback；desktop notice 能输出 `data-cuu-sprite-state`；pet surface 不渲染 `wh-app-shell`；勿扰模式下 urgent 审批不会弹窗但会保留系统通知意图；queue badge CSS 有锚点；偏好加载/存储/归一化和面板 HTML 有测试；`knowledge-search` 可返回 evidence card；`use_for_current_task` 可提交证据并回显 WorkItem card。

仍未完成：

- 18 个动作的正式透明 PNG / WebP full coverage；目前已落 6 clip motion pack seed。
- `cuu.sprite.json` 已有运行时 JSON manifest；仍未达到 full coverage production pack。
...
- Rive / Live2D 高表现力路线。
- full coverage 动作资产与 idle scheduler 的视觉落地：当前 scheduler 语义已落，核心业务 clip 已开始命中 motion pack，但未补状态和 idle 微动作仍会回退到 idle clip。
