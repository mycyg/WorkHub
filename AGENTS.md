...
- **测试不许整段替换 `process.stdout.write`**：`node --test` 把每个文件放子进程跑、父进程解析子进程 stdout 的 TAP 行计数，报告器对上一条测试的 `ok N` 行是异步写出的——落进被替换的窗口就被吞掉，那条测试会从汇总里悄悄消失（`# tests` 变少、`# fail` 仍 0）。要断言结构化日志，用 `@workhub/tools/test-support` 的 `captureStdoutLines` / `captureStderrLines`（捕获且透传）；门禁 `pnpm audit:test-conventions` 会拦赋值写法。
