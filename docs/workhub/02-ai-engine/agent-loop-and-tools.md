...
- `apps/api/src/routes/agent-runs.ts` 默认在 enqueue 后触发 `runNext()` drain，由 PG claim 决定实际执行权；测试可用 `autoRun:false` 隔离 queue 单元行为。
...
- R2.2 已补同一 work item active run DB gate：`agent_runs_work_item_active_uq` 保证 `queued/running` 只允许一条；`createRunIfWorkItemIdle()` 把并发 enqueue 收敛为 `409 agent_run_already_active`；详见 [`r2-multi-worker-pump.md`](./r2-multi-worker-pump.md)。
...
- R2.1/R2.2 已关闭 AgentRun claim/lease 与同 work item active run 真空；R2.3 已补 Redis PushBus / Presence 跨实例后端与 unsubscribe 竞态门；R2.4 已把 `all` topic 收为 admin-only 并固定资源 topic fail-closed。`AgentRunQueue` 仍保留进程内 Map/Set 作为无 DB fallback。完整 R2 仍需补长 LLM call interval heartbeat 与真实 PG/Redis matrix。
...
后续施工必须继续完成 R2.5 PG/Redis 集成测试与长 LLM call heartbeat、完整审批中心和真实 React route 产品化，再回到 Cuu 出站 Agent 入口。
