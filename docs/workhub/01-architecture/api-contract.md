...
| **[新]** `POST /api/proposals/{id}/merge` | `{confirm?: true, conflict_resolution?: {accept_incoming_target_keys?: string[], bulk_action?: {action, target_keys, conflict_count?}}}` | `ProposalMergeResult`(含 `merge_snapshot_id`、rollback、events、audit facts)；409 `merge_conflict` 带 `details.conflicts[]`；R1.42 起批量 keep/accept 会写 `proposal.bulk_action` 审计 | reviewer / 策略自动 |
| **[新]** `POST /api/merge-proposals/{id}/choose` | `{option_key}` | `MergeProposalCandidateChoiceResult`(回显 `chosen_option_key`、`chosen_at`、candidate) | reviewer |
| **[新]** `POST /api/merge-proposals/{id}/apply` | `{confirm?: true, structured_field_overrides?, structured_item_overrides?, task_plan_scope?, text_hunk_overrides?}` | `ProposalMergeResult`；允许未选择的 `ai_fusion` 直接一键采用，服务端会把本次点击写入 `chosen_*` 后物化融合稿，并写入正式 accepted deliverable / Drive version / merge audit；R1.19 起 `text_doc/spec_doc` 直接写候选正文；R1.41 起 `text_hunk_overrides` 可逐段 materialize 最终文本；已选择其它候选时 409 | reviewer |
| **[新]** `POST /api/workitems/{id}/deliverables/{acceptedChangeId}/restore` | — | `{accepted_deliverable}`(恢复后的当前正式交付物 VM)；409 表示没有上一版或版本已变化 | 可见性门 |
| **[新]** `GET /api/workitems/{id}/conflicts` | — | `ProposalConflictListResult`：`{conflicts:[{target_key, merge_proposal_id?, headline, recommended_option_id, options[]}]}`；有 `ai_fusion` 可采用时，option action 直指 `/api/merge-proposals/{id}/apply` | 可见性门 |

**R2.6 REST / Page 资源权限收口(2026-06-10)**:Proposal 读写端点不再只信 route 层登录态或 reviewer 语义，必须先解析所属 `work_item_id` 并调用 `WorkItemService.detailPage({ workItemId, actor })`。覆盖 `POST/GET /api/workitems/{id}/proposals`、`GET /api/workitems/{id}/conflicts`、`GET /api/proposals/{id}`、`POST /api/proposals/{id}/review`、`POST /api/proposals/{id}/merge`、`POST /api/merge-proposals/{id}/choose`、`POST /api/merge-proposals/{id}/apply` 与 `GET /api/pages/proposals/{id}`。`merge-proposals` 两个端点必须先通过 `ProposalService.getByMergeProposal()` 反查 proposal，再验 WorkItem 可见性，通过后才允许写 `chosen_*` 或物化正式交付物。
...
**R1 merge 返回语义(2026-06-09)**:`POST /api/proposals/{id}/merge` 成功时，DB transaction 已写 `proposals/branches/work_items` 状态、`snapshots(kind=merge)`、`accepted_deliverable_changes`、`merge_attempts(result="merged")`、`merge_proposals`、`audit_logs(action=proposal.merged)`。`proposal.merged.detail_json` 会携带 `merge_attempt_id`、`accepted_incoming_target_keys`、`resolved_conflict_target_keys`，把用户选择、merge snapshot 与正式采纳账本串起来。AgentRun-backed delivery 还会把源文件从 `AgentRun.workdir_ref` 复制到正式 storage root，追加 `ProjectDriveVersion`，前移 `ProjectDriveItem.current_version_id`，并把 `drive_item_id/drive_version_id` 写入 accepted row。若同一 target 当前正式版与 incoming `sha256_before/version_before` 不一致，generated 同路径 sha 不同，源文件缺失/越界，或源文件 sha 与 manifest 不一致，返回 409；用户面说“和正式版撞车”或“交付文件已变化”，不显示 merge/conflict 黑话。R1.9 起 `merge_conflict` 的错误体是 `{ok:false,error:{code:"merge_conflict",message,details:{conflicts},recoverable:true}}`，其中每个 conflict 都有 `keep_current` 与 `accept_incoming` 两个可执行 option；`accept_incoming.action.request_json` 固定带 `conflict_resolution.accept_incoming_target_keys:[target_key]`。只有带该 payload 的二次 merge 才允许覆盖当前正式版。R1.10 起 Web/Desktop 主窗会把 `details.conflicts[]` 渲染为严肃 option-first 冲突卡，独立 Cuu pet window 会把同一 payload 作为 Cuu action 提交；R1.11 起默认 merge 被 gate 挡住时也会写 `merge_attempts(result="conflict", conflicts_json)` 后再返回 409，保证 replay 能解释失败尝试；R1.12 起每个 attempt 下还会写 `merge_proposals` deterministic candidates，默认未选择，成功采纳 incoming 时写 `chosen_option_key="accept_incoming"`；R1.13 起 AgentRun replay 会返回 `merge_timeline[]`，展示 attempt、candidate、recommended option 与 chosen option；R1.14 起 API service 会在检测到同一 proposal 的冲突时调用可注入的 `MergeFusionCandidateGenerator`，仅对 `structured_record/text_doc/spec_doc` 尝试生成 `ai_fusion` candidate，候选写入 `merge_proposals.candidates_json` 并可在 409 `details.conflicts[].options[]` 中展示；R1.15 起 `POST /api/merge-proposals/{id}/choose` 可把 `ai_fusion` 或其他候选选择写入 `chosen_option_key/chosen_by_user_id/chosen_at` 并回显 candidate。R1.16 起 `POST /api/merge-proposals/{id}/apply` 可把已选 `ai_fusion.merged_value` 写成正式融合稿，创建 `ProjectDriveVersion`、`accepted_deliverable_changes`、`snapshots(kind=merge)`、`merge_attempts(result="merged")` 与 `proposal.merged` audit，并返回同一 `ProposalMergeResult`。R1.17 起 409 / `GET /api/workitems/{id}/conflicts` 会在 conflict 上带回 `merge_proposal_id`，当 `ai_fusion` 有可用 `merged_value` 时，`ai_fusion` option 直接显示“采用 AI 融合稿”，action 为 `POST /api/merge-proposals/{id}/apply {confirm:true}`；apply 若发现原 row 未选择，会把这次点击作为人工选择写入原 row 的 `chosen_option_key="ai_fusion"`、`chosen_by_user_id`、`chosen_at`，再物化正式交付物。R1.19 起 `text_doc/spec_doc` 的 apply 会从 `merged_value` 提取正文直接写成正式文本文件，正文含 `<<<<<<<` / `=======` / `>>>>>>>` 时返回 409 `merge_candidate_contains_conflict_markers`；其它目标继续物化 Markdown artifact。R1.20 起，生成 `ai_fusion` 候选前会为 `text_doc/spec_doc` 冲突读取真实文本上下文：`current` 来自当前 accepted Drive version，`incoming` 来自 AgentRun workdir 的 manifest path，`base` 在 `version_before/sha256_before` 能匹配历史 accepted row 时读取；三者以截断摘录形式进入 `content_context` prompt，读取失败或非 UTF-8 不阻塞 merge，只降级为缺上下文。R1.21 起，`ai_fusion` candidate 会在 `quality_gate.text_patch_preview` 持久化 current -> merged 的 unified patch 预览、增删行统计和 base/current/incoming overlap risk；该字段会随 replay `merge_timeline[].decisions[].candidates[].quality_gate` 透传。R1.22 起，Gold Path / Replay 严肃页会渲染该 `text_patch_preview`，显示 changed、增删行、overlap risk、base 状态与 unified hunk。R1.23 起，`ProposalConflictOption` 可选携带 `quality_gate`，409 `details.conflicts[].options[]` 与 `GET /api/workitems/{id}/conflicts` 会把 `ai_fusion.quality_gate.text_patch_preview` 下沉到“采用 AI 融合稿” option；`packages/ui/src/proposal/render.ts` 会在冲突卡按钮下显示采用前 diff、增删行、overlap risk 与 base 状态。R1.24 起，`MergeFusionCandidateGenerator` 会先对 `text_doc/spec_doc` 的完整 base/current/incoming 文本做 deterministic line diff3：无重叠 hunk 时直接生成 `source="diff3"` 的 `ai_fusion` candidate、`quality_gate.text_diff3` 与同一 patch preview；重叠 hunk、缺 base、文本被截断或非文本时仍回退到 LLM/两选一；其中重叠 hunk 会携带 R1.25 的 `text_diff3_conflicts[]` 与 `quality_gate.text_diff3` metadata。已选择其它候选时仍返回 409，避免覆盖人的既有裁决。LLM 未配置、失败、输出不合格、目标类型不支持时自动降级为原两个安全按钮；R1.24 后，低风险无重叠文本合并即使无 LLM 也可产生可审计 candidate；R1.25 后，重叠文本调解也有可回放的 hunk metadata。公开 merge 返回体暂不展开正式交付物 VM，WorkItem page 与 AgentRun replay page 已提供 accepted deliverables。R1.25 仍不是字段级结构化 patch、重叠 hunk 逐项确认/编辑、React route 级富 patch viewer 或多冲突工作台；结构化字段 patch、重叠 hunk 逐项确认/编辑、富 viewer 和多冲突工作台属于后续 `ai_resolved` v2。

**R1.41 text hunk apply 语义(2026-06-10)**:`POST /api/merge-proposals/{id}/apply` 可接收 `text_hunk_overrides:{hunks:[{hunk_index,start_line,end_line,decision}]}`。该能力仅支持 `text_doc/spec_doc`，并以 `quality_gate.text_diff3.conflict_ranges[]` 为权威范围；service 会读取 full base/current/incoming/AI fusion 文本，拒绝缺 range、重复 hunk、range mismatch、未覆盖全部 conflict ranges、stale current/base/source、非文本 target 与冲突 marker。成功时生成最终文本，写正式 Drive version、accepted ledger、merge snapshot、merge attempt 和 `proposal.merged` audit；audit detail 包含 `text_hunk_overrides`、`text_hunk_decisions`、`text_hunk_conflict_ranges`、`text_hunk_count` 与 base/current/incoming/output sha。R1.43 起 Replay 会把这些字段渲染为用户可读逐段来源回放。

**R1.42 multi-conflict bulk action 语义(2026-06-10)**:`POST /api/proposals/{id}/merge` 可接收 `conflict_resolution.bulk_action:{action:"keep_current"|"accept_incoming",target_keys:string[],conflict_count?}`。该字段只记录用户点击的批量意图，不改变现有 merge gate：`keep_current` 仍以 `accept_incoming_target_keys=[]` 进入冲突检测，若冲突未解会返回 409；`accept_incoming` 仍必须覆盖需要采用 incoming 的 target。repository 会在 conflict 与 merged 两条路径写 `audit_logs(action="proposal.bulk_action")`，detail 包含 `bulk_action`、`result`、`merge_attempt_id`、`accepted_incoming_target_keys`、`resolved_conflict_target_keys`、`blocked_target_keys` 与 `target_keys`；成功 merge 的 `proposal.merged.detail_json` 也会带 `bulk_action` 摘要。R1.43 起 Replay 会把批量动作、点击范围、采纳范围、已处理和被阻断目标渲染为用户可读回放。

**R1.43 replay hunk / bulk audit 语义(2026-06-10)**:`GET /api/agent-runs/{id}/replay` 组装 `merge_timeline[]` 时，会读取 proposal 级 audit rows 并按 `detail_json.merge_attempt_id` 绑定到具体 attempt。`ReplayMergeAttemptVM` 新增 `text_hunk_decisions[]`、`text_hunk_count?`、`text_hunk_output_sha256?` 与 `bulk_action?`。`text_hunk_decisions[]` 每项含 `hunk_index/start_line/end_line/decision`；`bulk_action` 含 `action/target_keys/conflict_count/result/accepted_incoming_target_keys/resolved_conflict_target_keys/blocked_target_keys/audit_id`。该字段只解释历史，不开放新写操作；缺失或畸形 audit 不阻断 Replay 主体渲染。

**R1 replay / accepted deliverable 读取与还原语义(2026-06-09/10)**:`GET /api/pages/workitems/{id}` 与 `GET /api/agent-runs/{id}/replay` 均返回 `accepted_deliverables[]`，每项包含 `drive_item_id`、`drive_version_id`、`filename`、`mime`、`size_bytes`、`download_href`、可选 `preview_href`，以及仅在 `accepted_version > 1` 时出现的 `restore_href`。`GET /api/agent-runs/{id}/replay` 还返回 `merge_timeline[]`：每个 attempt 包含 `proposal_id`、`work_item_id`、`result`、`conflict_count`、`target_keys`、`accepted_target_keys`、`conflicts[]`、`decisions[]`、R1.43 的 `text_hunk_decisions[]` 与可选 `bulk_action`；每个 decision 包含 `conflict_key`、`recommended_option_key`、`chosen_option_key`、`chosen_by_user_id`、`chosen_at` 与 `candidates[]`，candidate 包含 `option_key`、可选 `target_kind` / `rationale_md` / `merged_value` 以及 `recommended/chosen` 布尔值。`GET /api/workitems/{id}/deliverables/{acceptedChangeId}/download` 返回正式文件内容，不暴露 `storage_path`；若账本存在但正式文件已丢失，返回 404 而不是暴露内部路径。`GET /api/workitems/{id}/deliverables/{acceptedChangeId}/preview` 仅支持文本类预览，返回 `{preview_type:"text", text, truncated}`，非文本返回 415 并提示下载查看。`POST /api/workitems/{id}/deliverables/{acceptedChangeId}/restore` 会校验当前 Drive item 仍指向该 accepted row 的版本，然后把 `ProjectDriveItem.current_version_id` 指回上一版 accepted row 的 `drive_version_id`，将上一版重新设为 current accepted row，并写 `ProjectDriveOperation(op_type="restore_version")` 与 `AuditLog(action="accepted_deliverable.reverted")`。首版无上一版时返回 409，用户面文案说“还没有上一版可还原”，不显示 snapshot/revert/commit 黑话。
...
**R2.6 Approval 资源权限收口(2026-06-10)**:`ApprovalService` 仍负责“是否路由给当前人 / admin 是否可处理”的审批身份门；route 额外按 `request.work_item_id` 调用 WorkItem 可见性门。`GET /api/approvals` 会过滤不可见 WorkItem 的 request/item/count；`POST /api/approvals/{id}/respond` 与 `delegate` 先 `ApprovalService.get(id)`，再验 WorkItem，通过后才写响应或委派。`GET /api/pages/approvals` 与 REST list 保持同一过滤语义。没有 `work_item_id` 的系统级审批仍按原审批身份门处理。
