...
> **历史绿幕素材方案**：见 [`cuu-green-screen-desktop-pet-solution.md`](./cuu-green-screen-desktop-pet-solution.md)。它仍可解释“独立透明 `pet` window”和截图 QA 的来源，但绿幕 sprite atlas 已被当前 Live2D 二选项替代。
> **原创 Live2D 高表现力方案**：见 [`cuu-live2d-layered-asset-plan.md`](./cuu-live2d-layered-asset-plan.md)。它现在是未来原创模型替换路线，不再指向当前 PSD draft 或 sprite fallback。
> **2026-06-08 二选项收束**：当前源码已经废弃绿幕 sprite atlas、Bongo/CSS、PSD draft 和主窗 Cuu rail，Cuu 只在独立 `pet` window 中以黑猫 Hijiki / 白猫 Tororo 两个 Live2D Cubism 2 选项出现。Rust/Tauri 的窗口、托盘、通知、deep-link、pass-through、hide-on-hover、多屏恢复等仍按本篇推进；视觉模型包与 QA 以 [`cuu-live2d-cat-options-current-plan.md`](./cuu-live2d-cat-options-current-plan.md) 为准。
> **当前截图 / 动作审计**：见 [`current-state-visual-audit-and-construction-plan-2026-06-07.md`](./current-state-visual-audit-and-construction-plan-2026-06-07.md)。2026-06-07 已对 Web、desktop webview、browser pet preview 和真实 Tauri `Cuu` 窗口做截图，并对 `Cuu` 顶层窗口做 32 帧 motion capture。首轮发现 card mode 事件卡裁切；第一轮 card layout 又暴露 Cuu 只露耳朵 / 局部的失败样例；随后又发现静态 fallback 呼吸/缩放不能算鲜活动作。2026-06-08 多批 Bongo / PSD / sprite 真实录屏证明动作合同和 QA 链路可行，但用户已明确否决这些视觉路线；当前默认可见层已改为独立 `pet` surface 的黑猫 Hijiki / 白猫 Tororo Live2D Cubism2 runtime。Web / desktop 主窗已撤回 Cuu rail、Cuu notice、queue badge 和主窗 Cuu 偏好入口。
> **已撤回的 Web rail 生成图资产**：2026-06-08 追加过 `cuu-polished-idle-v1`，用 GPT Image 生成绿幕源图并抠成透明 PNG，曾用于修补 Gold Path / desktop 主窗右侧 Cuu rail。该主窗 rail 方案已撤回；图片只保留为历史审计和后续独立桌宠 / Live2D 重绘参考。
...
| `client-tauri/src-tauri/src/main.rs` | Tauri runtime entry scaffold | 接 `tauri::Builder`、`generate_context!`、`invoke_handler`，注册 pet/window command；setup 会先用 `WebviewWindowBuilder::from_config` 动态创建 `create:false` 的 `pet` window，并注入 `window.__WORKHUB_SURFACE__="pet"`；随后恢复/夹取 `pet-window-state.json` 并预定位 body-only Cuu，但不在 webview first paint 前 show；`set_pet_window_mode` 已执行 resize / position / show，且在 show/toggle、mode 切换时显式保持 `pet` always-on-top；`set_pet_window_settings` 已能同步 scale / opacity / pass-through / hide-on-hover，其中 scale 进入 body/card 几何，pass-through 执行 Tauri `set_ignore_cursor_events`，hide-on-hover 作为 webview 可恢复 soft dodge 设置；pet work area、window position 与 cursor sampling 已做 HiDPI physical→logical 换算，避免高缩放屏离屏；`start_pet_window_drag` 已执行 `start_dragging`，`save_pet_window_position` 已读取真实窗口位置并保存 body anchor，`sample_pet_cursor_near` 已读取真实 cursor 与 pet window rect，并返回 `look_x_percent/look_y_percent` 给 webview 形成连续凝视；setup 已加载 shell config、安装 WorkHub 托盘/deep-link/notification/single-instance plugin，并按配置启动 SSE worker；`WORKHUB_DISABLE_SSE=1` 仅作为 QA motion capture 开关，用于隔离纯输入手感录屏中的离线卡片干扰；`WORKHUB_CUU_QA_PET_SCALE_PERCENT` / `WORKHUB_CUU_QA_PET_OPACITY_PERCENT` / `WORKHUB_CUU_QA_PET_PASS_THROUGH` / `WORKHUB_CUU_QA_HIDE_ON_HOVER` 仅作为真实截图矩阵和 hide-on-hover 录屏的 QA 注入开关 |
...
| `client-tauri/src-tauri/src/pet_window.rs` | Cuu pet window geometry contract | `body_only` / `card` 双模式尺寸、右下角定位、从小猫锚点向左上展开、屏幕内夹取、鼠标接近判定、拖拽 plan；新增 `PetWindowSettings`，支持 75/100/125/150 scale、60/80/100 opacity 和 pass-through，并让 scale 后的 body/card 仍保持同一右下角 anchor；P1e-c 新增 `look_x_percent/look_y_percent`，用窗口中心和 near radius 算出 `-100..100` 指针方向 |
| `client-tauri/src-tauri/src/pet_commands.rs` | Cuu pet window command scaffold | 固定 `set_pet_window_mode`、`set_pet_window_settings`、`start_pet_window_drag`、`save_pet_window_position`、`sample_pet_cursor_near` command 名称和 typed plan，新增 body anchor 防漂移、active mode rect、cursor decision helper、look percent pointer plan 与 settings confirmation plan |
...
| `apps/desktop-webview/src/desktop-cuu-runtime.ts` | Cuu event/card bridge | 从 Tauri/mock listener 订阅 `push-event` / `sse-status` / `system-notification`；push/status 生成 `CuuCard` 供独立 `pet` surface 消费，主窗不再嵌入 Cuu notice |
| `apps/desktop-webview/src/cuu-preferences.ts` | Cuu preference model / pet-side panel | 本地存储 `attention_mode` / `sound_mode` / `reduced_motion` / `queue_limit` / `pet_scale_percent` / `pet_opacity_percent` / `pet_pass_through` / `pet_hide_on_hover` / `pet_model_pack_id`；后续 UI 应在独立 `pet` window、托盘或客户端设置承接，不再作为主窗右上角角色入口 |
| `packages/contracts/src/locale.ts` | shared locale contract | `WorkHubLocale`、`workHubLocaleStorageKey`、`defaultWorkHubLocale` 与 `normalizeWorkHubLocale()` 的单一真相源；Web、desktop webview、API client、Cuu/pet 均复用 |
| `apps/desktop-webview/src/browser.ts` | webview preview shell | 读取 `/api/pages/gold-path?locale=...`；主窗已接 `workhub.locale` 中英双语切换，Gold Path 静态 chrome、审批原因按钮、动作失败/未接线提示会随语言变化；启动时按 Rust injected `__WORKHUB_SURFACE__`、Tauri window label、`/pet`、`?surface=pet`、`#surface=pet` 或 `pet.html` 分流到 Cuu pet surface；否则只加载严肃主壳并订阅 Rust `navigate` 事件，不再注入 Cuu notice、queue badge 或 Cuu 偏好按钮 |
| `packages/ui/src/gold-path/i18n.ts` | shared client i18n foundation | 复用 contracts locale，提供 `goldPathT()` 与 Gold Path 固定 chrome 词表；Web 与 desktop webview 共用 |
| `packages/cuu/src/i18n.ts` | Cuu fixed-copy i18n | Cuu card adapter、pet surface、desktop shell status 的固定标题、按钮、状态、fallback message 中英双语；动态 event summary 保留原文 |
| `packages/ui/src/gold-path/render.ts` | Gold Path Settings page | `/settings` 页面改为严肃应用设置，只展示 AI runtime、桌面客户端、预算和语言；不再展示 Cuu rail、Cuu model pack 或 `./assets/cuu/*` 图片 |
| `apps/desktop-webview/src/pet-surface.ts` | Cuu pet webview surface | `pet` 窗口入口只渲染独立 Live2D Cuu 与一张轻气泡，不加载 Gold Path 主壳；默认可见层为黑猫 Hijiki Cubism2 iframe，白猫 Tororo 可选；旧 Bongo / sprite / atlas / PSD draft / 静态 fallback 均不进入 DOM；card mode 只有在 Rust placement 确认后渲染 full card，未确认或失败时走 compact fallback；P1.2 已把 `CuuCard.kind` / `priority` / `sections` / `progress` / `evidence_refs` / `input` 渲染进轻卡 |
| `apps/desktop-webview/src/pet-surface-qa.ts` | Cuu pet visual QA contract | 静态检查透明 root、CSS 变量驱动的右下角独立 surface、点击/拖拽热区、非主壳、Live2D cat runtime、黑猫/白猫二选项、pointer-reactive pose、hover soft-hide 属性、轻气泡、选项优先、heavy card context 和默认窗口手感属性；新增门禁禁止默认回到 Bongo/CSS、sprite/atlas 或 PSD draft |
| `apps/desktop-webview/src/pet-window-bridge.ts` | Cuu pet webview input/window bridge | 解析 mock / Tauri-like bridge，支持 `body_only` / `card` 模式切换、`setSettings(scale/opacity/pass-through/hide-on-hover)`、`startDragging`、位置保存、Rust cursor sample command plan 与 browser fallback；现在支持 `__TAURI__.core.invoke` / legacy `__TAURI__.invoke`，并校验 `set_pet_window_mode` placement 和 `set_pet_window_settings` settings 返回值；P1e-c 新增 `DesktopPetPointerSnapshot` 的 `look_x/look_y/avoidance_x/avoidance_y/hover_avoidance`，把 Rust percent 和 DOM pointer event 统一成 Live2D 可复用输入端口；P1e-d-a 新增 pointer smoothing helper；P1d-b-a 会在 Rust sample 确认 cursor 已离开窗口和 near radius 时清掉本地 hover，保证 soft hide 后可恢复 |
| `packages/cuu/src/model-pack.ts` | Cuu model pack registry / loader | 当前只保留 `cuu-hijiki-live2d-cubism2` 黑猫与 `cuu-tororo-live2d-cubism2` 白猫；`listCuuModelPacks()` / `describeCuuModelPackChoices()` / `resolveCuuVisibleModelPack()` 已落，旧 pack id、Bongo、PSD draft、sprite/atlas 和未知请求统一回退黑猫；scale / opacity / pass-through / hide-on-hover 仍由 pet window settings 支持 |
| `apps/desktop-webview/src/cuu-cat-live2d-runtime.ts` | Cuu Live2D cat renderer | 根据模型包选择 Hijiki / Tororo Cubism2 html/model.json，输出 `live2d_cubism2_cat` runtime 与 `approved_cat_option` 状态；同一输入合同后续映射到眨眼、看鼠标、任务动作和窗口 QA |
| `apps/desktop-webview/public/cuu/live2d/{hijiki,tororo}/*` | Cuu approved Live2D asset pack | 当前源码内置的两个 approved cat option；Hijiki 为默认黑猫，Tororo 为可选白猫；商用发布前必须确认授权或替换为原创等效模型 |
...
| `scripts/qa/cuu-tauri-motion-capture.ps1` | Windows Tauri motion capture | 启动真实 debug app，先用 `first-frame-probe.png` 等到 Cuu 橘色/有效视觉像素达标，再连续抓取 `Cuu` 顶层窗口 frames，输出 contact sheet、diff JSON、GIF/MP4；用于验证 Cuu 是否真实有动作、事件卡是否进入 card mode、是否被裁切；P1e-b 提供 `input-handfeel`，P1e-c 提供 `look-avoidance`，P1e-d-a 提供 `drag-smoothing`，P1d-b-a 提供 `hide-on-hover`，并支持 scale / opacity / pass-through / hide-on-hover QA 参数 |
| `scripts/qa/cuu-tauri-settings-capture.ps1` | Windows Tauri settings matrix capture | 包装 motion capture，一次验证 default、scale-75、scale-150、opacity-60、pass-through、hide-on-hover、combo-125-80-pass-hide，输出 `cuu-settings-contact-sheet.png` 与 `settings-capture-report.json` |
...
| 独立桌宠窗 `pet` | 已有 `ShellWindowPlan` + Tauri window config + `show/hide/toggle` control plan；生产 Tauri 由 Rust setup 动态创建 `create:false` 的 `pet` window 并注入 pet surface flag，浏览器调试保留 `/pet` / `?surface=pet` / `#surface=pet` / `pet.html`；`pet-surface-qa.ts` 已守住透明 root、右下角、非主壳、Live2D cat runtime、黑猫/白猫二选项、选项优先和 heavy card context；`pet_window.rs` 已固定 body-only/card 几何、右下角定位、展开锚点、鼠标接近、拖拽和 settings plan；`main.rs` 已把启动期 body-only 预定位、first-painted 后 show、pet show/hide/toggle、mode resize/position/show、drag、save-position、cursor sample、scale/opacity/pass-through/hide-on-hover settings 执行到真实 Tauri window / AppHandle API，并处理 HiDPI 坐标换算与运行期 topmost；托盘菜单可 toggle Cuu；2026-06-07 / 2026-06-08 的 Bongo、atlas、PSD 录屏保留为历史 QA 证据，不代表当前视觉实现；`skipTaskbar` 仍在 WorkHub plan | transparent / decorations false / always-on-top / skip taskbar；后续补 Live2D 黑猫/白猫真实 Tauri card fixture capture、多屏恢复实测、安装包 smoke、系统通知点击、full hide/pass-through 安全恢复和跨平台透明 capture |
| Cuu 视觉资产 | 绿幕 clip、sprite atlas、Bongo DOM/CSS 与 PSD draft probe 已作为失败/历史路线退出源码运行时；当前只保留 Hijiki 黑猫与 Tororo 白猫两个 Live2D Cubism2 资产包 | 先跑通两个 approved cat option 的独立 pet window 录屏 QA；后续若做原创模型，再按 Live2D 分层资产方案重绘、授权、绑定和替换 |
...
| Cuu 偏好 / shell 设置 | 本地存储与 `pet-window-bridge.ts` 仍保留尺寸 75/100/125/150、透明度 60/80/100、点击穿透、`pet_hide_on_hover`、`pet_model_pack_id` 等字段；P1d-c 已用真实 Tauri 设置矩阵确认这些偏好能进入 pet window。主窗 `/settings` 和 browser shell 不再展示 Cuu 偏好或模型包按钮；后续偏好 UI 应迁到独立 `pet` window、托盘或客户端设置层 | 继续接设备注册、安全 vault、托盘显隐、多屏恢复实测、系统通知偏好和 full hide/pass-through 安全恢复；Live2D 仍需 model pack gate 通过后才能开放 |
| 中英双语动态内容 | `GET /api/pages/*` 已支持 `locale` query 并回 `meta.locale`；Cuu card adapter、独立 pet 轻气泡、desktop shell status/action result 的固定文案已支持 `zh-CN` / `en-US`；动态标题/摘要/证据/proposal manifest 仍跟随 daemon 原文 | 后续补 `me.locale` 偏好、非 Gold Path render helpers 词表、服务端按 locale 生成 summary、Live2D pet card fixture 中英截图验收 |
...
![Cuu Bongo settings matrix real Tauri screenshots](./assets/audit/2026-06-08-cuu-bongo-p1d-settings/cuu-settings-contact-sheet.png)

- **当前窗口设置矩阵**：P1d-c 已用 `scripts/qa/cuu-tauri-settings-capture.ps1` 在真实 Tauri `Cuu` 顶层窗口里验证 default、scale-75、scale-150、opacity-60、pass-through、hide-on-hover、combo-125-80-pass-hide。`settings-capture-report.json` 中 `passed=true`，所有 case 首帧非空、Cuu 全身可见，不再出现只露耳朵或缩放后裁切。它证明窗口能力合同已接到真实 pet window；主窗 Settings 里的模型包 UI 已撤回，后续模型包/外观选择只应在独立桌宠或客户端设置层承接。
...
- **当前 webview surface**：`apps/desktop-webview/src/browser.ts` 会按 Rust injected `__WORKHUB_SURFACE__`、Tauri window label、`/pet`、`?surface=pet`、`#surface=pet` 或 `pet.html` 分流到 `pet-surface.ts`，该 surface 只渲染 Live2D 黑猫/白猫 Cuu 和一张轻气泡，不加载 Gold Path 主壳；生产 Tauri `pet` window 加载根 bundle，由 Rust 初始化脚本选中 pet surface，避免刷新/静态路由 fallback 问题。
- **当前活体行为**：`packages/cuu/src/idle-scheduler.ts` 已提供基础 scheduler，`pet-surface.ts` 会在没有 active card 时 tick 并更新 `data-cuu-idle-action`；当前可见层只接受 `cuu-hijiki-live2d-cubism2` 黑猫与 `cuu-tororo-live2d-cubism2` 白猫两个模型包，旧 Bongo、PSD draft、sprite/atlas 和未知 pack id 回退黑猫。P1e-a 后 `cursor_near` 首次进入附近区域会立即触发 `look_at_mouse`；P1e-c 后 surface 同时输出 `data-pet-cursor-near` / `data-pet-hovered` / `data-pet-dragging` / `data-pet-look-x/y` / `data-pet-hover-avoidance`；P1e-d-a 后 surface 输出 `data-pet-pointer-smoothing-alpha="0.58"`，Rust sample 会低通平滑，dragging 时保持 `drag_hold`；P1d-b-a 后 surface 输出 `data-pet-hide-on-hover` / `data-pet-hover-hidden` 并在 hover 时做可恢复 soft dodge。`pet-surface-qa.ts` 已把透明 root、右下角独立 surface、点击/拖拽热区、非主壳、Live2D cat runtime、黑猫/白猫二选项、pointer-reactive pose、smoothing alpha、hide-on-hover attrs、轻气泡、选项优先卡片和窗口手感属性做成静态门禁；`pet-window-bridge.ts` 已把 pointer hover / drag / release 与 Rust cursor sample 喂给 scheduler，并会调用 Tauri window `startDragging` 或 Rust command fallback；`pet_commands.rs` 已固定 `set_pet_window_mode`、`set_pet_window_settings`、`start_pet_window_drag`、`save_pet_window_position`、`sample_pet_cursor_near`，其中 cursor sample 会返回 `look_x_percent/look_y_percent`；`main.rs` 已注册这些 command，并把启动期 body-only 预定位、first-painted 后 show、mode resize/position/show、settings scale/opacity/pass-through/hide-on-hover、drag、save-position、cursor sampling 执行到真实 Tauri window / AppHandle API，拖拽位置保存到 Tauri Config `pet-window-state.json`，启动时会 clamp 回当前 work area；所有 monitor work area、window outer position 和 cursor position 会从 physical px 转成 logical px，防止 HiDPI 离屏；运行期会显式设置 `set_always_on_top(true)`，保证 Cuu 不被主窗盖住。当前仍缺黑猫/白猫真实 Tauri motion capture、安装包 smoke、跨平台透明 capture、多屏恢复实测、长驻 CPU/GPU 采样和 full hide/pass-through 安全策略。
...
- **当前偏好模型**：`apps/desktop-webview/src/cuu-preferences.ts` 仍提供偏好存储、归一化和可渲染面板能力；但主窗不再挂载右上角 Cuu 轻入口。展开 UI 后续应迁到独立 `pet` window、托盘或客户端设置层。偏好写入 localStorage 后仍可通过 `pet-window-bridge.ts` 调用 `set_pet_window_settings` 更新独立 pet window。
...
- **视觉资产（新增硬约束）**：P1 不再接受恐怖谷 PSD、抽象图标、CSS/线条临摹、Bongo、sprite/atlas 或 inline 静态 fallback 作为完成标准；当前默认可见层只使用黑猫 Hijiki / 白猫 Tororo Live2D 二选项。真实运行态必须显示可辨认的非缩放动作；静态 fallback 只能用于诊断或非运行兜底，不能作为 motion QA 通过依据。生产前继续做黑/白两套真实录屏、任务动作映射、full hide/pass-through 安全恢复、授权确认或原创等效模型替换、长时间 idle 性能检查。
- **两态**：
...
- **主窗视觉边界**：Web / desktop 主窗不再显示 Cuu 右侧栏、Cuu notice 或桌宠偏好按钮；`cuu-polished-idle-v1` 仅保留为历史参考和后续独立桌宠素材参考。独立桌宠窗口仍必须通过多帧 motion capture 才算鲜活感通过。
...
> **当前 WorkHub 状态**：Rust 已有启动期 `workhub-shell-config.json` + env loader，可让 SSE worker 在有 token 时连接 `/me`；BONGO-P2a-b 已补 Gold Path `/settings` 模型包 UI 与中英截图；但 webview Settings 写盘、设备注册、secure vault、worker restart 尚未落地。以下旧项目锚点仍作为行为目标。
...
- Cuu 模型包（边界修正）：主窗 `/settings` 不再展示模型包；`describeCuuModelPackChoices()` 仍可供独立 `pet` window、托盘或客户端设置层使用。保存只写 `normalizeCuuSelectableModelPackId()` 允许的 `pet_model_pack_id`，避免 PSD / Live2D 实验线被误设为默认。
...
- **现状**：`Cargo.toml` 已有 Tauri runtime 依赖和基础插件，但尚未接 `tauri-plugin-updater` / `tauri-plugin-process` / `tauri-plugin-autostart`；`tauri.conf.json` 无 `updater` 配置，故目前无应用内自动更新。`tauri-plugin-process` / `autostart` 属后续目标，不应误判为当前已接。
- **WorkHub 演进（建议）**：
...
- **点击模型**：idle 时窗口可小尺寸跟随 Cuu 外接矩形；展开时扩大交互区域。P1d-a 已接 `pass_through` 到 Tauri `set_ignore_cursor_events`；P1d-b-a 已接可恢复的 `pet_hide_on_hover` soft dodge，hover 时只做缩小/透明/位移，不直接让窗口不可点。后续若升级到 BongoCat 式全透明隐藏或真实 pass-through hide，必须先落托盘、快捷键、边缘热区和自动恢复策略，否则用户可能无法重新抓住 Cuu。
...
- P1 默认必须能不依赖专业绑定工具运行，因此当前采用真实 PNG `sprite_atlas` / `img_stack` 作为默认可见层；BongoCat 思路只保留为 fallback、动作 storyboard 和模型包门禁参考。主窗右侧 rail 的 GPT Image 透明 PNG 方案已撤回，不替代独立 pet window motion gate。
- Sprite atlas / Hatch pack 作为 fallback 与动作素材参考，不能替代默认可爱度验收。
- Live2D 是 Cuu 长期高表现力主线：用 GPT Image / 人工精修产出分层 PSD，Cubism 绑定后导出 `.model3.json`；但只有美术 QA、真实 Tauri 录屏和 `assertCuuModelPackCanBeDefault()` 都通过后，才允许替换 `sprite_atlas` 默认。
