...
| R4-7 Web live API/PG seed smoke | 在 R4.5 mock live-browser smoke 基础上接真实 API server + deterministic PG seed，覆盖 Home/Approvals/WorkItem/Proposal/Replay/Cost/Settings 的 path nav、locale query/meta、文本边界和无 Cuu 主窗门。 | 后续 `pnpm qa:r4-web-live-api-pg-seed` |
