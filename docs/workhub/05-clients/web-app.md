...
| R4 route tree / hydration boundary | `packages/ui/src/gold-path/route-components.ts`、`packages/ui/src/gold-path/product-shell.ts`、`apps/web/src/routes.ts`、`apps/web/qa/r4-web-live-route-interaction.ts` | R4.16 已给所有 active route components 建立 `data-r4-hydration-*` boundary、`webReactRouteTree` Page VM truth registry 和 active-only/action parity browser gates；本机 Chrome smoke 38 步通过 |
...
| R4.20 Dataflow foundation | `apps/web/src/routes.ts`、`apps/web/src/browser.ts`、`apps/api/src/sse/stream.ts`、`packages/events/src/sse.ts`、`packages/ui/src/gold-path/product-shell.ts`、`apps/web/qa/r4-web-live-route-interaction.ts` | R4.20 已把 ready route 从 `/api/pages/gold-path` fixture chrome 剥离，改为 active Page VM + product shell source；SSE runtime 升级为 app-level EventSource reuse，事件后只重拉当前 route Page VM 或 Home React props；SSE frame 支持 `id:`，客户端持久化 cursor 并重连携带 `last_event_id`；42 步 Chrome smoke 通过且 `goldPath=0` |
...
| 项 | 当前实现 | 后续目标 |
|---|---|---|
| Route components | `packages/ui/src/gold-path/route-components.ts` 输出 WorkItem / Proposal / Cost / Settings 四个 Web route component，均带 `data-r4-route-component`、`source=page-vm` 与 route-specific count marker | R4.12 把 action/notice feedback 接到统一 locale contract；R4.13 收敛 Proposal advanced editor |
...
边界：R4.11 仍不是完整 React SPA；它完成的是 ready route componentization 的第二刀。Proposal 的高级 conflict workbench、field editor、line editor、subrecord editor 已在 R4.13 后续模块收敛到 route component；用户输入、证据摘录、manifest、LLM 正文继续保持 VM 原文。
...
| 项 | 当前实现 | 后续目标 |
|---|---|---|
| Advanced route sections | Proposal route component 读取 route surface 的 `proposal_conflicts`，渲染 conflict workbench、line editor、structured field editor、subrecord editor，并暴露 `data-r4-proposal-conflicts/line-editor/field-editor/subrecord-editor` | R4.14 已把同样 active-only/marker discipline 扩到 Intake / Knowledge fallback |
| Data truth | `/proposals/:id` 先读 Proposal Page VM，再读 `/api/workitems/:id/conflicts` 过滤当前 proposal；`ProposalDetailVM` 不被冲突缓存污染 | 后续逐步减少 `gold-path` template 对 detail route 内容的依赖 |
| Action dispatcher | `apps/web/src/browser.ts` 支持 button-style `data-action-href` / `data-request-json-template`；line/task-plan/subrecord/custom field apply 走 typed REST mutation；空 custom field 显示 `field_value_required` notice 且不发 mutation | 后续 Intake submit / Knowledge fallback action 复用同一 fail-closed notice contract |
| Visual QA | `pnpm qa:r4-web-live-route-interaction` 以 R4.13 env 输出 29 步 report/contact sheet，`r4_13_proposal_advanced_route_sections`、`r4_13_advanced_apply_payloads`、`r4_13_custom_field_fail_closed`、`r4_13_structured_editor_visual_no_overflow` 全 true | R4.14 已增加 intake/knowledge mobile no-overflow |

边界：R4.13 不是完整 React SPA，也不改变后端 merge 语义。它把已有 Proposal advanced helper 和 candidate apply payload 产品化到 Web route，并通过截图、DOM marker、请求计数证明没有变成 IDE、WorkItem 工作台或第二套审批中心。
...
| 项 | 当前实现 | 后续目标 |
|---|---|---|
| Intake route | `/intake/:sessionId` 先读 `client.getSession()`，再注入 `intake_session` route surface；component 暴露 `source=session-vm`、option count、progress count、collapsed free text marker | R4.15 保持 option-first contract，同时把 Settings/locale/device boundary 做硬化 |
| Intake actions | 空选择显示 `intake_option_required` 且不发 mutation；选择后 `nextQuestion()` 刷新 confirm；confirm 后 `createWorkItem()` 导航 WorkItem | 后续真实 React route tree 迁移时继续复用同一 fail-closed payload materializer |
| Knowledge route | `/knowledge/search` 解析 `q/project_id/work_item_id`，调用 `client.searchKnowledge(...,{ locale })`，component 暴露 `source=evidence-bubble`、evidence/action/missing marker | 后续补完整搜索筛选、索引状态、历史 run，但仍必须显示来源/缺失说明 |
...
| 项 | 当前实现 | 后续目标 |
|---|---|---|
| Hydration boundary | 每个 `WebRouteComponent` 带 `hydration` 元数据，HTML 输出 `data-r4-hydration-boundary/route/source/locale/page-vm/action-count/adapter` | R4.17 用同一边界承载首批真实 React-compatible route components |
| Route tree registry | `apps/web/src/routes.ts` 暴露 `webReactRouteTree`，ready root 带 route-tree key、Page VM、mode、adapter、active-only 和 route count | 后续迁移时保持 typed loader truth，不让 component 自行 fetch |
| Active panel | `product-shell.ts` 在 active panel 暴露 `data-r4-hydration-panel`，browser smoke 证明 ready route 只有一个 panel 和一个 hydration panel | 真实 component mount/unmount 也必须遵守单 active route lifecycle |
| Visual QA | 本机 Chrome 38 步 smoke 覆盖 hydration boundary、Page VM truth、action parity、locale/settings regression、active-only regression 与 R4.15 Settings boundary regression | R4.17 增加 React component marker 与 HTML fallback parity |

边界：R4.16 不是完整 React component migration；它只建立可验证的迁移边界。所有 mutation 仍走现有 delegated browser action dispatcher，Settings / desktop boundary 和 no Cuu/no secret/no overflow gates 均不降级。
...
| 项 | 当前实现 | 后续目标 |
|---|---|---|
| Component adapter | `route-react-components.ts` 定义 Home / Settings typed props、component name、fallback state、action hrefs 与 fingerprint | R4.18 已扩展到 Cost / Replay |
| HTML fallback parity | route section 与 hydration root 暴露 `data-r4-react-component-*`、`data-r4-hydration-react-component-*` | 后续引入真实 React runtime 前继续保持 fallback 可用 |
| Route tree truth | `webReactRouteTree` 标出 Home / Settings component adapter，ready root 与 hydration root marker 一致 | 不让 component 自行 fetch 或从 DOM 反推数据 |
| Visual QA | 本机 Chrome 38 步 smoke 覆盖 R4.17 component marker、fallback parity、single action path、Settings boundary 与 R4.16 hydration regression | R4.18 已增加 Cost / Replay parity gates |

边界：R4.17 不引入 React runtime dependency，不调用 `hydrateRoot()`，不改视觉，不新增第二套 click/mutation handler。Home 仍是 AI-first 工作台，Settings 仍是严肃管理页；主窗继续无 Cuu、无 secret、无 Kanban/hash/weekly 文案、无 horizontal/text overflow。
...
| 项 | 当前实现 | 后续目标 |
|---|---|---|
| Cost adapter | `CostRouteComponent` props 来自 `CostDashboardVM`：token、cost、budget/risk/model/trend/notice counts 与 action hrefs | R4.19 继续保留 Cost regression，避免成本页变营销 dashboard |
| Replay adapter | `ReplayRouteComponent` props 来自 `renderAgentRunReplay()` 输出：run、work item、step、accepted deliverable、merge、structured audit 与 href counts | 后续 Proposal advanced split migration 不破坏 replay audit/recovery workbench |
| Restore dispatcher | `browser.ts` 识别 accepted deliverable restore href 并调用 typed `restoreAcceptedDeliverable()`；browser smoke 证明 restore POST 只发一次 | 后续把更多恢复/回滚动作接入同一 dispatcher，而不是新增 handler |
| Visual QA | 本机 Chrome 39 步 smoke 覆盖 R4.18 component marker、fallback parity、Replay 非零 deliverable action parity、R4.17 first migration regression | R4.19-pre 先证明真 React mount；R4.19 再增加 Proposal split adapter 与 advanced fallback gates |

边界：R4.18 仍不引入 React runtime dependency，不调用 `hydrateRoot()`，不迁移 Proposal advanced / Intake / Knowledge；所有 mutation 继续走 delegated browser action dispatcher。中期审查结论是 R4.16-R4.18 均只能算 React-compatible markers/props adapters，R4.19 前必须先用 R4.19-pre 证明真实 `createRoot` mount、dispatcher 共存与 SSE props update。主窗继续无 Cuu、无 secret、无 Kanban/hash/weekly 文案、无 horizontal/text overflow。
...
| 项 | 当前实现 | 后续目标 |
|---|---|---|
| True React runtime | `@workhub/web` 引入 React 18，`apps/web/src/react-route-mount.ts` 在 Home hydration root 下用 `createRoot()` 真挂 hidden probe | R4.19/R4.20 逐步把可见 readonly section 迁成真实 React component |
| Dispatcher coexistence | React probe link 冒泡进入现有 delegated dispatcher，browser smoke 看到 `action_pending` + `actionId=r4_react_mount_probe` | R4.19 Proposal split 继续单 dispatcher，不新增第二套 click handler |
| SSE props update | Home `budget.warning` SSE 走 `react-props` refresh mode，`propsUpdateCount=1`，mount count 保持 1 | R4.20 扩为 app 级 SSE 长连接与可见局部 Page VM refetch |
...
| 项 | 当前实现 | 后续目标 |
|---|---|---|
| Fixture chrome retirement | `apps/web/src/routes.ts` ready route 不再调用 `/api/pages/gold-path`；product shell 从前端真实 route registry、active Page VM metrics 和 active-only route component 组装 | R4.21 已把 runtime 抽包；R4.22 继续保持生产 route 不依赖 P0.5 fixture surface |
| App-level SSE | `apps/web/src/browser.ts` 维护 app-level EventSource map，按 URL 复用 stream；route switch 只同步 target set，`stream/me` 不随 render 重建 | R4.21 已抽成可注入 shared runtime，并给 desktop-webview 对齐 notice/payload/line-editor |
| Page VM local refetch | Home 已挂 React probe 时继续 `react-props` 更新；其他 route 的 SSE refresh 只重拉当前 route Page VM，不再重拉 gold-path chrome | R4.22 让更复杂的 Proposal editor state 进入 React controlled state |
| Cursor contract | `packages/events` 支持 SSE `id:`；API stream 读取 `Last-Event-ID` / `last_event_id` 并回显 `resume_mode`；Web 持久化最近 event id 并重连携带 cursor | 后续如需真历史 replay，要在 broker/事件存储层明确补，不在客户端假装无丢失 |
| Visual QA | R4 live route interaction smoke 42 步通过，新增 R4.20 gates：app-level runtime、route switch stable EventSource、Page VM local refetch、shell chrome no fixture、Last-Event-ID/cursor、dirty guard regression、Home React props regression | R4.21 已补 shared-runtime unit tests 与 desktop-webview regression；R4.22 继续限制 smoke sprawl |

边界：R4.20 不把 SSE payload 当真相源，不承诺 broker 历史事件 replay；它明确的是“cursor + REST reconcile”。主窗继续无 Cuu、无 secret、无 Kanban/hash/weekly 文案、无 horizontal/text overflow；双语 chrome 来自 product shell locale contract，不再来自中文 fixture 正则替换。
...
| 项 | 当前实现 | 后续目标 |
|---|---|---|
| Shared runtime package | `packages/web-runtime` 暴露 browser locale、HTML/CSS escape、structured notices、action payload parser/materializer、dirty marker、route line editor binding 与 injectable live runtime | R4.22 的 React mutation editor 继续复用同一 action/notice/dirty contract |
| Web app wiring | `apps/web/src/browser.ts` 调用 shared runtime，保留 Web route orchestration、typed client sequencing 和 R4.20 app-level SSE/Page VM reconcile 语义 | 后续迁 visible React editor 时不新增第二套 dispatcher |
| Desktop alignment | `apps/desktop-webview/src/browser.ts` 删除旧 proposal parser、merge conflict extractor、line editor payload updater 等重复实现，接 shared helpers；locale persistence 失败显示 structured notice | 后续桌面主窗 route loader 产品化时继续复用 shared runtime，而不是再复制 Web browser.ts |
| QA gates | `@workhub/web-runtime` typecheck/test、Web/desktop-webview typecheck/test、workspace typecheck/test 与 R4 42 步 browser smoke 均通过；R4.21 新增 shared-runtime parity、notice locale parity、R4.20 SSE regression、dirty guard regression、no-new-smoke-sprawl gates | 把浏览器 smoke 逐步拆成更小 CI 级 specs，contact sheet 保留为里程碑视觉门 |

边界：R4.21 是运行时共享，不是视觉改版。它不把 desktop-webview 直接升级为完整 Web Page VM route loader，也不迁 Proposal mutation editor controlled state；这些进入 R4.22 及后续收尾。
...
| 项 | 当前实现 | 后续目标 |
|---|---|---|
| Visible React editor | `apps/web/src/react-route-mount.ts` 新增 `ProposalMutationEditor`，在 Proposal advanced host 下用 React 18 `createRoot()` 挂载可见 structured field scalar editor | R4.23 继续迁 line editor 的 hunk decision/search/scope 最小切片 |
| Controlled state | React textarea 保存 custom field controlled state；SSE dirty path 触发 notice + 手动刷新，输入值不丢 | line decision、search query、scope choice 也进入 React controlled state |
| Dispatcher bridge | React accept/keep/custom actions 仍输出既有 `data-action-*` 与 `data-request-json*`，由 Web delegated dispatcher 和 `@workhub/web-runtime` materializer 处理 | 不新增 component-local fetch 或第二套 mutation handler |
| HTML fallback boundary | HTML structured field fallback 仍由 renderer 输出；React mount 后只隐藏原 fallback，并记录 `fallbackPreserved=true` / `fallbackHidden=true` | 未迁移 line/subrecord/bulk editor 继续保留 fallback marker |
| QA gates | `@workhub/web-runtime`、`@workhub/web`、`@workhub/ui`、`@workhub/desktop-webview` 单包门与 workspace typecheck/test 均通过；R4 42 步 browser smoke 新增 R4.22 五个 gates 且全部为 true | R4.23 继续限制 smoke sprawl，并把复杂 editor gates 拆成可 CI 化的小 spec |

边界：R4.22 不重写 Proposal 首屏，不迁 line editor/subrecord/bulk workbench，也不改变后端 merge/apply 语义。它证明的是一个可见 React editor 能在 R4.20/R4.21 数据流与 shared runtime 上安全存在：真实 mount、controlled state、dirty SSE guard、single dispatcher 与 fallback boundary 同时成立。主窗继续无 Cuu、无 secret、无 Kanban/hash/weekly 文案、无 horizontal/text overflow。
...
| 项 | 当前实现 | 后续目标 |
|---|---|---|
| Visible React line editor | `apps/web/src/react-route-mount.ts` 新增 `ProposalLineEditor`，在 Proposal advanced host 下用 React 18 `createRoot()` 挂载 text hunk editor | R4.24 不再扩张 editor，先做 runtime/governance 收尾 |
| Controlled state | hunk decision、search query、current file panel 由 React state 保存；SSE dirty path 后 `keep_current` 与 `scope` 不丢 | 后续如迁 subrecord/bulk，必须复用同一 dirty/fallback/dispatcher 合同 |
| Payload parity | React apply 产出完整 `text_hunk_overrides.hunks[]`，包含 hunk index、line range 与 decision；`textHunkFullCoverage=true` | 后端 merge/apply 语义保持不变，继续由 R1.41 materializer 审计 |
| Dispatcher bridge | React button/hidden payload 仍走 `data-action-*`、delegated dispatcher 与 `@workhub/web-runtime` materializer | 不新增 component-local fetch |
| HTML fallback boundary | 原 HTML line editor preserved/hidden；structured field、subrecord、bulk workbench 继续保留各自 fallback marker | R4.24 继续用 fallback regression 防止误删未迁移能力 |
