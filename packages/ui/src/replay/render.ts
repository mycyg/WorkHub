...
export function renderAgentRunReplay(
  ...
  const steps = vm.steps
    .map((step) => `<div class="wh-row"><div><strong>${escapeHtml(agentStepPhaseLabel(locale, step.phase))}</strong><p class="wh-subtle">${escapeHtml(step.output_excerpt ?? step.tool_name ?? copy(locale, "记录了一个步骤。", "Recorded one step."))}</p></div><span class="wh-pill">#${escapeHtml(String(step.step_no))}</span></div>`)
    .join("");
  ...
  const main = `<section class="wh-replay-main">
    ...
    <div class="wh-grid">
      ${run.status ? `<article class="wh-card" data-replay-run-status="${escapeHtml(run.status)}"><strong>${escapeHtml(copy(locale, "状态", "Status"))}</strong><p class="wh-subtle">${escapeHtml(agentRunStatusLabel(locale, run.status))}</p></article>` : ""}
