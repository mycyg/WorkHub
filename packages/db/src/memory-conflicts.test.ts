...
test("memory conflict repository lists only open user/workspace cards with an honest cap", async () => {
...
test("memory conflict repository resolves only the actor's open workspace card", async () => {
...
test("memory conflict repository deduplicates one open card per user key", async () => {
  const { db, queries } = createQueryRecorder([
    [row()],
  ...
  assert.ok(queryReferences((secondInsert?.onConflict as { target?: unknown })?.target, memoryConflicts.workspaceId));
  assert.ok(queryReferences((secondInsert?.onConflict as { target?: unknown })?.target, memoryConflicts.userId));
  assert.ok(queryReferences((secondInsert?.onConflict as { target?: unknown })?.target, memoryConflicts.category));
  assert.ok(queryReferences((secondInsert?.onConflict as { target?: unknown })?.target, memoryConflicts.key));
  assert.ok(
