...
- 📐 **规格树索引**:[`docs/workhub/`](docs/workhub/README.md) —— 116 篇(架构 / AI 引擎 / 协作 / 业务模块 / 客户端 / 路线图 / 成本治理 / 视觉 QA)
...
- 生产沙箱与 Agent 执行后续要求 Linux；桌面端已补 Windows 物理托盘、Linux GNOME StatusNotifier/AppIndicator 与 macOS menu bar 中英 smoke；Web R4.23 已补 Proposal line editor text hunk 第二段真实可见 React mutation island，R4.22 structured field scalar editor、R4.21 shared runtime、R4.20 app-level SSE、Page VM local refetch、Last-Event-ID/cursor、Proposal dirty guard regression 与 `/api/pages/gold-path` fixture chrome 退役均保持通过；下一步推进 R4.24 Web runtime finalization。数据库验收优先本地构建/本地 PG+Redis 复跑。
