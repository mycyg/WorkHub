...
| Cuu 桌宠 | 独立 pet window、黑猫/白猫 Live2D registry、偏好二选项、QA 合同、概念图已同步真实模型帧、pass-through 源码恢复门、P1.6 behavior manifest 源码合同、P1.7 业务录屏入口与黑猫 approval smoke、R3.10 真实 launcher/en-US capture、R3.12 真实 zh-CN/en-US run-stream completion capture、R3.13.1 真实 zh-CN/en-US run-failure terminal capture | 401/403、offline/network capture、刷新恢复、settings matrix、长期性能、授权或原创替换 |
...
| Cuu 是会动的小猫桌宠 | 黑/白 Live2D registry 已落，概念图已同步真实模型帧，P1.6 behavior manifest 源码合同已落，P1.7 可录真实业务场景且已有黑猫 approval smoke，R3.12 已有真实 Tauri run-stream completion 中英双语证据，R3.13.1 已有真实 Tauri run-failure 中英双语证据且 trace card 文本/预算区无超框 | 401/403、offline/network、刷新恢复、长期性能 |
...
| Cuu motion | P1.10 以前的黑/白 contact sheet、GIF/MP4、diff report只作为回归证据；R3.10 launcher/en-US、R3.12 zh-CN/en-US run-stream completion 与 R3.13.1 zh-CN/en-US run-failure terminal 是当前业务功能证据；R3.13.1 还要求最终帧无文本/预算超框；不新增外观矩阵 |
...
| R3 | Cuu 自然语言 / option-first 出站 Agent 入口，不新增外观；R3.12 已补真实 Tauri run-stream completion，中英双语证据；R3.13.1 已补真实 Tauri run-failure terminal，中英双语证据；下一步 401/403、offline/network 与刷新恢复 |
