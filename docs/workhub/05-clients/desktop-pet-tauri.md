...
> **当前截图 / 动作审计**：见 [`current-state-visual-audit-and-construction-plan-2026-06-07.md`](./current-state-visual-audit-and-construction-plan-2026-06-07.md)。2026-06-07 已对 Web、desktop webview、browser pet preview 和真实 Tauri `Cuu` 窗口做截图，并对 `Cuu` 顶层窗口做 32 帧 motion capture。首轮发现 card mode 事件卡裁切；第一轮 card layout 又暴露 Cuu 只露耳朵 / 局部的失败样例；随后又发现静态 fallback 呼吸/缩放不能算鲜活动作。同日已补 bridge placement 校验、compact fallback、full-body HiDPI 站位、离线人话卡、dev server sprite asset 路径修复、运行态禁用静态 fallback 和多帧 motion QA，最终事件卡窗口可扩到 `394 x 568`，Cuu 完整身体可见，body-only 第一屏有真实摇尾动作。2026-06-08 已把默认视觉切到 Bongo-style 低恐怖谷 renderer，完成 P1b 动作增强、真实 Tauri `PrintWindow` 抓取、P1c first-painted 首帧门禁、BONGO-REF 默认模型包门禁、BONGO-P2a-a model pack registry / loader、BONGO-P2a-b Settings 模型包 UI、P1e-b 输入手感真实录屏、P1e-c 连续看鼠标 / hover 避让真实录屏、P1e-d-a pointer smoothing / drag grip 真实录屏、P1d-b-a hide-on-hover 软隐藏 / 恢复真实录屏、P1d-c 窗口设置矩阵真实截图和 P1e-d-b 60s idle jitter / flicker 真实录屏：wave/search/sync/revise/celebrate 可辨，GIF/MP4 已产出，最新 frame 000 已是 body-only Cuu 全身可见，hover/tap/drag 场景无离线卡片污染且窗口可拖动；`look-avoidance` 场景能录到 left/right cursor-near、hover_top_right、tap、drag/release 7 个事件，`drag-smoothing` 场景能录到 left/right/left cursor-near、hover、tap、两段 drag move 和 release，`hide-on-hover` 场景能录到 hover 后软隐藏、cursor leave 后恢复、再次 hover 后再软隐藏，`settings` 矩阵能录到 default、scale-75、scale-150、opacity-60、pass-through、hide-on-hover 和组合设置，`/settings` 中英截图能看到 Bongo 当前默认与 Live2D 实验锁定，`idle-long-run` 场景 31 帧约 60 秒无低可见帧、无窗口漂移，`cuu-bongo-p1` 是当前唯一 `approved_default` 与可选模型包，默认 DOM 暴露 `data-cuu-model-pack-selection-reason="registry_default"`。Live2D 144 层 PSD v1 继续作为并行实验线，只有 Cubism 录屏和 `CuuModelPackManifest` 默认门都通过后才允许替换 Bongo 默认。
> **Web rail 生成图资产**：2026-06-08 追加 `cuu-polished-idle-v1`，用 GPT Image 生成绿幕源图并抠成透明 PNG，优先用于 Gold Path / desktop 主窗右侧 Cuu rail；`bongo_cuu` CSS 几何小猫保留为加载失败 fallback。该切片只修正主窗占位视觉粗糙问题，不改变独立 `pet` window 的长期 Live2D / Bongo motion 验收门。
...
| `apps/desktop-webview/src/cuu-preferences.ts` | Cuu preference panel | 右上角轻入口，面板默认隐藏；本地存储 `attention_mode` / `sound_mode` / `reduced_motion` / `queue_limit` / `pet_scale_percent` / `pet_opacity_percent` / `pet_pass_through` / `pet_hide_on_hover` / `pet_model_pack_id`，把点击偏好写回 `CuuController`，并允许 Rust QA 初始化脚本注入窗口设置偏好；模型包列表来自 `describeCuuModelPackChoices()`，Live2D 实验包 disabled |
...
| `packages/ui/src/gold-path/render.ts` | Gold Path Settings page / Cuu rail | 新增 `/settings` 页面；Cuu 外观区展示 `cuu-bongo-p1` 当前默认、`cuu-live2d-cubism-v2` 实验锁定、窗口手感与语言偏好说明；Gold Path 右侧 Cuu rail 优先加载 `./assets/cuu/cuu-polished-idle-v1-alpha.png` 生成图资产，CSS 小猫仅作 fallback；中英截图和 Web rail 生成图截图已入审计资产 |
| `apps/desktop-webview/src/pet-surface.ts` | Cuu pet webview surface | `pet` 窗口入口只渲染 Cuu atlas / Bongo 本体和一张轻气泡，不加载 Gold Path 主壳；card mode 只有在 Rust placement 确认后渲染 full card，未确认或失败时走 compact fallback；P1.2 已把 `CuuCard.kind` / `priority` / `sections` / `progress` / `evidence_refs` / `input` 渲染进轻卡，操作区前置，打回理由用固定按钮；P1.2b 已把选中的 clarification option 通过 `selected_option_ids` 提交给 session API；P1d-a 已输出 `data-pet-scale-percent`、`data-pet-opacity-percent`、`data-pet-pass-through` 和缩放后的窗口尺寸 CSS 变量；P1d-b-a 已输出 `data-pet-hide-on-hover`、`data-pet-hover-hidden`、`data-pet-hover-hide-mode` 和 `--wh-pet-hide-*`，hover 时做可恢复 soft dodge；P1e-c 已输出 `data-pet-cursor-near` / `data-pet-hovered` / `data-pet-dragging` / `data-pet-look-x/y` / `data-pet-hover-avoidance`，并用 `--wh-pet-look-*` 与 `--wh-pet-avoid-*` 变量驱动 Bongo pose；P1e-d-a 已输出 `data-pet-pointer-smoothing-alpha`，并在 dragging 时保持 `drag_hold` |
| `apps/desktop-webview/src/pet-surface-qa.ts` | Cuu pet visual QA contract | 静态检查透明 root、CSS 变量驱动的右下角独立 surface、点击/拖拽热区、非主壳、Bongo 默认、pointer-reactive pose、hover soft-hide 属性、真实多帧 atlas fallback、轻气泡、选项优先、heavy card context 和默认窗口手感属性；新增 `bongo_hover_hide_handfeel` 门禁 |
...
| `packages/cuu/src/model-pack.ts` | Cuu model pack default gate + registry / loader | 对齐 BongoCat 的可替换模型思想，当前 `cuu-bongo-p1` 为唯一 `approved_default`；`listCuuModelPacks()` / `describeCuuModelPackChoices()` / `resolveCuuVisibleModelPack()` 已落，Live2D 候选会以 `experimental_locked` 展示但回退 Bongo；PSD draft 资产即使可渲染，也不能作为默认候选；P1d-a/P1d-b-a 后 scale / opacity / pass-through / hide-on-hover 已标为 supported；`plannedCuuLive2DCubismModelPack` 只作为未来 Cubism 路线占位，不能默认 |
...
| 独立桌宠窗 `pet` | 已有 `ShellWindowPlan` + Tauri window config + `show/hide/toggle` control plan；生产 Tauri 由 Rust setup 动态创建 `create:false` 的 `pet` window 并注入 pet surface flag，浏览器调试保留 `/pet` / `?surface=pet` / `#surface=pet` / `pet.html`；`pet-surface-qa.ts` 已守住透明 root、右下角、非主壳、Bongo 默认、多帧 atlas fallback、选项优先和 heavy card context；`pet_window.rs` 已固定 body-only/card 几何、右下角定位、展开锚点、鼠标接近、拖拽和 settings plan；`main.rs` 已把启动期 body-only 预定位、first-painted 后 show、pet show/hide/toggle、mode resize/position/show、drag、save-position、cursor sample、scale/opacity/pass-through/hide-on-hover settings 执行到真实 Tauri window / AppHandle API，并处理 HiDPI 坐标换算与运行期 topmost；托盘菜单可 toggle Cuu；2026-06-07 Windows debug smoke 已确认 `Cuu` 窗口 visible + topmost + 在右下角显示 Cuu 和气泡，且主窗隐藏后仍可见；card mode motion capture 已确认事件卡出现后窗口可扩到 `394 x 568`，最终 HiDPI 抓帧中 Cuu 完整身体可见、轻卡右侧有留白；2026-06-08 Bongo P1b/P1c/P1d-b-a/P1d-c 真实 Tauri capture 已产出，最新 frame 000 起 body-only 全身可见，hide-on-hover 场景可软隐藏、离开恢复、再次隐藏，settings matrix 覆盖 7 个窗口设置组合；同日 BONGO-P2a-b 已补 `/settings` 中英截图和 Cuu 偏好模型包选择，P1.2 browser CDP 抓帧确认审批轻卡能同时显示 Cuu 全身、按钮、变更摘要和风险摘要；`skipTaskbar` 仍在 WorkHub plan | transparent / decorations false / always-on-top / skip taskbar；后续补 P1.2 真实 Tauri card fixture capture、Live2D Cubism runtime、多屏恢复实测、安装包 smoke、系统通知点击、full hide/pass-through 安全恢复和跨平台透明 capture；atlas/Hatch 仅 fallback |
...
| Cuu 偏好 / shell 设置 | desktop webview 内已有偏好面板和本地存储；P1d-a 新增尺寸 75/100/125/150、透明度 60/80/100、点击穿透；P1d-b-a 新增 `pet_hide_on_hover` 软隐藏开关；P1d-c 已用真实 Tauri 设置矩阵确认这些偏好能进入 pet window；BONGO-P2a-b 新增 `pet_model_pack_id` 归一化、Settings 页面和 Cuu 偏好气泡模型包 UI；`pet-window-bridge.ts` 会把主窗偏好同步到独立 pet window，Rust 侧已有 `pet-window-state.json` 位置落盘、`workhub-shell-config.json` 读取入口和 `set_pet_window_settings` | 继续接设备注册、安全 vault、托盘显隐、多屏恢复实测、系统通知偏好和 full hide/pass-through 安全恢复；Live2D 仍需 model pack gate 通过后才能开放 |
...
- **当前窗口设置矩阵**：P1d-c 已用 `scripts/qa/cuu-tauri-settings-capture.ps1` 在真实 Tauri `Cuu` 顶层窗口里验证 default、scale-75、scale-150、opacity-60、pass-through、hide-on-hover、combo-125-80-pass-hide。`settings-capture-report.json` 中 `passed=true`，所有 case 首帧非空、Cuu 全身可见，不再出现只露耳朵或缩放后裁切。它证明窗口能力合同已接到真实 pet window；BONGO-P2a-b 已补主窗 Settings 页面里的模型包 UI，负责解释“当前默认 Bongo / Live2D 实验锁定”。
...
- **当前活体行为**：`packages/cuu/src/idle-scheduler.ts` 已提供基础 scheduler，`pet-surface.ts` 会在没有 active card 时 tick 并更新 `data-cuu-idle-action`，并按该 action 选择真实 atlas clip；P1e-a 后 `cursor_near` 首次进入附近区域会立即触发 `look_at_mouse`；P1e-c 后 surface 同时输出 `data-pet-cursor-near` / `data-pet-hovered` / `data-pet-dragging` / `data-pet-look-x/y` / `data-pet-hover-avoidance`，并用 CSS 变量驱动头、眼、面部凝视与 hover 轻避让；P1e-d-a 后 surface 输出 `data-pet-pointer-smoothing-alpha="0.58"`，Rust sample 会低通平滑，dragging 时保持 `drag_hold`；P1d-b-a 后 surface 输出 `data-pet-hide-on-hover` / `data-pet-hover-hidden` 并在 hover 时做可恢复 soft dodge。`pet-surface-qa.ts` 已把透明 root、右下角独立 surface、点击/拖拽热区、非主壳、pointer-reactive pose、smoothing alpha、hide-on-hover attrs、真实多帧 atlas、轻气泡、选项优先卡片和窗口手感属性做成静态门禁；`pet-window-bridge.ts` 已把 pointer hover / drag / release 与 Rust cursor sample 喂给 scheduler，并会调用 Tauri window `startDragging` 或 Rust command fallback；`pet_commands.rs` 已固定 `set_pet_window_mode`、`set_pet_window_settings`、`start_pet_window_drag`、`save_pet_window_position`、`sample_pet_cursor_near`，其中 cursor sample 会返回 `look_x_percent/look_y_percent`；`main.rs` 已注册这些 command，并把启动期 body-only 预定位、first-painted 后 show、mode resize/position/show、settings scale/opacity/pass-through/hide-on-hover、drag、save-position、cursor sampling 执行到真实 Tauri window / AppHandle API，拖拽位置保存到 Tauri Config `pet-window-state.json`，启动时会 clamp 回当前 work area；所有 monitor work area、window outer position 和 cursor position 会从 physical px 转成 logical px，防止 HiDPI 离屏；运行期会显式设置 `set_always_on_top(true)`，保证 Cuu 不被主窗盖住。当前已完成 Windows debug `PrintWindow` 像素 smoke、`input-handfeel`、`look-avoidance`、`drag-smoothing`、`hide-on-hover`、`settings` 矩阵和 `idle-long-run` 真实录屏，会在隐藏主窗后校验 `Cuu` 顶层窗口 visible/topmost/bottom-right、可见像素、hover/tap/drag/left-right look、输入平滑事件、拖拽坐标、软隐藏恢复、scale/opacity/pass-through/hide-on-hover 组合、60 秒低可见帧与窗口漂移；仍缺安装包 smoke、跨平台透明 capture、多屏恢复实测、长驻 CPU/GPU 采样和 full hide/pass-through 安全策略。
...
- **当前偏好面板**：`apps/desktop-webview/src/cuu-preferences.ts` 已提供右上角轻入口，面板默认隐藏；展开后可设置提醒模式（正常/安静/勿扰）、声音（开启/静音）、减少动效、队列上限、桌宠尺寸、透明度、点击穿透、悬停避让和模型包。模型包选项来自 `describeCuuModelPackChoices()`：Bongo 可保存，Live2D 显示实验锁定且不可点。偏好写入 localStorage 并同步到 `CuuController`，主窗会通过 `pet-window-bridge.ts` 立即调用 `set_pet_window_settings` 更新独立 pet window。
...
- **视觉资产（新增硬约束）**：P1 不再接受恐怖谷 PSD、抽象图标或 inline 静态 fallback 作为完成标准；当前默认使用 `bongo_cuu` 低恐怖谷 renderer，atlas / Hatch 作为 fallback，PSD / Live2D 作为实验线。Bongo P1b 已补挥手、检索、同步、打回、抱文件、庆祝和拖拽动作，并已通过 browser CDP 与真实 Tauri `PrintWindow` 多帧抓取；P1c 已补 first-painted 首帧门禁，frame 000 不再空白；BONGO-REF 已把 `CuuModelPackManifest` 接成默认门禁，`cuu-bongo-p1` 通过，PSD draft 默认候选失败；BONGO-P2a-a 已把 registry / loader 接到 renderer，Live2D 实验包和未知 PSD 请求都会回退 Bongo；BONGO-P2a-b 已把模型包选择接进 Settings 和 Cuu 偏好气泡；P1d-a 已补窗口尺寸、透明度和点击穿透契约；P1d-b-a 已补 hide-on-hover 软隐藏 / 恢复真实录屏；P1e-a/P1e-b/P1e-c/P1e-d-a/P1e-d-b 已补 cursor-near 输入响应、hover/tap/drag 真实 Tauri 输入录屏、连续看鼠标、hover 避让、pointer smoothing、drag grip 和 60s idle jitter / flicker 长驻 QA。真实运行态必须显示可辨认的非缩放动作；静态 fallback 只能用于诊断或非运行兜底，不能作为 motion QA 通过依据。生产前继续做动作幅度二轮、full hide/pass-through 安全恢复、anchor 微调、WebP/PNG 压缩、alpha 边缘 QA、长时间 idle 性能检查。
- **两态**：
...
- **主窗占位视觉**：Web / desktop 主窗右侧栏已用 `cuu-polished-idle-v1` 生成图透明 PNG 替换早期 CSS 几何小猫；这只是主窗 rail 视觉升级，独立桌宠窗口仍必须通过多帧 motion capture 才算鲜活感通过。
...
- Cuu 模型包（WorkHub 已落）：`/settings` 与 Cuu 偏好气泡都读取 `describeCuuModelPackChoices()`；`cuu-bongo-p1` 是当前默认且可保存，`cuu-live2d-cubism-v2` 是 `experimental_locked` 且按钮 disabled。保存只写 `normalizeCuuSelectableModelPackId()` 允许的 `pet_model_pack_id`，避免 PSD / Live2D 实验线被误设为默认。
...
- P1 默认必须能不依赖专业绑定工具运行，因此当前采用 `bongo_cuu` DOM/CSS 低恐怖谷 renderer；详见 [`cuu-bongo-style-runtime-plan.md`](./cuu-bongo-style-runtime-plan.md)。P1b 已通过 browser 状态墙和真实 Tauri contact sheet 验证动作可读性；P1c 已通过 first-painted 首帧门禁，frame 000 不再是 blank；BONGO-REF / BONGO-P2a-a 已把参考 BongoCat 的模型可替换思想收敛成 `CuuModelPackManifest` + registry / loader，默认包为 `cuu-bongo-p1`，实验 Live2D 请求会以 `experimental_locked` 回退 Bongo。BONGO-P2a-c 另把主窗右侧 rail 的粗糙 CSS 小猫替换为 GPT Image 生成、绿幕抠图后的透明 PNG，但该 bitmap 不替代独立 pet window motion gate。
