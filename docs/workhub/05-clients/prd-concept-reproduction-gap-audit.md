...
- Cuu 已有卡片、motion hint、sprite manifest、controller MVP、18 clip 绿幕/alpha motion pack、基础 idle scheduler、Rust injected pet surface、浏览器调试 surface、pet surface 静态视觉 QA、pet window 几何合同、command scaffold、webview pointer/drag bridge、Windows debug `PrintWindow` 像素 smoke、内联静态兜底图、Bongo-style 默认 renderer、Bongo P1b 动作增强、Bongo P1c first-painted 首帧门禁、BONGO-REF model pack 默认门禁、BONGO-P1d-a 窗口手感契约、BONGO-P1d-b-a hide-on-hover 软隐藏 / 恢复真实录屏、BONGO-P1e-b/P1e-c/P1e-d-a 真实输入录屏、真实 Tauri Bongo GIF/MP4、Live2D 分层拆件概念图、正面基准稿、PSD 生产板、`contract_only` 图层 manifest、144 层 PSD draft 和 `psd_draft_probe`；`CuuMotionHint.sprite_state` 业务状态与 idle / interaction 微动作已 full coverage，但真实形态仍缺 Bongo 动作二轮幅度、窗口设置真实截图、full hide/pass-through 安全恢复、60s jitter QA、model pack loader、精修 Live2D 分层 PSD / Cubism rig / runtime、anchor / 压缩 / 性能 / 多屏 QA。
...
| 缺口 | 为什么重要 | 目标落点 |
|---|---|---|
| 真实小猫动画资产 | 18 clip motion pack 已落，业务状态与 idle / interaction micro action 均 full coverage；Bongo-style 默认 renderer、Bongo P1b 动作增强、P1c first-painted 首帧门禁、BONGO-REF model pack 默认门禁、BONGO-P1d-a 窗口手感契约、BONGO-P1d-b-a hide-on-hover soft dodge、BONGO-P1e-b/P1e-c/P1e-d-a 输入手感真实录屏和真实 Tauri GIF/MP4 已落；Live2D 分层概念图、正面基准稿、PSD 生产板、`contract_only` manifest 和 `psd_draft_probe` 已落；要复现概念图仍需 Bongo 动作二轮幅度、窗口设置真实截图、full hide/pass-through 安全恢复、60s jitter QA、model pack loader、正式 PSD、Cubism rig、anchor 微调、透明边缘 QA、WebP/PNG 压缩、帧率与长驻性能验证 | `packages/cuu/src/model-pack.ts`、`apps/desktop-webview/src/cuu-bongo-runtime.ts`、`apps/desktop-webview/src/assets/cuu/*`、`apps/desktop-webview/src/assets/cuu/live2d/*` 或未来 `client-tauri/web-src/src/assets/cuu/*` |
...
| 拖拽 / 收起 / 静音 / 勿扰 | 静音 / 勿扰 / 减少动效 / 队列上限 / 尺寸 / 透明度 / 点击穿透 / 悬停避让已有 desktop webview 面板；拖拽 bridge、Rust drag plan、真实位置记忆、pass-through 命令、hide-on-hover soft dodge、pointer smoothing 和 drag grip 持续姿态已落；收起细节、full hide/pass-through 安全恢复、60s jitter QA 与真实 Tauri Settings 承接仍待补 | Rust window state + TS preference |
...
2. **Bongo P1d-b / P1e-d 动作与窗口体验**：P1d-a 已补缩放、透明度和点击穿透契约，P1d-b-a 已补 hide-on-hover 软隐藏 / 恢复真实录屏，P1e-a/P1e-b/P1e-c/P1e-d-a 已补 cursor-near、pointer DOM attrs、hover/tap/drag、连续看鼠标、hover 避让、pointer smoothing 和 drag grip 真实录屏；下一步补真实 Tauri 设置截图、多屏恢复、显示/隐藏快捷入口、60s jitter QA、full hide/pass-through 安全恢复，并加大抱文件 / 审批敲桌 / 完成庆祝的动作幅度。
