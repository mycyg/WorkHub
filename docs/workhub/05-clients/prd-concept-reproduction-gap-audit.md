...
| WEB-P3 | `/proposals/:id` | 多类型交付物变更说明、预览、结构化字段落点、标量字段高级编辑、子记录逐项 diff/editor、折叠多冲突批量检查；后续补真实 route 富 preview、长列表截图门和批量执行/审计产品化 |
| WEB-P4 | `/knowledge` / Cuu search | Web 做兜底，Cuu 气泡做默认入口 |
| WEB-P5 | `/dashboard/cost` | P-COST Usage / BudgetNotice / CostSummaryVM 接真实数据 |
| WEB-P6 | `/agent-runs/:id/replay` | 真实 trace、accepted deliverables、merge timeline、字段写回审计与 eval/replay 对齐；后续补 React route 富 viewer |
