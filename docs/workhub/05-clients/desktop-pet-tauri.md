...
> **当前截图 / 动作审计**：见 [`current-state-visual-audit-and-construction-plan-2026-06-07.md`](./current-state-visual-audit-and-construction-plan-2026-06-07.md)。2026-06-07 已对 Web、desktop webview、browser pet preview 和真实 Tauri `Cuu` 窗口做截图，并对 `Cuu` 顶层窗口做 32 帧 motion capture。首轮发现 card mode 事件卡裁切；第一轮 card layout 又暴露 Cuu 只露耳朵 / 局部的失败样例；随后又发现静态 fallback 呼吸/缩放不能算鲜活动作。同日已补 bridge placement 校验、compact fallback、full-body HiDPI 站位、离线人话卡、dev server sprite asset 路径修复、运行态禁用静态 fallback 和多帧 motion QA，最终事件卡窗口可扩到 `394 x 568`，Cuu 完整身体可见，body-only 第一屏有真实摇尾动作。但 8 层裁片 prototype 仍不算鲜活感通过；当前 P1 主线转为 Live2D 144 层 PSD v1 清理、Cubism 绑定、多屏/跨平台透明 capture 与长期运行 QA。
