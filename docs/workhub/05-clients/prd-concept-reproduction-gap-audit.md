...
- `client-tauri/src-tauri` 当前已有 `tauri` / `tauri-build` 依赖、`tauri.conf.json` / capability scaffold、`build.rs`、`main.rs` pet command handler，并已把 mode resize/position/show、drag、save-position、cursor sampling 执行到真实 window / AppHandle API，位置保存到 Tauri Config `pet-window-state.json`；但还没有多屏恢复实测、托盘、通知、deep-link、updater。
...
| Rust shell | config/http/sse/event/window planning/control planning crate + Tauri config/capability scaffold + 最小 Tauri `main.rs` command handler，pet mode/drag/save-position/cursor-sample 已执行到窗口 / AppHandle API，位置已落盘 | 缺多屏恢复实测、托盘、通知、deep-link、设备令牌 vault、本地 sync/delivery/updater |
