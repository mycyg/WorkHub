...
- Cuu 已有卡片、motion hint、sprite manifest、controller MVP、18 clip 绿幕/alpha motion pack、基础 idle scheduler、Rust injected pet surface、浏览器调试 surface、pet surface 静态视觉 QA、pet window 几何合同、command scaffold、webview pointer/drag bridge、Windows debug `PrintWindow` 像素 smoke、内联静态兜底图、Bongo-style 默认 renderer、Bongo P1b 动作增强、Bongo P1c first-painted 首帧门禁、BONGO-REF model pack 默认门禁、BONGO-P1d-a 窗口手感契约、BONGO-P1e-b/P1e-c/P1e-d-a 真实输入录屏、真实 Tauri Bongo GIF/MP4、Live2D 分层拆件概念图、正面基准稿、PSD 生产板、`contract_only` 图层 manifest、144 层 PSD draft 和 `psd_draft_probe`；`CuuMotionHint.sprite_state` 业务状态与 idle / interaction 微动作已 full coverage，但真实形态仍缺 Bongo 动作二轮幅度、窗口设置真实截图/hide-on-hover、60s jitter QA、model pack loader、精修 Live2D 分层 PSD / Cubism rig / runtime、anchor / 压缩 / 性能 / 多屏 QA。
...
| 拖拽 / 收起 / 静音 / 勿扰 | 静音 / 勿扰 / 减少动效 / 队列上限 / 尺寸 / 透明度 / 点击穿透已有 desktop webview 面板；拖拽 bridge、Rust drag plan、真实位置记忆、pass-through 命令、pointer smoothing 和 drag grip 持续姿态已落；收起细节、hide-on-hover、60s jitter QA 与真实 Tauri Settings 承接仍待补 | Rust window state + TS preference |
| 活体 idle scheduler | 基础 scheduler、pointer hover/drag/release bridge、Rust cursor sample、pointer smoothing 和 dragging 时 `drag_hold` 保持已落，且 idle / interaction 动作已可映射到真实 atlas clip；磁盘位置恢复已落；仍缺系统 idle 采样、多屏实测、60s jitter 和长驻 QA | `apps/desktop-webview/src/pet-surface.ts`、`apps/desktop-webview/src/pet-window-bridge.ts`、`packages/cuu/src/idle-scheduler.ts` |
...
| GAP-CUU-02B | Controller visual completion | `packages/cuu/src/controller.ts`、`apps/desktop-webview/src/cuu-preferences.ts`、`apps/desktop-webview/src/browser.ts` | GAP-CUU-02 | **MVP 已落**：show / replace / queue / badge / drop 可测，desktop badge、超时推进、偏好面板和窗口手感字段已接；待真实 Tauri Settings 页面、通知点击/偏好和真实窗口视觉 QA |
...
2. **Bongo P1d-b / P1e-d 动作与窗口体验**：P1d-a 已补缩放、透明度和点击穿透契约，P1e-a/P1e-b/P1e-c/P1e-d-a 已补 cursor-near、pointer DOM attrs、hover/tap/drag、连续看鼠标、hover 避让、pointer smoothing 和 drag grip 真实录屏；下一步补真实 Tauri 设置截图、hide-on-hover、多屏恢复、显示/隐藏快捷入口、60s jitter QA，并加大抱文件 / 审批敲桌 / 完成庆祝的动作幅度。
