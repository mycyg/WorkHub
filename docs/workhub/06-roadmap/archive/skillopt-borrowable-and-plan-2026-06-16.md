...
- **R8.1 闭合最便宜的复利环（S 批）**：K1 读回弃用日志 + K3 递减 edit-budget + K5 curation 成本分账。产出：curation 不再重复白烧、每夜改动有上限、AI 战绩显「自进化花费」。
  ...
  - ✅ **K5 已落**（生产 vs 自我改进分账）：`@workhub/cost UsageSource` 加 `"curation"` + `isSelfImprovementSource()`；蒸馏调用打 `source:"curation"`；成本面板 `CostDashboardVM.labor_split`（`buildLaborSplit` 把账目拆「干活/自进化」+ ratio）；前端成本路由渲染「干活 vs 自进化」卡（i18n 双语）。真 key+PG e2e：蒸馏→usage_record source=curation→cost ledger→`buildCostDashboardPage` labor_split 自进化花费可见全链通。**R8.1 三件套（K1+K3+K5）收齐。**
- **R8.2 精修而非churn（M）**：K2 受限编辑补丁 + 打字化段落 schema（+ 顺带 #20 token 大小带守卫，技能注入每个未来 prompt，得管体积）。产出：技能演进有可审 diff、保留 provenance。
  - ✅ **K2 后端机制已落**（受限编辑补丁 = 「梯度」）：`contracts` 加 `skillEditPatch{,Response}Schema` + `applySkillEditPatch`（纯函数段落级 applier：`## 标题` 块为单位 Add/Modify/Remove，frontmatter/preamble 永不被触碰，逐 op 尽力而为，拒绝偷塞 `## ` 的正文）+ `TEAM_SKILL_MAX_EDIT_OPS=3`/`MAX_REFINES_PER_CURATION=2`/规范段落词汇。`skill-curation.ts` 加 `validateSkillEditPatch`（confidence+乐观并发 base_version+应用后再验 frontmatter/markers）+ `buildSkillRefinementPrompt`/`parseSkillEditPatchResponse`。worker 新增「精修步」：给激活技能正文→要补丁→应用→`promote` 出新版本（samplesJson 存 `refined_from_version`+ops+rationale 作 provenance），审计 `team_skill.refined_via_patch`/`refine_discarded`，记 `source:"curation"`（K5 分账）。真 key 验证：激活 v2 缺「边界情况」段 + 数据缺口升级信号 → curator 打出**单 op `add_section:边界情况`**，base_version 对、frontmatter 与旧「套路」段保留、非整篇重写。测试 contracts 7/7、curation 14/14。
  - ✅ **前端自进化可见（战绩面）**：`AiWorklogVM` 加 `skills_promoted_today`/`skills_refined_today`（从审计 `distilled_and_promoted`+`refined_via_patch` 今日聚合，`pilot-metrics.readAiWorklogRows` 查、`buildAiWorklog` 计数）；首页战绩横幅在有自进化时追加「还顺手自我精进：技能 +X · 精修 Y」（`data-r4-home-self-evolve`，零活动不显）。真 PG e2e：写 1 新增+2 精修审计 → worklog 服务计数 +1/+2 全链通。测试 worklog 4/4、ui route-components 32/32、web routes 24/24。（成本侧自进化花费见 K5 labor_split；这里是「干了多少自进化」的活动计数。）
  - ✅ **K2 对抗式加固**（R8.2 含 #20 体积守卫）：`TEAM_SKILL_MAX_CONTENT_CHARS=8000` 体积守卫（技能注入每个 worker prompt，新增+精修后超限即拒 `exceeds_size_budget`）；精修防 churn —— 应用后逐字等于现役 → 拒 `no_effective_change`（不空转 bump 版本）；worker 一技能一夜只精修一次（同 key 第二补丁会对旧底稿双重 churn，跳过）。测试 curation 16/16。
- **R8.3 从失败学习（M）→ 已由 K2 实质覆盖**：K4「append-only 升级 appendix」的核心价值（升级信号 → 技能里加去重「踩坑/边界情况」段）**已被 K2 实现**——K2 真 key 验证里 curator 正是从数据缺口升级信号蒸出 `add_section:边界情况`；且 applier 的 `section_exists` 天然防重复追加、`no_effective_change` 防空转。**单独再造一个 K4 机制属冗余胶水，不做**（避免「凑数」）。若未来要纯确定性零-LLM 的升级追加，再单立。
- **R8.4 吵信号下的 gradient（M）→ 暂缓（最低价值）**：K6 结果分桶 minibatch 反思。当前 curation 已用「3 档粗桶」心智（accept/escalation 硬信号），K6 的多批分桶在 pilot 真实流量起来前价值有限，留作 future。
