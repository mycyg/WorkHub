...
> 状态(2026-06-11):**110 篇文档已落盘**。最新权威施工顺序见 [`06-roadmap/recovery-r0-r4-roadmap-2026-06-08.md`](./06-roadmap/recovery-r0-r4-roadmap-2026-06-08.md),详细领取计划见 [`06-roadmap/review-driven-r0-r4-detailed-construction-plan-2026-06-08.md`](./06-roadmap/review-driven-r0-r4-detailed-construction-plan-2026-06-08.md):R0 止血对账 -> R1 真实纵切 -> R2 多 worker -> R3 Cuu Agent 入口 -> R4 Web 产品化。Cuu 当前只保留独立桌宠窗口中的黑猫/白猫 Live2D 二选项；R3.23 已补真实 Linux GNOME StatusNotifier/AppIndicator DBusMenu action 与 macOS menu bar status item 中英真机 smoke，用户截图暴露的 failed run card 文本越框已补 Chrome 直渲染回归门；R4.1 已补 Web 高频页四态/双语/无溢出 route-state matrix foundation，R4.2 已补真实 URL route registry、loader 状态机与 typed Page VM endpoint proof，R4.3 已补多记录 Page VM ready/detail route 视觉 QA，R4.4 已补 Web product shell baseline、导航内部溢出与文本盒溢出门禁，R4.5 已补 Vite live browser route interaction smoke、重复 listener guard 与移动端滚动遮挡门，R4.6 已补 Rust system-string i18n、tray/notification/deep-link diagnostics 双语系统串，R4.7-R4.9 已通过真实 PG、Redis/SSE 与 locale metrics 浏览器 smoke，R4.10 已完成 Home/Approvals/Replay Web route componentization first slice，R4.11 已完成 WorkItem/Proposal/Cost/Settings route componentization second slice 与 Settings typed Page VM，R4.12 已完成 Web action/notice locale route UX，R4.13 已完成 Proposal advanced route UX convergence，R4.14 已完成 Option Intake / Knowledge route componentization，R4.15 已完成 Settings / locale / device boundary hardening，R4.16 已完成 React route tree / hydration boundary，R4.17 已完成 Home / Settings React-compatible first migration，R4.18 已完成 Cost / Replay React-compatible expansion 与 Replay restore single-dispatcher proof；2026-06-11 已落 R4 中期审查 [`06-roadmap/r4-mid-review-upgrade-audit-2026-06-11.md`](./06-roadmap/r4-mid-review-upgrade-audit-2026-06-11.md)，R4.19-pre 已完成真 React mount spike，证明 Home `createRoot` mount、delegated dispatcher 共存与 Home SSE `react-props` update；下一步 R4.19 增加 Proposal split migration、编辑态 dirty guard 与 fixture-chrome 冻结 gate，随后按审查建议排 R4.20 数据流地基、R4.21 共享 runtime、R4.22 mutation editor 迁移。数据库验收后续优先本地构建/本地 PG+Redis 复跑。
...
| `r4-19-proposal-advanced-split-migration-plan-2026-06-11.md` | **R4.19**:Proposal advanced split migration，计划先迁 readonly summary / conflict boundary，mutation-heavy editors 保留 fallback；承接 dirty edit SSE guard 与 fixture-chrome 冻结 gate | planned |
