...
| **R9-OQ-NAMING-PASS** | 普通用户审查（2026-07-08）：核心概念跨页多名——「AI 拆的计划」5 个叫法（任务计划/分工计划/计划提议/军团计划…且计划审阅页头写「交付物变更申请」）、工作项/任务/事项混用、「发起新任务」入口 6 个叫法而导航叫「接入」、变更申请状态词三套（已合并/已采纳/已合入）、推进中/进行中/执行中混用 | 与 R9-OQ-EN-NAMING 同性质但范围更大（zh 为主）：横跨 nav/masthead/route-components/cuu/api 文案与几十处测试断言，需要先定权威词表再一次 copy pass | 2026-07-08 普通用户视角 workflow（design-language surface，4 条 medium+2 low 全部指向命名） | 先在 glossary-dejargon.md 定四组权威词（计划=「任务计划」/工作项=「任务」/入口=「新任务」/采纳=「已采纳」），再专门 copy-pass commit 同步断言 | 记录待办，与 EN-NAMING 合并成一次词表统一切片 |
| **R9-OQ-QUEUE-PROMOTE** | 普通用户审查（2026-07-08）：首页「其余待办」行点击落点五花八门（冲突行→回放页、审批/升级行→无处理动作的工作项页），没有一个落点能把卡处理掉 | 正确交互=点击队列行把该卡提升为首页主卡（原地展开动作），需要 home 渲染支持 focus 参数或客户端状态交换，牵动 attention VM 渲染与 SSE 重渲时序 | 2026-07-08 普通用户视角 workflow（daily-decisions surface，medium×2） | web 端实现 ?focus=<attention_id>：loader 把命中项与 primary 交换；队列行 href 改 /?focus=id | 单独切片实施 |
| **R9-OQ-APPROVAL-POLICY-UI** | 普通用户审查（2026-07-08）：勾选「以后同类审批自动通过」建立的常驻策略在 web 无处查看、无处撤销 | 需要 settings 页新增权限策略区（GET/PUT /api/permissions 已存在，纯前端缺口），含撤销动作与空态 | 2026-07-08 普通用户视角 workflow（operability surface，medium） | settings 页渲策略列表 + 删除动作接 /api/permissions | 单独切片实施 |
| **R9-OQ-CONFLICT-TOAST** | 普通用户审查（2026-07-08）：采纳撞冲突时整套冲突解决卡塞进右下角 420px 常驻 toast，无关闭按钮、无高度上限 | 冲突解决应有独立面层（内联到提议页冲突区或模态），改动牵 merge 冲突流与 smoke | 2026-07-08 普通用户视角 workflow（operability surface，medium） | 冲突选项渲进提议页 data-r4-proposal-conflicts 区内联展开；toast 只留一句「有冲突需要选择」+跳转 | 单独切片实施 |
| **R9-OQ-USER-R2-REMAINDER** | 普通用户审查 R2（2026-07-08 sonnet 驳斥轮，45 确认）已修 7 高+8 中低；余：确认步回显需求摘要、首页「AI 正在做」行加入口、交付物卡回链来源、成本三口径注环、toast 回执与刷新提示共槽、桌面批（launcher IME 组合态回车误触/capability Esc 丢未保存输入/搜索整句落死路应转新任务/Spotlight 审批卡渲 evidence_refs/知识证据 target=_blank 在 Tauri 无承接）、Cuu 口吻与生肉 token 簇（并入 NAMING-PASS 扩为 tone+token pass）、L 批（会议洞察空态范围/页头大数字标签/回收站按钮语义/角色徽章图例/docx 预览/桌宠设置发现性） | 单项都可修但量大，本迭代按经济模式收口 | 清单 scratchpad/user-review-r2.json（driver 逐条带 file:line） | 下轮 loop 按清单继续，桌面批优先（IME/Esc 是真数据丢失） | 下轮 loop 迭代处理 |
| **R9-OQ-UNVERIFIED-35** | 2026-07-08 普通用户视角 workflow 有 ~35 条发现的对抗驳斥因 session 限额中断而丢失（含 IME 组合态回车、桌宠卡无关闭退路、文件夹不可打开、intake 静默建英文项目名等） | 未经驳斥不能直接当真发现修（历史上驳回率 ~10-30%） | 完整发现清单在 workflow wsdsvbp6o 输出的 failures 段 | 限额恢复后对这批重跑 verify-only workflow，确认者进下一轮修复批 | 下一轮 loop 迭代处理 |
