...
- Cuu 已有卡片、motion hint、sprite manifest、controller MVP、18 clip 绿幕/alpha motion pack、基础 idle scheduler、Rust injected pet surface、浏览器调试 surface、pet surface 静态视觉 QA、pet window 几何合同、command scaffold、webview pointer/drag bridge、Windows debug `PrintWindow` 像素 smoke、内联静态兜底图、Bongo-style 默认 renderer、Bongo P1b 动作增强、Bongo P1c first-painted 首帧门禁、BONGO-REF model pack 默认门禁、BONGO-P2a-a model pack registry / loader、BONGO-P2a-b Settings 模型包 UI、BONGO-P1d-a 窗口手感契约、BONGO-P1d-b-a hide-on-hover 软隐藏 / 恢复真实录屏、BONGO-P1d-c 设置矩阵真实截图、BONGO-P1e-b/P1e-c/P1e-d-a/P1e-d-b 真实输入/长驻录屏、真实 Tauri Bongo GIF/MP4、Live2D 分层拆件概念图、正面基准稿、PSD 生产板、`contract_only` 图层 manifest、144 层 PSD draft 和 `psd_draft_probe`；`CuuMotionHint.sprite_state` 业务状态与 idle / interaction 微动作已 full coverage，但真实形态仍缺 Bongo 动作二轮幅度、full hide/pass-through 安全恢复、精修 Live2D 分层 PSD / Cubism rig / runtime、anchor / 压缩 / 性能 / 多屏 QA。
...
| 缺口 | 为什么重要 | 目标落点 |
|---|---|---|
| 真实小猫动画资产 | 18 clip motion pack 已落，业务状态与 idle / interaction micro action 均 full coverage；Bongo-style 默认 renderer、Bongo P1b 动作增强、P1c first-painted 首帧门禁、BONGO-REF model pack 默认门禁、BONGO-P2a-a model pack registry / loader、BONGO-P2a-b Settings 模型包 UI、BONGO-P1d-a 窗口手感契约、BONGO-P1d-b-a hide-on-hover soft dodge、BONGO-P1d-c 设置矩阵真实截图、BONGO-P1e-b/P1e-c/P1e-d-a 输入手感真实录屏、BONGO-P1e-d-b 60s 长驻可见 / 防闪烁 QA 和真实 Tauri GIF/MP4 已落；Live2D 分层概念图、正面基准稿、PSD 生产板、`contract_only` manifest 和 `psd_draft_probe` 已落；要复现概念图仍需 Bongo 动作二轮幅度、full hide/pass-through 安全恢复、正式 PSD、Cubism rig、anchor 微调、透明边缘 QA、WebP/PNG 压缩、帧率与长驻性能验证 | `packages/cuu/src/model-pack.ts`、`apps/desktop-webview/src/cuu-bongo-runtime.ts`、`apps/desktop-webview/src/assets/cuu/*`、`apps/desktop-webview/src/assets/cuu/live2d/*` 或未来 `client-tauri/web-src/src/assets/cuu/*` |
...
| CUX-BONGO-014 | Settings 模型包 UI 与偏好保存 | `packages/ui/src/gold-path/render.ts`、`packages/ui/src/gold-path/i18n.ts`、`packages/ui/src/gold-path/app-shell.ts`、`packages/ui/package.json`、`packages/cuu/src/controller.ts`、`packages/cuu/src/model-pack.ts`、`apps/desktop-webview/src/cuu-preferences.ts`、`apps/desktop-webview/src/browser.ts`、`docs/workhub/05-clients/assets/audit/2026-06-08-cuu-settings-model-pack-ui/*` | CUX-BONGO-013 | **已落 BONGO-P2a-b**：`/settings` 与 Cuu 偏好气泡展示同源 model pack choices；`cuu-bongo-p1` 可选且当前默认，`cuu-live2d-cubism-v2` 实验锁定且 disabled；中英截图已落，非法/实验 `pet_model_pack_id` 会被归一化拒绝 |
| CUX-BONGO-015 | Web rail 生成图视觉替换 | `packages/ui/src/gold-path/render.ts`、`packages/ui/src/gold-path/render.test.ts`、`apps/web/public/assets/cuu/*`、`apps/desktop-webview/public/assets/cuu/*`、`docs/workhub/05-clients/assets/cuu/generated-polished/*`、`docs/workhub/05-clients/assets/audit/2026-06-08-cuu-polished-rail/*` | CUX-BONGO-014 | **已落 BONGO-P2a-c**：用 GPT Image 生成 Cuu 绿幕源图，抠图裁切为透明 PNG；Gold Path rail 优先加载 bitmap，CSS 几何小猫降为 fallback；截图确认不再是粗糙占位，但独立 pet window 仍需 Bongo 动作二轮 / Live2D 验收 |
