 ...
 *
 * Vendored near-verbatim from `reference-pi/packages/agent/src/agent-loop.ts`
 * (MIT, commit dcfe36c7). See ../NOTICE.md for provenance and the full list of
 * adaptations. The control flow of `runLoop` and its helpers is intentionally
 * kept line-for-line comparable with the pi original. The only changes are:
 * - import paths repointed to the local vendor barrel (`./compat.js`, `./types.js`);
...
export function agentLoopContinue(
	...
	// `!`: guarded by the length check above (WorkHub noUncheckedIndexedAccess).
