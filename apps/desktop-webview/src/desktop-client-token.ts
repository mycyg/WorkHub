...
// 缓解现状：CSP 已收敛（connect-src 仅 'self' + 本机回环，见 tauri.conf.json），即使令牌被读到也难以
// 经 fetch/EventSource 外泄；真正的修复是把令牌迁到 Rust 壳层托管存储（壳层已有 set_client_token 通道
// 管一份内存副本供 SSE worker 用），迁移提案见
