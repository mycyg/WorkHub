...
- 当前 `ai_fusion` 写回对 `text_doc/spec_doc` 已是候选正文直写，对其它目标仍是保守 Markdown artifact。R1.20 已把真实 current/incoming/base 文本摘录喂给 LLM 候选生成，R1.21 已在 candidate `quality_gate` 中持久化 text patch preview，R1.22 已在 Replay 页面渲染该 preview，R1.23 已在 Proposal 冲突卡显示采用前最小 diff，但它仍不是字段级原位 patch、自动 text/spec doc diff3 或 React route 级富 patch viewer。真正把融合内容写回结构化记录字段、自动处理无重叠 hunk，需要后续 `ai_resolved` v2 切片。
