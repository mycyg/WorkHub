...
Rust 边界保持不变：Rust 不调用业务 API、不绕过 auth、不拥有 Agent 状态机。下一步 Rust/Tauri 只作为回归门支撑 R4 主窗产品化、route componentization 与后续跨平台打包验收。
