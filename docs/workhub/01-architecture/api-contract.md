...
| **[新]** `POST /api/merge-proposals/{id}/apply` | `{confirm?: true}` | `ProposalMergeResult`；允许未选择的 `ai_fusion` 直接一键采用，服务端会把本次点击写入 `chosen_*` 后物化 Markdown 融合稿，并写入正式 accepted deliverable / Drive version / merge audit；已选择其它候选时 409 | reviewer |
