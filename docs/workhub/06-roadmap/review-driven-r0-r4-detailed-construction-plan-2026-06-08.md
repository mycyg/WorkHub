...
5. **补审批人与 merge 真实语义**
   ...
   - 剩余：完整 permission policy routing、审批中心持久 `ApprovalRequest`、`ai_fusion` v2 自动 text diff3、Proposal 冲突卡富 patch viewer、字段级结构化 patch；R1.9 已先落 deterministic 两选一 API，R1.10 已接端侧按钮，R1.11/R1.12 已接尝试、候选与选择审计，R1.13 已接 replay 展示，R1.14 已接 LLM 融合候选入口，R1.15 已接候选选择审计入口，R1.16 已接 AI 融合稿物化采纳入口，R1.17 已接 Web/Desktop/Cuu 一键采用 AI 融合稿入口，R1.19 已接 text/spec 正文直写，R1.20 已接真实内容三方读取，R1.21 已接数据层 patch preview，R1.22 已接 Replay 可见 patch preview。
...
- 未完成：完整 approval policy routing、`ai_fusion` v2 自动 text diff3、Proposal 冲突卡富 patch viewer、字段级结构化 patch、多冲突工作台仍未完成。R1.9 已关闭“冲突只能裸 409、用户无法点选处理”的最小缺口，R1.11/R1.12 已关闭“冲突选择没有持久尝试和候选审计”的缺口，R1.13 已关闭“replay 看不到当时候选和选择”的缺口，R1.18 已关闭“BudgetPolicy 只在内存 override、无审计”的缺口，R1.19/R1.20/R1.21/R1.22 已关闭 text/spec 正文直写、真实三方文本上下文、数据层 patch preview 与 Replay 可见 patch preview 缺口。
...
- 已由 R1.14/R1.15/R1.16/R1.17/R1.19/R1.20/R1.21/R1.22 部分补齐：LLM `ai_fusion` 候选生成、质量门、持久化、选择审计、Markdown 融合稿正式采纳、冲突卡一键采用、text/spec 正文直写、真实 current/incoming/base 文本上下文、数据层 `text_patch_preview`、Replay 可见 patch preview 与真实 PG one-click smoke 已接入；仍缺自动 text diff3、Proposal 冲突卡富 patch viewer、字段级结构化 patch 和多冲突工作台。
...
- R1.16/R1.19/R1.20/R1.21/R1.22 已补 AI 融合稿物化采纳、text/spec 正文直写、真实 current/incoming/base prompt context、数据层 `text_patch_preview` 与 Replay 可见 patch preview；仍缺自动 text diff3、Proposal 冲突卡富 patch viewer、字段级结构化 patch 与多冲突逐项选择历史。
...
- 已由 R1.14/R1.16/R1.19/R1.20/R1.21/R1.22 部分补齐：LLM `ai_fusion` 候选生成、candidate rationale、recommended option、失败降级、Markdown 融合稿物化采纳、text/spec 正文直写、真实 current/incoming/base prompt context、数据层 `text_patch_preview` 与 Replay 可见 patch preview 已接入；仍缺自动 text diff3、Proposal 冲突卡富 patch viewer 与字段级结构化 patch。
...
- 已由 R1.14/R1.16/R1.19/R1.20/R1.21/R1.22 部分补齐：LLM 融合候选生成、candidate rationale、推荐项、失败降级、Markdown 融合稿物化采纳、text/spec 正文直写、真实 current/incoming/base prompt context、数据层 `text_patch_preview` 与 Replay 可见 patch preview 已接入；仍缺自动 text diff3、Proposal 冲突卡富 patch viewer 与字段级结构化 patch。
...
- 已由 R1.16/R1.17/R1.19/R1.21/R1.22 部分补齐：`ai_fusion` 可物化为正式交付物并走 Drive version / accepted ledger / rollback/audit 链路，冲突卡可一键采用，`text_doc/spec_doc` 会正文直写，candidate 带数据层 patch preview，Replay 可见该 preview；仍缺 `structured_record` 字段合并器、自动 text diff3 与 Proposal 冲突卡富 patch viewer。
...
- R1.20 已补 text/spec 的真实 current/incoming/base prompt context；R1.21 已补数据层 patch preview；R1.22 已补 Replay 可见 patch preview；后续仍需自动 diff3、Proposal 冲突卡富 patch viewer 与结构化字段 patch。
...
- `ai_fusion` v2 原位写回：R1.19 已补 `text_doc/spec_doc` 候选正文直写与冲突 marker 拒绝；R1.20 已补真实 current/incoming/base prompt context；R1.21 已补数据层 `text_patch_preview`；R1.22 已补 Replay 可见 patch preview；仍缺自动 diff3、Proposal 冲突卡富 patch viewer、rollback proof，以及 `structured_record` 字段级 merge policy 与 schema-aware patch。
...
- `ai_fusion` v2 原位写回：R1.19 已补 `text_doc/spec_doc` 的正文直写和冲突 marker 拒绝；R1.20 已补真实 current/incoming/base prompt context；R1.21 已补数据层 `text_patch_preview`；R1.22 已补 Replay 可见 patch preview；仍缺自动 diff3、Proposal 冲突卡富 patch viewer、rollback proof，以及 `structured_record` 字段级 merge policy 与 schema-aware patch。
...
- 真正 diff3：无重叠 hunk 自动合并、重叠 hunk 进入 AI mediation、Proposal 冲突卡富 patch viewer、冲突块逐项确认尚未落地；R1.21 只完成了数据层 current -> merged patch preview，R1.22 只完成了 Replay 可见 patch preview。
...
- Patch UI：R1.22 已在 `packages/ui/src/gold-path/render.ts` 的 Replay timeline 中渲染 `quality_gate.text_patch_preview`，显示 changed、增删行、overlap risk、base 状态与 unified hunk；仍未把 preview 下沉到 Proposal 冲突卡 API/VM，也不是 React route 级富 patch viewer。
...
| Proposal 冲突卡 | 仍只显示 option-first action；要展示 patch preview 需后续扩展 `ProposalConflict` / page VM |
...
- 自动 text diff3 与无重叠 hunk 合并。
- Proposal 冲突卡富 patch viewer 与用户采用前预览。
...
仍不能宣称 R1/R2 全部完成，因为 `ai_fusion` v2 自动 text diff3、Proposal 冲突卡富 patch viewer、字段级结构化 patch、多冲突工作台、完整 approval policy routing、PG claim/多 worker 仍未落地。
