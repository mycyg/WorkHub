...
const outputDir = path.join(
  ...
  "assets",
  "audit",
  "2026-06-11-r4-web-live-api-pg-seed"
);
...
async function startApiServer(port: number) {
  ...
  child.once("exit", (code, signal) => {
    logs.push(`api process exited code=${String(code)} signal=${String(signal)}`);
  });
  try {
...
<head>
  ...
  <title>R4.7 Web Live API PG Seed Contact Sheet</title>
  <style>
...
async function runScenario(cdp: CdpClient, baseUrl: string, seed: R4WebLiveApiPgSeedResult) {
  ...
  await navigate(cdp, `${baseUrl}/missing-r4-live-api-pg-route`, "error");
  steps.push(await captureStep(cdp, { id: "13-unknown-route-error", url: `${baseUrl}/missing-r4-live-api-pg-route`, viewport: desktop, expectedStatus: "error" }));
...
async function seedDatabase() {
  if (settings.appEnv === "production") {
    throw new Error("Refusing to run R4.7 live API PG seed smoke with APP_ENV=production.");
  }
...
async function main() {
  ...
  if (!chromePath) {
    throw new Error("Chrome/Chromium is required. Set CHROME_PATH to run R4.7 live API PG seed QA.");
  }
  ...
  const userDataDir = path.join(os.tmpdir(), `workhub-r4-live-api-pg-${Date.now()}`);
  ...
  try {
    ...
    const gates = {
      ...
      api_server_started: Boolean(api?.child.pid),
    ...
    const report = {
      ...
      module: "R4.7 Web live API + Postgres seed smoke",
    ...
    await writeFile(path.join(outputDir, "live-api-pg-seed-report.json"), `${JSON.stringify(report, null, 2)}\n`, "utf8");
    await writeFile(
      path.join(outputDir, "smoke-summary.md"),
      [
        "# R4.7 Web Live API + Postgres Seed Smoke",
        "",
        `- ok: ${String(ok)}`,
    ...
    if (!ok) {
      throw new Error(`R4.7 live API PG seed smoke failed: ${JSON.stringify(gates)}`);
    }
