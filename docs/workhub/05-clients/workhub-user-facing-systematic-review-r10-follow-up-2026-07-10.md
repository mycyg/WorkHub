...
本节是原始审查之上的 living remediation ledger，只记录修复与 fresh 验证证据；上方原始审查基线、下方 P0-1 历史发现及整份报告的 `HOLD` 发布结论均保持不变。`FIXED_REVIEWED` 的严格含义是：该 finding 的实现修复已提交，已在所记录的实现 head 上完成 fresh verification，且已通过独立 task review 与 whole-batch implementation review；它只关闭该 finding，不表示 Batch 0 或产品已完成，也不表示产品已达到 9/10 或可发布。因此，当前 P0-1 已关闭，但由于仍有 P1 阻断项未收口，整份报告仍保持 `HOLD`。

| Finding | Status | Remediation commits | Fresh verification evidence |
|---|---|---|---|
| P0-1 | `FIXED_REVIEWED` | Task 1 directory scope: `110e415d4`; Task 1 fail-closed/repository hardening: `5aab1337b`; Task 2 delegate authorization: `58a3168a7`; Task 2 active-user directory hardening: `8865577cc`; Task 2 dependency-contract traceability comment: `0e04838c9`; Task 3 literal DOM options: `591510f7f` | Fresh verification at current implementation head `0e04838c9`:<br>`pnpm --filter @workhub/api test`: tests `853`, pass `853`, fail `0`, exit `0`.<br>`pnpm --filter @workhub/web test`: tests `68`, pass `68`, fail `0`, exit `0`.<br>`pnpm --filter @workhub/api typecheck`, `pnpm --filter @workhub/web typecheck`, and `pnpm --filter @workhub/db typecheck`: all exit `0`.<br>`pnpm audit:portable-config`, `pnpm audit:target-paths`, and `pnpm audit:migrations`: all exit `0`.<br>Fresh isolated PostgreSQL 16/Redis 7 Pilot smoke at the same implementation head, using host ports `55439`/`6389` and `pnpm qa:r2-pg-redis-smoke`: exit `0`, `workspace-scoped active member directory isolation ok`, final `ok: true`, owner SSE `200`, stranger SSE `403`, and `run_status: succeeded`.<br>Fresh cross-workspace exploit regression `pnpm --filter @workhub/api exec node --import tsx --test --test-name-pattern="delegate rejects an active user without an actor-workspace membership" src/approvals.test.ts`: matched `1`; TAP total `1`, pass `1`, skipped `0`, fail `0`, exit `0`; the regression produced no routed-user mutation, notification, SSE event, or `approval.delegated` success audit.<br>Fresh active-user fail-closed follow-up at `8865577cc`: focused directory/soft-delete/repository-throw tests TAP total `3`, pass `3`, fail `0`, exit `0`; focused OpenAPI contract TAP total `1`, pass `1`, fail `0`, exit `0`; full `src/approvals.test.ts` TAP total `67`, pass `67`, fail `0`, exit `0`; `pnpm --filter @workhub/api typecheck` exit `0`. All three active-user regressions assert unchanged routing, no notification create/archive, no SSE/bus event, no `approval.delegated` success audit, and no membership lookup before a proven active user; missing user-directory capability returns `503 delegate_user_directory_unavailable`, while an absent/soft-deleted active user remains the non-enumerating `404 delegate_target_not_found`.<br>Fresh unsafe-handle search at the same implementation head: `rg -n -e "listActiveRefs\b" -e "select\.innerHTML = result\.users" packages/db/src apps/api/src apps/web/src`: no matches (expected `rg` exit `1`).<br>Final independent whole-batch implementation review at `0e04838c9`: Critical `0`, Important `0`, Minor `0`. |
