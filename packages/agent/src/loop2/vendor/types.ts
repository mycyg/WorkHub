/**
 * Vendored near-verbatim from `reference-pi/packages/agent/src/types.ts`
 * (MIT, commit dcfe36c7). See ../NOTICE.md for provenance and adaptations.
 *
 * Adaptations vs pi original:
 * - pi-ai type imports repointed to the local `./ai-types.js` closure and the
 *   `AssistantMessageEventStream` class type to `./event-stream.js`.
 * - typebox removed: `TSchema` -> `JsonSchema` (JSON Schema object shape) and
 *   `Static<TParameters>` -> `Record<string, unknown>` for tool argument types.
 *   Real schema typing/validation is delegated to zod in the Phase 1 tool wrapper.
 * - `AgentToolResult.terminate` widened to include `undefined` for WorkHub's
 *   `exactOptionalPropertyTypes` (the loop's afterToolCall merge assigns it).
 */
 ...
 *
 * pi typed `TParameters` with typebox and derived argument types via `Static`.
 * The WorkHub vendor drops typebox: `parameters` is a JSON Schema object and tool
 * arguments are `Record<string, unknown>`. Concrete zod typing lives in the Phase 1
 * tool wrapper.
 */
export interface AgentTool<TParameters extends JsonSchema = JsonSchema, TDetails = any> extends Tool<TParameters> {
	/** Human-readable label for UI display. */
	label: string;
	/**
	 * Optional compatibility shim for raw tool-call arguments before schema validation.
	 * Must return an object that matches the tool's parameters.
	 */
...
export interface AgentContext {
	...
	/** Tools available for this run. (Widened to `| undefined` for `exactOptionalPropertyTypes`.) */
