...
| Desktop 主窗 | Tauri webview 加载、surface 分流、Rust bridge、`/settings` pet 恢复面板、pet settings event bridge、右键 hover -> main settings 同步截图 | 安装包、设备令牌门、生产更新、物理系统托盘点击证据、跨平台 smoke |
| Rust shell | 窗口、SSE、通知、deep-link、pet geometry、cursor sample、托盘 `restore-pet-interaction` 源码门与 command-backed handler 恢复证据 | 私有 SSE 重连、动态托盘状态、本地同步、跨平台截图/权限策略、物理 OS 托盘点击录屏 |
| Cuu 桌宠 | 独立 pet window、黑猫/白猫 Live2D registry、偏好二选项、QA 合同、概念图已同步真实模型帧、pass-through 源码恢复门、P1.6 behavior manifest 源码合同、P1.7 业务录屏入口与黑猫 approval smoke、R3.10 真实 launcher/en-US capture、R3.12 真实 zh-CN/en-US run-stream completion capture、R3.13.1 真实 zh-CN/en-US run-failure terminal capture、R3.13.2 真实 zh-CN/en-US 401/403/offline capture、R3.13.3 webview boot session/run restore、R3.15 真实 reload capture、R3.17 settings/menu matrix、R3.18 主窗 pass-through 恢复、R3.19 tray handler 恢复、R3.20a 右键 hover -> 主窗 settings 截图与文本边界证据 | 长期性能、授权或原创替换 |
...
| Cuu 是会动的小猫桌宠 | 黑/白 Live2D registry 已落，概念图已同步真实模型帧，P1.6 behavior manifest 源码合同已落，P1.7 可录真实业务场景且已有黑猫 approval smoke，R3.12 已有真实 Tauri run-stream completion 中英双语证据，R3.13.1 已有真实 Tauri run-failure 中英双语证据且 trace card 文本/预算区无超框，R3.13.2 已有真实 Tauri 401/403/offline 中英双语证据且右缘无裁切，R3.13.3 已有 webview boot session/run 恢复单测，R3.15 已有真实 reload capture，R3.17 已有 settings/menu matrix，R3.18 已有 `/settings` pass-through 恢复证据，R3.19 已有 tray handler 恢复与中英短提示文本边界证据，R3.20a 已有右键菜单 hover -> 主窗 settings 中英截图 | 长期性能 |
| Cuu 不在 Web/主窗里 | 当前文档和代码收束中 | 截图审查确认无主窗 Cuu 本体 |
| Rust 客户端哲学是轻、气泡、少打扰 | Tauri shell、托盘 settings、pass-through 恢复源码门、tray handler command-backed 恢复证据已对齐 | 通知、deep-link、物理 OS 托盘点击录屏、安装包 |
...
| CUX-L2D-05 | 恢复策略 | `/settings` pass-through 恢复与 tray handler 恢复证据已落；仍需物理 OS 托盘点击、full hide、多屏恢复 |
...
| PET-R2 | tray recovery | Cuu 显隐、主窗、收件箱、设置已落；`restore-pet-interaction` 已落且有 command-backed handler 恢复证据；继续补物理 OS 托盘点击录屏 |
...
| Cuu motion | P1.10 以前的黑/白 contact sheet、GIF/MP4、diff report只作为回归证据；R3.10 launcher/en-US、R3.12 zh-CN/en-US run-stream completion、R3.13.1 zh-CN/en-US run-failure terminal、R3.13.2 zh-CN/en-US 401/403/offline、R3.15 reload restore、R3.17 settings/menu matrix、R3.18 `/settings` pass-through 恢复、R3.19 tray handler 恢复与 R3.20a hover sync 是当前业务功能视觉证据；R3.13.3 是 webview boot 恢复源码/单测证据；R3.13.1 要求最终帧无文本/预算超框，R3.13.2 要求 `right_edge_clip_gate` 通过，R3.17-R3.20a 要求 settings/menu/recovery layout gate 与 `overflow.offenders=[]`；不新增外观矩阵 |
...
| R3 | Cuu 自然语言 / option-first 出站 Agent 入口，不新增外观；R3.12 已补真实 Tauri run-stream completion，中英双语证据；R3.13.1 已补真实 Tauri run-failure terminal，中英双语证据；R3.13.2 已补真实 Tauri 401/403/offline，中英双语证据；R3.13.3 已补 webview boot session/run 恢复；R3.15 已补真实 reload capture；R3.17 已补 settings/menu matrix；R3.18 已补 `/settings` pass-through 恢复；R3.19 已补 tray handler 恢复与文本边界；R3.20a 已补 hover sync 截图与主窗 overflow gate；下一步物理 OS 托盘点击、Linux/macOS smoke |
