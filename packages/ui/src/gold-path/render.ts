...
export type GoldPathRenderedPage = {
  key: "home" | "intake" | "approvals" | "workitem" | "proposal" | "drive" | "meetings" | "notifications" | "calendar" | "replay" | "cost" | "knowledge" | "settings";
  route: string;
  title: string;
  html: string;
  primaryHrefs: string[];
...
export const goldPathCss = [
  ...
  ".wh-title{font-size:30px;line-height:1.24;margin:8px 0 8px;overflow-wrap:anywhere}.wh-subtle{color:var(--muted);line-height:1.55;overflow-wrap:anywhere}.wh-grid{display:grid;grid-template-columns:repeat(auto-fit,minmax(min(220px,100%),1fr));gap:14px;margin-top:18px;min-width:0;max-width:100%}",
