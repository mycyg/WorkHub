...
R4.20 完成后已进入并完成 R4.21 shared web runtime、R4.22 Proposal mutation editor 第一段迁移、R4.23 Proposal line editor 第二段迁移与 R4.24 Web runtime finalization：Web 与 desktop-webview 的 dispatcher、notice、locale、dirty guard、SSE refresh 运行时已收敛到 `@workhub/web-runtime`，Proposal structured field scalar editor 与 line editor 已完成 visible React controlled-state 验证，生产导航不再写 hash route。下一步进入 R5.1 Drive business slice。
