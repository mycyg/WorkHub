...
气泡位置验收补充：业务 card 使用 `520x720` 透明 pet canvas，为长失败/预算卡预留纵向安全区；气泡必须围绕 Cuu 本体出现。当前 full card 锚点为 `left:88px; bottom:392px; width:300px`，猫体为 `right:72px; bottom:72px; width:240px; height:320px`。如果气泡回到透明窗口左上、明显远离 Cuu，或文本横向越出气泡框，视为概念图复现失败。
...
### 6.5 R3 Agent 出站入口（R3.1/R3.2/R3.3/R3.4 已落）

R3.1 已新增 Cuu option-first 启动卡，详见 [`cuu-r3-agent-entry.md`](./cuu-r3-agent-entry.md)。当前交互是：用户点击独立 pet window 中的 Cuu body，如果没有正在展示的业务卡片，Cuu 展开 `cuu-agent-launcher` 轻卡；用户点选「文档/方案草稿」「结构化数据」「小型代码/模板」之一后，`start_agent_from_cuu` typed action 先调用真实 `POST /api/sessions`。如果后端返回 `SessionVM.question.options[]`，Cuu 展示 `cardFromSessionVm(session)`，继续让用户点选澄清项；如果后端不要求澄清，再调用真实 `POST /api/workitems`、`POST /api/workitems/:id/agent-runs`，最后把 `AgentRunLiveVM` 转成 Cuu 进度卡。

R3.2 已补第一版 run stream 回流：启动成功后 `pet-surface.ts` 订阅 `AgentRunLiveVM.stream_href`，匹配 run 事件后重新拉 `GET /api/agent-runs/:id` 并刷新 Cuu card；`budget_exhausted`、403/401、offline/network 和 generic error 会变成 Cuu 轻卡。R3.3 已补 `SessionVM.question` 回退与 `nextQuestion()` 返回 question card，避免桌宠绕过澄清或只显示一句 message。R3.4 已把确认题 `create-workitem` 接成真实 typed action：先记录澄清答案，再 `createWorkItem({kickoff_agent:true})`，最后 `startAgentRun()` 并返回 run card。该切片仍是 TS 合同和单元测试，不替代真实 Tauri pet window 截图/录屏。
...
| 真实链路 | `SessionVM -> question card -> confirm -> WorkItemDetailVM -> AgentRunLiveVM` |
| 返回 | 需要澄清时 `cardFromSessionVm(session)`；启动成功时 `cardFromAgentRunLive(run)`，run stream 后续刷新同一类卡 |
...
| 验证 | `@workhub/desktop-webview` test/typecheck 已覆盖 launcher DOM、三段 API 调用、SessionVM 回退、nextQuestion card、确认后启动、run stream refresh、错误卡 |

R3.4 仍不能算 R3 完成：还缺真实 Tauri 点击截图、真实 daemon launcher-to-run smoke、刷新恢复，以及 API dev server 证明该链路不是 mock client 内循环。
...
| R3 | 恢复 Cuu 功能施工：新增自然语言 / option-first 出站指令入口，驱动真实 R1/R2 Agent 引擎 | R3.1 已落 option-first launcher；R3.2 已落 TS run stream 回流和错误态；R3.3 已落 SessionVM question 回退；R3.4 已落确认后启动；R3.5 待补真实 Tauri 截图与 launcher-to-run smoke |
