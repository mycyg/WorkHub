...
11. R3.21 已补 Linux 首轮 X11/Xvfb 截图方案：`scripts/qa/cuu-tauri-linux-smoke.sh` 会在 Linux 上跑 WebView test/build、Tauri cargo test/build，再用 Xvfb + openbox + Vite devUrl 捕获 `WorkHub` 主窗、独立 `Cuu 520x720` pet window、DOM report 和全屏截图；证据在 `./assets/audit/2026-06-11-cuu-r3-linux-tray-smoke/mycyg-xvfb-openbox-devserver-smoke/`。
12. Linux/macOS 策略已由 R3.23 补齐：Windows 继续用 Win32 `PrintWindow` + UIA tray driver；Linux 用 GNOME StatusNotifier/AppIndicator DBusMenu action；macOS 用 menu bar status item + Accessibility + Screen Recording 真机 smoke。后续这些路径作为回归门，不再是当前阻塞。
