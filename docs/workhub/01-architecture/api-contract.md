...
| **[新]** `POST /api/merge-proposals/{id}/apply` | `{confirm?: true}` | `ProposalMergeResult`；允许未选择的 `ai_fusion` 直接一键采用，服务端会把本次点击写入 `chosen_*` 后物化融合稿，并写入正式 accepted deliverable / Drive version / merge audit；R1.19 起 `text_doc/spec_doc` 直接写候选正文，非文本/结构化目标保留 Markdown artifact；已选择其它候选时 409 | reviewer |
