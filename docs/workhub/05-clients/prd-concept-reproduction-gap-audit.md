...
- Cuu 已有卡片、motion hint、sprite manifest、controller MVP、18 clip 绿幕/alpha motion pack、基础 idle scheduler、Rust injected pet surface、浏览器调试 surface、pet surface 静态视觉 QA、pet window 几何合同、command scaffold、webview pointer/drag bridge、Windows debug `PrintWindow` 像素 smoke、内联静态兜底图、Bongo-style 默认 renderer、Bongo P1b 动作增强、Bongo P1c first-painted 首帧门禁、BONGO-REF model pack 默认门禁、BONGO-P1d-a 窗口手感契约、真实 Tauri Bongo GIF/MP4、Live2D 分层拆件概念图、正面基准稿、PSD 生产板、`contract_only` 图层 manifest、144 层 PSD draft 和 `psd_draft_probe`；`CuuMotionHint.sprite_state` 业务状态与 idle / interaction 微动作已 full coverage，但真实形态仍缺 Bongo 动作二轮幅度、窗口设置真实截图/hide-on-hover、model pack loader、精修 Live2D 分层 PSD / Cubism rig / runtime、anchor / 压缩 / 性能 / 多屏 QA。
...
| 拖拽 / 收起 / 静音 / 勿扰 | 静音 / 勿扰 / 减少动效 / 队列上限 / 尺寸 / 透明度 / 点击穿透已有 desktop webview 面板；拖拽 bridge、Rust drag plan、真实位置记忆和 pass-through 命令已落；收起细节、hide-on-hover 与真实 Tauri Settings 承接仍待补 | Rust window state + TS preference |
...
| CUX-BONGO-007 | Bongo P1e-c 连续看鼠标与 hover 避让 | `apps/desktop-webview/src/pet-window-bridge.ts`、`apps/desktop-webview/src/pet-surface.ts`、`packages/cuu/src/idle-scheduler.ts`、`scripts/qa/cuu-tauri-motion-capture.ps1` | CUX-BONGO-006 | 待做：`cursor_x/y` 平滑映射到头部/眼球偏移；鼠标靠近目标区域时 Cuu 轻避让或临时收起；drag 姿态保持到 release；同一 input-handfeel 门重新录屏 |
...
2. **Bongo P1d-b / P1e-c 动作与窗口体验**：P1d-a 已补缩放、透明度和点击穿透契约，P1e-a/P1e-b 已补 cursor-near、pointer DOM attrs 与 hover/tap/drag 真实录屏底座；下一步补真实 Tauri 设置截图、hide-on-hover、hover 避让、多屏恢复、显示/隐藏快捷入口、连续看鼠标参数，并加大抱文件 / 审批敲桌 / 完成庆祝的动作幅度。
