...
test("R9.3 agent memory repository writes new L1 memory with an append-only version", async () => {
  ...
  // R9.3 triage: the old assertion started at the agent_memory lookup, which was wrong because
  // an arbitrary task_plan_item id could be used unless the parent task plan workspace is checked first.
...
test("R9.3 agent memory repository refuses L1 writes outside the workspace task-plan context", async () => {
  const { db, queries } = createQueryRecorder([[]]);
  const repository = createAgentMemoryRepository(db);
...
test("R9.3 agent memory repository appends when a concurrent first L1 write wins the unique key", async () => {
  const updatedRow = {
    ...memoryRow,
    valueMd: "并发输家补充偏好",
    baseVersion: 1,
...
test("R9.3 memory promotion context reads same-plan L1 candidates with source actor and workspace filters", async () => {
  ...
  // R9.3 triage: the old assertion stopped at task_plan_items and source_run workspace; it missed
  // the parent task_plans.workspace_id fence, so cross-workspace L1 entries could be considered.
