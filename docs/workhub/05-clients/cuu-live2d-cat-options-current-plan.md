...
2. 当前工程主线：继续 R1/R2 后端缺口；真实 `sessions/workitems/knowledge/page workitem` service、CostLedger 默认 store、merge accepted deliverable ledger、persistent merge audit、AgentRun-backed delivery 正式文件落盘、WorkItem page / AgentRun replay accepted deliverables 与下载/文本预览已接入，下一批优先正式交付物 revert、AI 冲突调解、BudgetPolicy 持久化、PG queue claim 与多 worker。
