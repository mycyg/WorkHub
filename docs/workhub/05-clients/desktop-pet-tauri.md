...
> **当前截图 / 动作审计**：见 [`current-state-visual-audit-and-construction-plan-2026-06-07.md`](./current-state-visual-audit-and-construction-plan-2026-06-07.md)。2026-06-07 已对 Web、desktop webview、browser pet preview 和真实 Tauri `Cuu` 窗口做截图，并对 `Cuu` 顶层窗口做 32 帧 motion capture；当前 P0 缺口是 card mode 事件卡裁切。
...
| `apps/desktop-webview/src/cuu-atlas-runtime.ts` | Cuu atlas renderer | 按 atlas frame rect 生成 clip sheet `<img>` frame stack，非覆盖状态会标记 fallback；内联静态 Cuu fallback 保证 Tauri/WebView2 大 PNG 加载异常时仍显示呼吸态 Cuu |
...
| `scripts/qa/cuu-tauri-smoke.ps1` | Windows Tauri runtime smoke | 启动真实 debug app，定位 `Cuu` 顶层窗口，校验 visible/topmost/bottom-right，隐藏主窗，并用 `PrintWindow(PW_RENDERFULLCONTENT)` 对透明/layered WebView2 pet 窗口做像素检查 |
...
- **当前 motion capture 发现（2026-06-07）**：32 帧 `PrintWindow(PW_RENDERFULLCONTENT)` 抓取确认 Cuu body-only 窗口有轻微呼吸/轮廓变化；但第 20 帧附近出现离线卡片时，`pet` 窗口仍停在约 `194 x 228` 的 body-only 物理尺寸，导致卡片正文和 Cuu 被裁切。后续 P0 修复必须审计 `pet-window-bridge.ts` 的 Tauri invoke 可用性、`pet-surface.ts` 的 card mode await/fallback、`main.rs` 的 `set_pet_window_mode` command diagnostic，并把 motion capture 脚本纳入 QA。
...
- **视觉资产（新增硬约束）**：P1 不再接受抽象图标或 procedural CSS 作为完成标准；当前已有 18 clip 绿幕 motion pack 和内联静态 Cuu fallback，用于证明业务动作与微动作都可进入运行时，并保证 WebView2 大图加载失败时不是空窗口。生产前继续做 anchor 微调、WebP/PNG 压缩、alpha 边缘 QA、长时间 idle 性能检查。
- **两态**：
