...
| `apps/desktop-webview/src/cuu-preferences.ts` | Cuu preference panel | 右上角轻入口，面板默认隐藏；本地存储 `attention_mode` / `sound_mode` / `reduced_motion` / `queue_limit` / `pet_scale_percent` / `pet_opacity_percent` / `pet_pass_through` / `pet_hide_on_hover`，把点击偏好写回 `CuuController`，并允许 Rust QA 初始化脚本注入窗口设置偏好 |
...
| Cuu 偏好 / shell 设置 | desktop webview 内已有偏好面板和本地存储；P1d-a 新增尺寸 75/100/125/150、透明度 60/80/100、点击穿透；P1d-b-a 新增 `pet_hide_on_hover` 软隐藏开关；P1d-c 已用真实 Tauri 设置矩阵确认这些偏好能进入 pet window；`pet-window-bridge.ts` 会把主窗偏好同步到独立 pet window，Rust 侧已有 `pet-window-state.json` 位置落盘、`workhub-shell-config.json` 读取入口和 `set_pet_window_settings` | 迁入真实 Tauri Settings / pet window，接设备注册、安全 vault、托盘显隐、多屏恢复实测、系统通知偏好和 full hide/pass-through 安全恢复 |
...
- **当前窗口设置矩阵**：P1d-c 已用 `scripts/qa/cuu-tauri-settings-capture.ps1` 在真实 Tauri `Cuu` 顶层窗口里验证 default、scale-75、scale-150、opacity-60、pass-through、hide-on-hover、combo-125-80-pass-hide。`settings-capture-report.json` 中 `passed=true`，所有 case 首帧非空、Cuu 全身可见，不再出现只露耳朵或缩放后裁切。它证明窗口能力合同已接到真实 pet window；真实 Settings 页面 UI 截图仍是下一步。
...
- **当前偏好面板**：`apps/desktop-webview/src/cuu-preferences.ts` 已提供右上角轻入口，面板默认隐藏；展开后可设置提醒模式（正常/安静/勿扰）、声音（开启/静音）、减少动效、队列上限、桌宠尺寸、透明度、点击穿透和悬停避让；偏好写入 localStorage 并同步到 `CuuController`，主窗会通过 `pet-window-bridge.ts` 立即调用 `set_pet_window_settings` 更新独立 pet window。
