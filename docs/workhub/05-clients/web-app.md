...
| 项 | 当前实现 | 后续目标 |
|---|---|---|
| Fixture chrome retirement | `apps/web/src/routes.ts` ready route 不再调用 `/api/pages/gold-path`；product shell 从前端真实 route registry、active Page VM metrics 和 active-only route component 组装 | R4.21 继续把 shared shell/runtime 抽包，R4.22 前不再让生产 route 依赖 P0.5 fixture surface |
| App-level SSE | `apps/web/src/browser.ts` 维护 app-level EventSource map，按 URL 复用 stream；route switch 只同步 target set，`stream/me` 不随 render 重建 | R4.21 抽成可注入 shared runtime，给 desktop-webview 对齐 |
