...
| N9 | **拉小群**:项目内小协作组群聊,建群时选成员,可选 Cuu 是否进入(默认进入) | 新批 G1·小群（地基现成:conversation_participants 已支持多人,kind 沿用 collab+参与者>1 即小群;需加 cuu_enabled 布尔迁移;**Cuu 触发语义=小群里 @Cuu 才回话**(逐条自动回在多人场景是骚扰),不进入=纯人类群 turns 直接禁用;主区观察者不进小群;建群 UI 长在 P2 的新建会话入口上,成员多选+Cuu 开关） |
