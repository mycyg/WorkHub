...
P0「地基」把现有 **需求管理大师**(FastAPI 单体 + SQLite 单 worker + Tauri/Web 客户端)迁移演进为 **WorkHub 的可并发地基**:一个 **headless agent daemon**(FastAPI + OpenAPI + SSE)+ **PostgreSQL** + **消息 broker**,把 `auto_agent` 抽成可复用的 **Agent 引擎核心**,并补齐 **分层权限 / 审计快照 / provider 抽象** 等 AI-native 必需底座。
...
现有单体有四个"假设单 worker"的进程内单例,**多 Agent + 多人并发下会静默出错**(`DEPLOY.md:97`):SSE bus、presence、AI 澄清并发槽、后台任务去重。AI-native 的核心是"AI 默认在后台大量干活",这与单 worker 天花板**直接冲突**。同时缺失:Alembic 迁移、provider 抽象、分层权限、通用快照/回滚。P0 就是拆掉这堵墙、补齐这些底座。

## 3. 范围(Scope)

**In(P0 必须):** 仓库脚手架与配置可移植化;实体迁移 + `requirements→work_items`;PostgreSQL + Alembic;鉴权/设备令牌移植;事件 bus → broker(解除单 worker);分层权限引擎骨架;LLM provider 注册表(改接 7 处);Agent 引擎核心抽取(loop/工具注册表/沙箱/预算 + AgentRun 持久化);生命周期/通知扩展(登记新状态);审计 + 快照/回滚红线;headless daemon 拆分 + 客户端最小改接(OpenAPI 客户端 + 跨域)。

**Out(明确推迟):** 智能派活(P2)、经理模式编排(P2)、分支-提议-合并的完整合并语义(P3)、双向同步(P3)、Cuu 桌宠与页面(P3/P4)、看板/度量(P4)、多租户公网(P5)。
...
```
                 ┌──────────────── WorkHub headless daemon (FastAPI, N workers) ────────────────┐
  Web (SPA)  ──► │  OpenAPI 路由组  │  Agent 引擎核心(loop/工具/沙箱/预算/AgentRun)            │
  Tauri 客户端 ─► │  鉴权/设备令牌门  │  分层权限引擎(allow/deny/ask)  │  审计+快照/回滚         │
  (+未来 Cuu)    │  Provider 注册表(DeepSeek-via-Anthropic 首发)                                │
                 └───────┬───────────────────────┬───────────────────────────┬──────────────────┘
                         │                        │                           │
                   PostgreSQL(实体+Alembic)   Broker(Redis / PG LISTEN-NOTIFY)   沙箱工作目录
...
| **F3** | PostgreSQL + Alembic | 换库 + 真迁移体系 | 换 engine、删 SQLite PRAGMA、init Alembic、首迁移、类型审校(`utcnow`→`timestamptz`、Text-JSON→JSONB、String(32)→UUID) | F2 | 类型强转**静默出错**(naive datetime vs timestamptz) |
...
- **P0a 地基之地基(F1→F2→F3):** 仓库、实体、PG+Alembic。产出"能在 PG 上跑起来的空 daemon"。
...
2. **PG/Alembic:** 所有 schema 变更走 Alembic 迁移;新增可变实体带 `version`(乐观锁)与 `deleted_at`(软删除);JSON→JSONB;时间一律 `timestamptz`(消灭 naive `utcnow`)。
...
8. **事件 taxonomy(供 F5,对齐桌宠概念):** `agent.run.started`/`run.step`、`permission.ask`、`proposal.ready`、`confidence.assessed`、`escalation.created`、`knowledge.evidence.ready`、`sync.progress`/`sync.conflict`、`revision.requested`、`work.completed`(见 [`cuu-desktop-pet-concept.md` §9](../workhub/05-clients/cuu-desktop-pet-concept.md))。
9. **provider 单出口:** 所有 LLM 调用经 registry;低风险任务可路由廉价模型(NFR-05),每调用计量 token/成本。

## 7. 系统级影响(System-Wide Impact)

- **交互图:** daemon 启动 → lifespan(Alembic head 校验 → broker 连接 → 周期任务 leader 选举)→ 路由就绪。一次 AI 运行:`POST /workitem/{id}/run` → 入 AgentRun 队列 → worker 取出 → AgentLoop(工具→沙箱→快照→事件 `run:{id}`)→ 完成判定 → ConfidenceRecord → 生命周期里程碑 → 通知 `user:{id}` + 审计。
...
- [ ] 全部 schema 经 Alembic;`alembic upgrade head` 从空库可重建;无 `create_all`/运行时 ALTER。
...
- **测试策略:** §7 五场景为集成测试基线;安全敏感移植件须回归;Alembic 迁移须 up/down 可逆测试。
...
| F3 PG+Alembic | `p0-foundation/F03-postgres-alembic-plan.md` |
...
- **概念图:** [`05-clients/page-concepts.md`](../workhub/05-clients/page-concepts.md)、[`cuu-desktop-pet-concept.md`](../workhub/05-clients/cuu-desktop-pet-concept.md)(P0 仅取事件 taxonomy 与客户端改接边界;Cuu/页面属 P3/P4)。
