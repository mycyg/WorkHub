    ...
    async merge(input) {
      ...
      try {
        ...
        if (error instanceof ProposalRepositoryTaskPlanApprovalError) {
          throw new ProposalServiceError(409, "task_plan_approval_failed", "这份任务计划已经不能批准（可能已被取消），本次采纳没有生效，请刷新后再处理。");
        }
        // R9-BLOCK-7.154：人审修订写回前的图/预算校验失败，整笔合入已回滚。
        if (error instanceof TaskPlanItemGraphMismatch) {
          throw new ProposalServiceError(409, "task_plan_items_invalid", "计划修订里的子任务依赖或引用不合法，本次采纳没有生效，请修正后重试。");
        }
        if (error instanceof TaskPlanBudgetShareMismatch) {
          throw new ProposalServiceError(409, "task_plan_budget_share_invalid", `计划修订的预算份额合计是 ${error.totalSharePct}%，必须刚好 100%，本次采纳没有生效。`);
        }
        throw error;
      }
      if (!rows) {
