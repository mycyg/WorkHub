...
| QA 业务场景注入 | `apps/desktop-webview/src/cuu-qa-scenarios.ts` / `WORKHUB_CUU_QA_SCENARIO` / `WORKHUB_CUU_QA_LOCALE` | 允许真实 Tauri pet window 录 `launcher` / `clarify` / `approval` / `search` / `sync` / `done` / `offline`；R3.10 已补 `launcher` 双语 locale 注入与 WebView2 CDP 点击 |
