...
> **2026-06-09/10 R1 TS 切片**：`accepted_deliverable_changes` 已作为正式采纳账本落地；AgentRun-backed delivery 也已接入最小 `ProjectDriveItem/Version`，WorkItem page 与 AgentRun replay page 已能展示 accepted deliverables，并提供下载/文本预览与 restore。R1.9-R1.42 已补冲突卡、merge attempts/proposals、replay timeline、AI fusion、text/spec 正文直写、真实 current/incoming/base 文本、patch preview、无重叠 diff3、重叠 hunk metadata、结构化字段/子记录写回、目标 plan 选择、逐段 text hunk materialize、批量 keep/accept `bulk_action` 审计与 merge audit。仍未完成的是富预览、云对象存储 adapter、Replay hunk/bulk decision 用户可读回放与 React route 级逐行编辑。
...
- **R1.12-R1.42 当前候选与回放入口**:`merge_attempts` 下会同时写 `merge_proposals`。R1.13-R1.42 已把 candidate/recommended/chosen、AI fusion、text/spec 正文直写、真实三方文本、patch preview、text_diff3、structured dry-run、字段/子记录写回、目标 plan 选择、`text_hunk_overrides` 逐段 materialize 和 `bulk_action` 审计接入同一 merge/audit 链路。后续仍需 Replay hunk/bulk decision 用户可读回放与 React route 级逐行编辑。
...
>
> **当前 R1.9-R1.42 状态**：已先落“二选一枚举”作为可用纵切，并逐步接入 merge attempts/proposals、replay timeline、AI fusion、text/spec 正文直写、真实文本上下文、patch preview、无重叠 diff3、重叠 hunk metadata、结构化字段和子记录写回、目标 plan 选择、`text_hunk_overrides` 逐段 materialize，以及批量 keep/accept `bulk_action` 审计。LLM 未配置、失败、JSON 不合格、目标类型不支持时继续降级为两选一或 Markdown artifact；无重叠文本合并不再依赖 LLM；重叠文本可由用户逐段点选。尚未完成的是 Replay hunk/bulk decision 用户可读回放和 React route 级逐行编辑。
