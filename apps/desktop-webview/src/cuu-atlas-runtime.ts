...
export const desktopCuuAtlasBaseCss = [
  ...
  ".wh-cuu-atlas[data-loop=false] .wh-cuu-atlas-img-frame{animation-iteration-count:1}",
...
function renderDesktopCuuAtlasClipState(
  ...
  return {
    ...
    css: `${desktopCuuAtlasBaseCss}${clipImage ? buildImageFrameKeyframes(keyframes, clip) : buildAtlasKeyframes(keyframes, clip, scale, undefined)}`,
    html: `<div class="wh-cuu-atlas" data-cuu-atlas-state="${escapeHtml(clip.state)}" data-cuu-requested-state="${escapeHtml(state)}" data-fallback="${fallback ? "true" : "false"}" data-loop="${clip.loop ? "true" : "false"}" data-frame-count="${clip.frames.length}" data-cuu-image-mode="${clipImage ? "clip_sheet" : "atlas"}" data-cuu-render-mode="${clipImage ? "img_stack" : "background"}" data-cuu-static-fallback="${fallbackImagePath ? "true" : "false"}" aria-label="${escapeHtml(reducedMotionFallback)}" style="${escapeHtml(style)}">${fallbackImagePath ? `<img class="wh-cuu-atlas-static-fallback" src="${escapeHtml(fallbackImagePath)}" alt="" aria-hidden="true" draggable="false">` : ""}${clipImage ? renderImageFrameStack(keyframes, clip, reducedFrame, scale, clipImage, imagePath) : `<div class="wh-cuu-atlas-frame" data-frame-id="${escapeHtml(reducedFrame.id)}"></div>`}</div>`
  };
}
...
function renderImageFrameStack(
  ...
  return `<div class="wh-cuu-atlas-img-stack">${clip.frames.map((frame, index) => {
    ...
    return `<img class="wh-cuu-atlas-img-frame" src="${escapeHtml(imagePath)}" alt="" aria-hidden="true" draggable="false" data-frame-id="${escapeHtml(frame.id)}" data-reduced="${frame.id === reducedFrame.id ? "true" : "false"}" onload="this.dataset.loaded='true'" onerror="this.dataset.failed='true'" style="${escapeHtml(frameStyle)}">`;
  }).join("")}</div>`;
}
...
function buildImageFrameKeyframes(name: string, clip: CuuSpriteAtlasClip) {
  ...
  for (const [index, frame] of clip.frames.entries()) {
    ...
    const segments = [
      ...
      `${formatPercent(after)},100%{opacity:0}`
    ];
...
function documentRelativeAssetPath(value: string) {
  try {
    ...
    if (assetUrl.origin === documentUrl.origin) {
      const assetIndex = assetUrl.pathname.lastIndexOf("/assets/");
      if (assetIndex >= 0) {
        return `.${assetUrl.pathname.slice(assetIndex)}`;
      }
    }
  } catch {
    return value;
