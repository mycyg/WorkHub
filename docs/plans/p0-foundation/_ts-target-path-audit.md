...
| F09 Lifecycle/notifications | `lifecycle.py`, `notifications.py` | `apps/api/src/services/lifecycle.ts`, `apps/api/src/services/notifications.ts`, `packages/events/src/lifecycle.ts`, `packages/events/src/toAttentionItem.ts`, `packages/events/src/toCuuState.ts` | lifecycle service, notification service, event helpers | 新状态必须登记,不发 `all` 私有内容 |
