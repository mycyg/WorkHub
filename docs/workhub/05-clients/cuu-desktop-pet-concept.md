...
### 6.5 R3 Agent 出站入口（R3.1/R3.2/R3.3 已落）
...
R3.2 已补第一版 run stream 回流：启动成功后 `pet-surface.ts` 订阅 `AgentRunLiveVM.stream_href`，匹配 run 事件后重新拉 `GET /api/agent-runs/:id` 并刷新 Cuu card；`budget_exhausted`、403/401、offline/network 和 generic error 会变成 Cuu 轻卡。R3.3 已补 `SessionVM.question` 回退与 `nextQuestion()` 返回 question card，避免桌宠绕过澄清或只显示一句 message。该切片仍是 TS 合同和单元测试，不替代真实 Tauri pet window 截图/录屏。
...
| 验证 | `@workhub/desktop-webview` test/typecheck 已覆盖 launcher DOM、三段 API 调用、SessionVM 回退、nextQuestion card、run stream refresh、错误卡 |

R3.3 仍不能算 R3 完成：还缺真实 Tauri 点击截图、真实 daemon launcher-to-run smoke、刷新恢复，以及确认澄清后自动 `createWorkItem -> startAgentRun` 的最后一段 typed action。
...
| R3 | 恢复 Cuu 功能施工：新增自然语言 / option-first 出站指令入口，驱动真实 R1/R2 Agent 引擎 | R3.1 已落 option-first launcher；R3.2 已落 TS run stream 回流和错误态；R3.3 已落 SessionVM question 回退；R3.4 待补确认后启动、真实 Tauri 截图与 launcher-to-run smoke |
