...
| CUX-BONGO-006 | Bongo P1e-a 输入响应合同 | `packages/cuu/src/idle-scheduler.ts`、`packages/cuu/src/model-pack.ts`、`apps/desktop-webview/src/pet-surface.ts`、`apps/desktop-webview/src/pet-surface.test.ts` | CUX-BONGO-005 | **已落 P1e-a**：`cursor_near` interaction 首次进入附近区域立即 `look_at_mouse`；surface 输出 `data-pet-cursor-near` / `data-pet-hovered` / `data-pet-dragging`；默认 model pack 的 scale / opacity / pass-through 状态已对齐 supported；仍待 P1e-b 真实录屏 |
...
2. **Bongo P1d-b / P1e-b 动作与窗口体验**：P1d-a 已补缩放、透明度和点击穿透契约，P1e-a 已补 cursor-near 立即看鼠标和 pointer DOM attrs；下一步补真实 Tauri 设置截图、hover/near/tap/drag 录屏、hide-on-hover、hover 避让、多屏恢复、显示/隐藏快捷入口，并加大抱文件 / 审批敲桌 / 完成庆祝的动作幅度。
