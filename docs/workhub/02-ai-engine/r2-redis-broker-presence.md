...
| `all` topic 仍可能过宽 | R2.4 已改为 admin-only；资源 topic 默认 fail-closed；R2.5 已接 WorkItem/Proposal resolver；R2.6 已收口 Proposal/Approval REST 与 Page endpoint WorkItem gate | R4 逐步迁移公共事件到 workspace/org 或资源 topic |
...
2. **R3 Cuu Agent 入口**：在 R2 地基稳定后，让 Cuu 走同一 session/intake/workitem/agent-run/proposal API。
