...
| R1.43 | Replay hunk / bulk audit 已落：`text_hunk_decisions` 与 `bulk_action` 已渲染为可读回放；`targetPlanId`、plan label、plan stage/status 仍待后续 field_merge audit polish |
| R4 | 把 plan scope、text hunk 与 bulk audit 场景纳入 route visual QA 和真实 loading/error/forbidden 状态矩阵 |
