...
> **R1.12-R1.43 当前实现**：`merge_proposals` 表已落地。默认 409 写入 `keep_current` / `accept_incoming` 两个 deterministic candidate，`recommended_option_key="keep_current"`、`chosen_option_key=null`；用户显式采纳 incoming 的成功 merge 会写 `chosen_option_key="accept_incoming"`、`chosen_by_user_id`、`chosen_at`。R1.14-R1.28 已补 LLM `ai_fusion` candidate、候选选择、正式融合稿物化、真实文本上下文、patch preview、deterministic diff3、重叠 hunk metadata、`structured_record_patch` 与 dry-run gate。R1.29-R1.34 已补 WorkItem 标量字段写回、base/current/incoming 冲突检测、字段级审计与 `structured_field_overrides`。R1.31-R1.40 已补 `acceptance_items/task_items` 子记录写回、逐项 diff/editor、任务子记录目标 plan 选择和多冲突折叠区。R1.41 起 `POST /api/merge-proposals/{id}/apply` 可接收 `text_hunk_overrides`，按 `quality_gate.text_diff3.conflict_ranges[]` 逐段 materialize text/spec 最终文本，并在 `proposal.merged.detail_json` 写入 hunk 决策与 base/current/incoming/output sha。R1.42 起 `POST /api/proposals/{id}/merge` 可接收 `conflict_resolution.bulk_action`，在批量 keep/accept 的 conflict 与 merged 两条路径写 `proposal.bulk_action` 审计，并在成功 merge 的 `proposal.merged.detail_json` 带 `bulk_action` 摘要。R1.43 起 Replay 会把 hunk 决策和 bulk action 渲染成用户可读回放。`needs_review`、`status`、目标不匹配、blocked dry-run、非法 task item、stale 文本上下文和 hunk range mismatch 仍 fail-closed；React route 级逐行选择/编辑产品化仍未完成。
...
> **2026-06-09/10 R1 TS 切片**：`accepted_deliverable_changes` 已作为正式采纳账本落地；AgentRun-backed delivery 也已接入最小 `ProjectDriveItem/Version`，WorkItem page 与 AgentRun replay page 已能展示 accepted deliverables，并提供下载/文本预览与 restore。R1.9-R1.43 已补冲突卡、merge attempts/proposals、replay timeline、AI fusion、text/spec 正文直写、真实 current/incoming/base 文本、patch preview、无重叠 diff3、重叠 hunk metadata、结构化字段/子记录写回、目标 plan 选择、逐段 text hunk materialize、批量 keep/accept `bulk_action` 审计、merge audit 与 Replay hunk/bulk 用户可读回放。仍未完成的是富预览、云对象存储 adapter、React route 级逐行编辑。

### 5.4 DOC 文本三方合并

- 优先**三方文本合并**(base/ours/theirs 的行级 merge,类 `diff3`)。无重叠 hunk → 自动合并(`merge_strategy=field_merge`/文本变体)。
- 重叠 hunk → 不写带 `<<<<<<<` 标记的脏文件(那是黑话且不可用),而是**整块作为一个 `ConflictItem`** 交 AI 调解,AI 产出"融合两边意图"的候选文本(§6.3)。R1.41 起可用 `text_hunk_overrides` 逐段选择 current / incoming / AI fusion，并 materialize 为最终文本。
...
- **R1.12-R1.43 当前候选与回放入口**:`merge_attempts` 下会同时写 `merge_proposals`。R1.13-R1.43 已把 candidate/recommended/chosen、AI fusion、text/spec 正文直写、真实三方文本、patch preview、text_diff3、structured dry-run、字段/子记录写回、目标 plan 选择、`text_hunk_overrides` 逐段 materialize、`bulk_action` 审计和用户可读 Replay 回放接入同一 merge/audit 链路。后续仍需 React route 级逐行编辑。
...
>
> **当前 R1.9-R1.43 状态**：已先落“二选一枚举”作为可用纵切，并逐步接入 merge attempts/proposals、replay timeline、AI fusion、text/spec 正文直写、真实文本上下文、patch preview、无重叠 diff3、重叠 hunk metadata、结构化字段和子记录写回、目标 plan 选择、`text_hunk_overrides` 逐段 materialize、批量 keep/accept `bulk_action` 审计，以及 Replay hunk/bulk 用户可读回放。LLM 未配置、失败、JSON 不合格、目标类型不支持时继续降级为两选一或 Markdown artifact；无重叠文本合并不再依赖 LLM；重叠文本可由用户逐段点选。尚未完成的是 React route 级逐行编辑。
...
| `POST /merge-proposals/{mpid}/apply` | 对 `ai_fusion` 物化融合稿；text/spec 写 accepted ledger / Drive version / audit，`text_hunk_overrides` 可逐段生成最终文本；ready + executable `structured_record` 写 WorkItem 标量字段、`acceptance_items`、最新 dispatch plan `task_items` 和 `field_merge` audit；未选择时把本次点击写为 `chosen_*`，已选其它候选时拒绝 | 「采用 AI 融合稿」 |
