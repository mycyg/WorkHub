...
- hover/tap/drag 录屏通过。
- 托盘可恢复 pass-through 或隐藏状态；R3.20b 已用 Windows UI Automation 证明真实 OS tray icon/menu item 点击恢复，R3.23 已用 GNOME StatusNotifier/AppIndicator DBusMenu Event 证明 Linux 主路径恢复，并在 macOS 26.3.1 真机以 menu bar status item 完成 zh-CN/en-US 原生菜单点击 smoke。
...
R3.1-R3.23 已落 TS webview 层、route-stack、boot click harness、第一份真实 Tauri launcher 证据、真实本机 HTTP dev-server launcher-to-run smoke、真实 Tauri run-stream completion capture、真实 Tauri run-failure terminal capture、真实 Tauri 401/403/offline error-state capture、pet webview boot restore、launcher spec metadata、真实 Tauri reload restore capture、真实 Tauri 业务状态矩阵 capture、真实 Tauri settings/menu matrix、pass-through recovery 主窗 settings 截图门、tray handler recovery 证据、右键 hover -> main settings 同步截图、Windows 物理 OS 托盘菜单项恢复证据、run failure/run stream 卡片文本 overflow 自动门、R3.22 failed/permission/offline/generic/main notice 文本与 frame hardgate，以及 R3.23 Linux GNOME StatusNotifier/AppIndicator 与 macOS menu bar 菜单动作，详见 [`cuu-r3-agent-entry.md`](./cuu-r3-agent-entry.md)：
...
- R3.23 已证明 Linux GNOME StatusNotifier/AppIndicator 主路径：远端 `ayatana-indicator-application.service` 暴露 `org.kde.StatusNotifierWatcher`，WorkHub StatusNotifier item、DBusMenu path、layout labels、Event action 与窗口 Map State 效果全部通过；证据在 `docs/workhub/05-clients/assets/audit/2026-06-11-r3-23-appindicator-statusnotifier-busctl/`。
- R3.23 已证明 macOS menu bar 主路径：`scripts/qa/cuu-tauri-macos-menu-smoke.sh` 在 macOS 26.3.1 上用 `osascript` + Accessibility 打开 `WorkHub menu_bar=2 description=status menu`，逐项点击 `restore/open-settings/open-inbox/toggle/show/hide/quit`，并保留 zh-CN/en-US 截图与日志；证据在 `docs/workhub/05-clients/assets/audit/2026-06-11-r3-23-macos-menu/`。

Rust 边界保持不变：Rust 不调用业务 API、不绕过 auth、不拥有 Agent 状态机。R4.21 已把 desktop-webview 主窗的 action payload、notice、locale、line editor helper 接到 `@workhub/web-runtime`，避免继续复制 Web browser dispatcher；下一步 Rust/Tauri 只作为回归门支撑 R4.22 Proposal mutation editor 迁移、主窗产品化与后续跨平台打包验收。
...
R3.22 当时不声明真实 Linux panel appindicator 或 macOS menu bar 已通过；R3.23 已关闭 Linux GNOME StatusNotifier/AppIndicator 主路径，并用 macOS 26.3.1 真机关闭 menu bar status item 主路径。
...
### 7.7 R4.21 shared web runtime boundary

2026-06-11 已把 desktop-webview 的浏览器分发器漂移收回一段：

- `apps/desktop-webview/src/browser.ts` 现在从 `@workhub/web-runtime` 复用 locale persistence、structured route notice、proposal/approval action payload materializer、merge conflict extractor、intake selected option payload 更新和 route line editor binding。
- 桌面主窗 proposal/approval action 的 fail-closed 提示与 Web 使用同一 notice VM，缺 reason、缺 option、缺 payload 时不再依赖旧拷贝。
- locale persistence 失败不再静默 reload，而是显示 `locale_persistence_failed` 结构化 notice。
- R4.21 不改变 pet window、Rust tray/menu、Cuu card 状态机或本地 Agent 能力；Cuu 仍只在独立 pet window。
- 限制：desktop-webview 仍未完整接入 Web app-level Page VM route loader，后续 R4/R5 主窗产品化继续沿 shared runtime 扩展。
...
| R3.23 Linux StatusNotifier smoke | `WORKHUB_LINUX_SMOKE_REQUIRE_REAL_DE=1 WORKHUB_LINUX_MENU_DRIVER=status-notifier bash scripts/qa/cuu-tauri-linux-smoke.sh`；证据见 `assets/audit/2026-06-11-r3-23-appindicator-statusnotifier-busctl/`，要求 WorkHub item、DBusMenu Event、window Map State 与 `bubble_gap_to_live2d_px >= 8` 同时通过 |
| R3.23 macOS menu bar smoke | `WORKHUB_MACOS_MENU_SMOKE_LOCALE=zh-CN/en-US bash scripts/qa/cuu-tauri-macos-menu-smoke.sh`；证据见 `assets/audit/2026-06-11-r3-23-macos-menu/`，要求 Screen Recording + Accessibility、`WorkHub menu_bar=2 description=status menu`、七个菜单 action 点击日志与关键截图同时通过 |
| Path hygiene | `git diff --name-only` 不含 `reference/` / `references/` |

真实视觉证据必须写入审计文档，不能只用测试命令替代。R3.10 的真实 Tauri launcher 验收已经保留 `cuu-motion-contact-sheet.png`、`cuu-motion-printwindow.gif`、`cuu-motion-printwindow.mp4`、`cuu-tauri-dom-report.json` 与 `motion-diff-report.json`。R3.12 的 zh-CN/en-US run-stream capture、R3.13.1 的 zh-CN/en-US run-failure capture、R3.13.2 的 zh-CN/en-US 401/403/offline capture、R3.15 的 reload session/active/terminal capture、R3.16 的 `clarify/search/sync/done/offline/approval` capture、R3.17 的 settings/menu capture、R3.18 的 pass-through recovery capture、R3.19 的 tray handler recovery capture、R3.20a 的 hover sync capture、R3.21 的 Linux Xvfb/openbox smoke、R3.22 的 text/frame smoke、R3.23 的 GNOME StatusNotifier/AppIndicator smoke 与 macOS menu bar smoke 同样保留截图、DOM report 或脚本 stdout 证据；API/Tauri stdout/stderr/frames 作为本地调试中间件生成，不进入 Git 跟踪证据。业务 card 截图还必须人工查看最终帧，并要求 `right_edge_clip_gate.passed=true` 或 R3.22+ `spatial_safety` 等价通过，确认标题、状态、actions、Run progress/Budget、evidence refs 或 permission/offline 文案不被窗口边界裁切。菜单截图还必须满足 `settings_menu_layout_gate.passed=true` 或原生系统菜单日志/截图等价通过，确认菜单动作来自 OS menu/tray surface 而不是 Tauri command fallback；主窗 settings 恢复或同步截图必须滚动到桌面客户端设置区，满足 `layout_gate.overflow.offenders=[]`、无 Cuu 本体、无模型预览，并直接显示相关控件状态。Linux GNOME StatusNotifier 这类远端黑屏截图不作为视觉验收，只以 DBusMenu、window state 与 DOM report 为准；macOS 真机截图可作为视觉验收，但必须同时有 Accessibility menu click 日志。
...
| R3 Agent launcher / run-stream 真实 Tauri capture | 已补真实 `pet` window `launcher/en-US` capture、zh-CN/en-US run-stream completion capture、zh-CN/en-US run-failure terminal capture、zh-CN/en-US 401/403/offline capture、zh-CN/en-US reload session/active/terminal capture、zh-CN/en-US 混合业务状态矩阵 capture、settings matrix、右键菜单 capture、pass-through recovery 主窗截图、tray handler recovery、右键 hover -> main settings 截图、Windows 物理 OS 托盘恢复、run card 文本 overflow gate、Linux Xvfb/openbox smoke、R3.22 failed/generic card frame hardgate、R3.23 GNOME StatusNotifier/AppIndicator menu action 与 macOS menu bar action；TS runtime、DOM render、run stream/error card tests、dev-server launcher-to-run smoke、run-stream smoke、run-failure smoke、error-fault smoke、R3.13.3 session/run restore 单测、R3.14 chip metadata spec readback、R3.15 reload restore smoke、R3.16 business matrix evidence、R3.17 settings/menu evidence、R3.18 pass-through recovery evidence、R3.19 tray handler recovery evidence、R3.20a hover sync evidence、R3.20b physical tray evidence、R3.21 Linux smoke、R3.22 text/frame evidence、R3.23 StatusNotifier evidence 与 R3.23 macOS evidence 已落 | R4.15 已补 Web Settings / locale / device boundary；R4.16 已补 React route tree / hydration boundary；R4.17 已补 Home / Settings React-compatible first migration；R4.18 已补 Cost / Replay React-compatible expansion；R4.19-pre 已补真 React mount spike；R4.19 已补 Proposal split；R4.20 已补 dataflow foundation；R4.21 已补 shared web runtime；R4.22 已补 Proposal structured field scalar visible React mutation editor；R4.23 已补 Proposal line editor visible React island；下一步进入 R4.24 Web runtime finalization |
...
| full hide/pass-through 恢复 | 主窗 `/settings` 与托盘 `restore-pet-interaction` 源码恢复门已落；R3.18 已补主窗 `/settings` 真恢复截图，R3.19 已补同 handler tray recovery 截图，R3.20b 已补 Windows 物理 OS 托盘菜单点击证据，R3.23 已补 Linux GNOME StatusNotifier 与 macOS menu bar `restore-pet-interaction`，均确认菜单动作可见且恢复提示/窗口状态可复查 | 后续作为跨平台回归门，不再是当前阻塞 |
| Linux/macOS capture | Linux Xvfb/openbox window/frame smoke、R3.22 text/frame hardgate、R3.23 GNOME StatusNotifier/AppIndicator 菜单动作与 macOS menu bar 中英 smoke 已补 | 下一步补 R4 主窗完整产品化 capture |
| 商用授权 | 未确认 | 联系授权或原创替换 |
...
| R3 恢复 | Cuu 自然语言 / option-first 出站入口 | R3.1-R3.23 已补 option-first launcher、真实 run stream/failure/error/reload/business/settings/tray/menu capture、Windows 物理托盘、Linux GNOME StatusNotifier/AppIndicator 与 macOS menu bar 主路径；R4.15-R4.23 已补 Web Settings/device boundary、route hydration、React mount spike、Proposal split、dataflow foundation、shared web runtime、第一段 visible React mutation editor 与 line editor visible React island；下一步进入 R4.24 Web runtime finalization |
