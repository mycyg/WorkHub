...
> **R1.11 已落最小持久化子集**：`packages/db/src/schema/core.ts` 已新增 `merge_attempts`，`ProposalRepository.merge()` 会在默认 merge 被冲突 gate 挡住时写 `result="conflict"`，在显式 `accept_incoming_target_keys` 后成功采纳时写 `result="merged"`。当前表先记录 deterministic 两选一所需的 `conflicts_json`、`accepted_target_keys`、`target_keys`、`conflict_count` 与 `merge_snapshot_id`；`merge_proposal_id`/LLM 融合候选仍属于后续 `MergeProposal` 切片。
...
>
> **当前 R1.9-R1.11 降级态**：已先落“二选一枚举”作为可用纵切：`keep_current`=保留正式版并回到变更申请；`accept_incoming`=用户明确采纳这次版本，POST body 固定为 `{conflict_resolution:{accept_incoming_target_keys:[target_key]}}`。Web/Desktop/Cuu 都已用 option-first 卡片承载这两个动作，保证小白可以点选，不需要输入文字；R1.11 已把冲突尝试与用户选择落到 `merge_attempts` 与 `proposal.merged` audit detail。后续再把 LLM 融合候选作为第三类 option 加入同一 contract，并用 `MergeProposal` 表承载候选 rationale 与 recommended/chosen option。
