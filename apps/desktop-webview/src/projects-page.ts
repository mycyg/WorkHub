...
// 纯渲染:把 ProjectListVM 渲染成一组玻璃项目卡(名称 + 进行中工作项计数 + 负责人 + 描述),
// 整卡可点(带 data-project-id/-slug/-name)以便 browser.ts 下钻到该项目的文件(client.pages.drive)。
...
function renderCard(project: ProjectListVM["projects"][number], zh: boolean): string {
  ...
  const desc = project.description ? `<p class="wh-proj-desc">${escapeHtml(project.description)}</p>` : "";
  return `<a class="wh-proj-card gl-press" data-proj-card="${escapeHtml(project.id)}" data-project-id="${escapeHtml(project.id)}" data-project-slug="${escapeHtml(project.slug)}" data-project-name="${escapeHtml(project.name)}">
    <div class="wh-proj-cardtop">
      <h3 class="wh-proj-name">${escapeHtml(project.name)}</h3>
      <span class="wh-proj-count${hasOpen ? " wh-proj-count--active" : ""}">${escapeHtml(countLabel(project.open_work_item_count, zh))}</span>
    </div>
    ${desc}
    <div class="wh-proj-meta">
...
export const projectsPageCss = [
  ...
  ".wh-proj-title{margin:0;font-size:20px;font-weight:900;color:#2c2746}",
  ".wh-proj-chips{display:flex;gap:8px;flex-wrap:wrap}",
  ".wh-proj-chip{font:700 12px/1 'Noto Sans SC',sans-serif;color:#6b6488;background:rgba(255,255,255,.55);border:1px solid rgba(255,255,255,.7);border-radius:11px;padding:7px 11px}.wh-proj-chip b{color:#7c83ff;font-size:14px;margin-right:4px}",
  ".wh-proj-chip--active b{color:#1faf86}",
  ".wh-proj-grid{display:grid;grid-template-columns:repeat(auto-fill,minmax(240px,1fr));gap:14px}",
  ".wh-proj-card{display:flex;flex-direction:column;gap:10px;padding:18px 18px 16px;border-radius:20px;text-decoration:none;cursor:pointer;background:rgba(255,255,255,.58);backdrop-filter:blur(32px) saturate(180%);-webkit-backdrop-filter:blur(32px) saturate(180%);border:1px solid rgba(255,255,255,.78);box-shadow:0 24px 54px -30px rgba(70,54,140,.55),inset 0 1px 0 rgba(255,255,255,.78);transition:transform .15s ease,box-shadow .15s ease}",
  ".wh-proj-card:hover{transform:translateY(-2px);box-shadow:0 30px 62px -28px rgba(70,54,140,.62),inset 0 1px 0 rgba(255,255,255,.85)}",
  ".wh-proj-cardtop{display:flex;align-items:flex-start;justify-content:space-between;gap:10px}",
  ".wh-proj-name{margin:0;font-size:16.5px;font-weight:800;color:#2c2746;line-height:1.35;overflow-wrap:anywhere}",
  ".wh-proj-count{flex:0 0 auto;font:800 11px/1 'M PLUS Rounded 1c','Noto Sans SC',sans-serif;color:#8b84ad;background:rgba(255,255,255,.6);border-radius:8px;padding:5px 8px;white-space:nowrap}",
  ".wh-proj-count--active{color:#1faf86;background:rgba(52,199,154,.16)}",
  ".wh-proj-desc{margin:0;font:500 13px/1.55 'Noto Sans SC',sans-serif;color:#5d567e;display:-webkit-box;-webkit-line-clamp:2;-webkit-box-orient:vertical;overflow:hidden}",
  ".wh-proj-meta{display:flex;align-items:center;justify-content:space-between;gap:8px;margin-top:auto;padding-top:4px}",
  ".wh-proj-owner{font:700 12px/1.4 'Noto Sans SC',sans-serif;color:#6b6488}",
  ".wh-proj-slug{font:700 11px/1 'M PLUS Rounded 1c',monospace;color:#a39ec0;background:rgba(124,131,255,.1);border-radius:6px;padding:4px 7px}",
  ...
  ".wh-proj-empty-title{margin:16px 0 0;font-size:20px;font-weight:900;color:#2c2746}",
  ".wh-proj-empty-sub{margin:10px 0 0;font-size:13.5px;line-height:1.6;color:#6b6488}"
].join("");
