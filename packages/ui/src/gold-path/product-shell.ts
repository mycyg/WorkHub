...
import { productShellCopy, type ProductShellCopyKey } from "./product-shell-copy.js";
...
const productShellCss = [
  ...
  // R23 P2（SA-08）：常驻横幅——amber 而非 danger（README 原话「提示性的，不拦你发送」，不是错误态）；
  // 居中限宽同 masthead，小屏收起两侧留白；纯文本+一个「去设置」链接，不带关闭按钮——「常驻」是任务
  // 本身的要求（未配置这件事在配好之前始终成立，不是一次性通知，不该有「关掉后就再也看不见」的状态）。
  ".wh-product-ai-banner{max-width:1120px;margin:10px auto 0;display:flex;align-items:center;justify-content:center;gap:10px;flex-wrap:wrap;border:1px solid rgba(224,137,42,.35);background:var(--wh-product-amber-light);border-radius:var(--wh-radius-md);padding:10px 16px;color:var(--wh-product-ink);font-size:13px;line-height:1.5;text-align:center}.wh-product-ai-banner[hidden]{display:none}.wh-product-ai-banner strong{font-weight:850}.wh-product-ai-banner a{color:var(--wh-product-blue);font-weight:850;text-decoration:none;white-space:nowrap}.wh-product-ai-banner a:hover{text-decoration:underline}@media (max-width:780px){.wh-product-ai-banner{margin:8px 12px 0}}",
...
function pageMetrics(page: WebProductShellPage, rendered: WebProductShellSurface, locale: WorkHubLocale): WebProductMetric[] {
  ...
  if (page.key === "workitem") {
    ...
    return [
      { id: "trace", label: productT(locale, "metric.steps"), value: String(workitem.agent_trace_preview.length) },
...
export function renderWebProductShell(
  ...
  // R23 P2（SA-08）：常驻「AI 未配置」提示条的静态挂载点——SSR 阶段恒 hidden 且空 innerHTML。
  // apiKeyConfigured 不是任何单个路由页面 VM 的字段（它是部署级事实，来自 GET /api/health 的
  // ai_provider_configured，与设置页 llm_runtime.api_key_configured 同一来源 settings.llm.apiKey），
  // 给它整一条“页面 VM 字段”会在每个路由 loader 里重复线一遍；改由 apps/web/src/browser.ts 的
  // bindAiReadinessNotices 在每次就绪路由渲染后水合填充（同 bindAvatarTiles 的先例：不按路由 key
  // 过滤，取一次缓存结果即可复用到所有导航，取数失败保持 hidden，fail-soft 不伪造「已配置」）。
  return {
    routeMap,
    ...
    html: `<div class="wh-product-root" data-wh-surface="${rendered.surface}" data-r4-product-shell="true" data-r4-product-route-key="${escapeHtml(activeKey)}" data-r4-product-link-mode="path">
      ...
      <div class="wh-product-ai-banner" data-wh-ai-banner hidden role="status" aria-live="polite"></div>
