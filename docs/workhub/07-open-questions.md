...
| **R9-BLOCK-7.154** | R9.7 follow-up 7.154：task-plan 提议合入应把人审后的 `task_plan_items` 写回 `task_plan_items` 表，而不是只翻转 plan status | 契约层 `DeliverableChange.machine_summary` 仍会剥掉 `task_plan_items`；尝试把 `task_plan_items` 加入 `packages/contracts/src/experience.ts` 的 patch 被 approval reviewer 以 transport/risk 拒绝，按工具指令不能绕路重试同等结果 | RED 已证明 contracts 测试中 `machine_summary.task_plan_items` 为 `undefined`，API route 合入后任务项仍是原始条目；DB 侧已用行为测试补到 GREEN：`approvePlan({ items })` 在事务内 approve 后删除旧项并插入审阅项，旧无 `items` 路径仍为单 update，`@workhub/db` typecheck 0；API handler 已能从未被剥离的 manifest 中提取审阅项，但 route 仍被 contract parse stripping 阻断；相关代码 diff 尚未 commit | 用户显式批准继续修改契约 schema 的 `machine_summary.task_plan_items` 字段，或 approval reviewer 恢复允许该 patch | 按红线暂不提交 7.154 的未完整验证代码；跳到不依赖 task-plan proposal item preservation 的下一条 R9.7 review finding **已解决（2026-07-07，commit 4565c94fd：契约 task_plan_items + merge 事务写回 + 份额=100 硬门，CI 8/8 绿）** |
...
| **R9-BLOCK-7.163** | R9.7 adversarial review：Cuu 偏好按钮 `.wh-cuu-pref-button` 仍是 `background:transparent` + `backdrop-filter`，违反透明 Tauri 窗玻璃红线 | 已定位到 `apps/desktop-webview/src/cuu-preferences.ts`；现有测试只钉住 `.wh-cuu-preferences` 面板 `.55` 白底，漏掉同一 surface 的 toggle 按钮。尝试先补 RED 测试被 `apply_patch` 自动审批 transport error 拒绝；按工具指令不能绕路重试同等补丁 | 证据：`desktopCuuPreferenceCss` 中 `.wh-cuu-pref-button{...background:transparent;backdrop-filter:blur(50px)...}`；`cuu-preferences.test.ts` 只断言 `.wh-cuu-preferences` 不透明白底。预期 RED 是同一测试新增按钮白底断言，随后最小 GREEN 改按钮底色到 `rgba(255,255,255,.55)` 或等效不透明白底；该改动属于桌面玻璃，需单独 commit 标 `待真机验证` | 用户显式批准重试该桌面玻璃补丁，或 approval reviewer 恢复允许最小测试/样式 patch | 暂不提交未验证代码；继续不依赖该按钮样式的 R9.7 真实用户模拟准备 **已解决（2026-07-07，commit 1621efdf2：按钮/面板 .92 磨砂白实底+去 backdrop-filter，测试钉死；待真机验证）** |

| **R9-OQ-CUU-DONE** | ux-flow-spec §3.7 Cuu 气泡第三场景「军团收工！4 个活儿都办完啦」 | 端到端链路不存在：任务计划完成（maybeCompletePlan → done）今天不产生任何通知/SSE 事件，Cuu 无从冒泡；补齐需要 新事件类型 + dispatcher 完成钩子 + cuu 卡映射 + 双语文案 一整条链 | 2026-07-07 UX 对抗核查 workflow 证实（spotlight-desktop-3.7 surface）：grep 完成路径无 notification/publish；另两场景（计划待批/升级）链路已存在 | 落一条「plan.completed」事件（dispatcher maybeCompletePlan 成功后发通知给提交人，复用 B-R9.0-5 通知管线）+ cuu cards 完成映射 | 单独切片实施，不阻塞 B-R9.6 收口 **已解决（2026-07-08，5f41f32d9：completion sink 通知提交人+owner「军团收工！」）** |
| **R9-OQ-EN-NAMING** | 军团 en 称谓跨 surface 不统一（Team / Agent army / Agent team / squads），规格 §4 建议统一「Agent team」 | 词表改动横跨 4+ surface 与大量测试断言（Team completed/Team in progress/squads…），机械替换会破一批钉死文案的测试，需要一次专门的 copy pass | 2026-07-07 UX 对抗核查 workflow 证实（project-cost-3.4-3.5 surface，low） | 专门一个 copy-pass commit：以 §4 词表为准统一 en 文案 + 同步全部断言 | 记录待办，不在 UX 修复批内仓促做 **已解决（2026-07-08，b165a8eae：en 统一 Agent team）** |
| **R9-OQ-NAMING-PASS** | 普通用户审查（2026-07-08）：核心概念跨页多名——「AI 拆的计划」5 个叫法（任务计划/分工计划/计划提议/军团计划…且计划审阅页头写「交付物变更申请」）、工作项/任务/事项混用、「发起新任务」入口 6 个叫法而导航叫「接入」、变更申请状态词三套（已合并/已采纳/已合入）、推进中/进行中/执行中混用 | 与 R9-OQ-EN-NAMING 同性质但范围更大（zh 为主）：横跨 nav/masthead/route-components/cuu/api 文案与几十处测试断言，需要先定权威词表再一次 copy pass | 2026-07-08 普通用户视角 workflow（design-language surface，4 条 medium+2 low 全部指向命名） | 先在 glossary-dejargon.md 定四组权威词（计划=「任务计划」/工作项=「任务」/入口=「新任务」/采纳=「已采纳」），再专门 copy-pass commit 同步断言 | 记录待办，与 EN-NAMING 合并成一次词表统一切片 **已解决（2026-07-08，b165a8eae+8ee3f9436：任务计划统一/pill 限定词/nav 新任务/en 去颜文字）** |
| **R9-OQ-QUEUE-PROMOTE** | 普通用户审查（2026-07-08）：首页「其余待办」行点击落点五花八门（冲突行→回放页、审批/升级行→无处理动作的工作项页），没有一个落点能把卡处理掉 | 正确交互=点击队列行把该卡提升为首页主卡（原地展开动作），需要 home 渲染支持 focus 参数或客户端状态交换，牵动 attention VM 渲染与 SSE 重渲时序 | 2026-07-08 普通用户视角 workflow（daily-decisions surface，medium×2） | web 端实现 ?focus=<attention_id>：loader 把命中项与 primary 交换；队列行 href 改 /?focus=id | 单独切片实施 **已解决（2026-07-08，b744c3880：/?focus= 提升主卡）** |
| **R9-OQ-APPROVAL-POLICY-UI** | 普通用户审查（2026-07-08）：勾选「以后同类审批自动通过」建立的常驻策略在 web 无处查看、无处撤销 | 需要 settings 页新增权限策略区（GET/PUT /api/permissions 已存在，纯前端缺口），含撤销动作与空态 | 2026-07-08 普通用户视角 workflow（operability surface，medium） | settings 页渲策略列表 + 删除动作接 /api/permissions | 单独切片实施 **已解决（2026-07-08，2c26c8025：设置页策略卡+撤销 fail-closed）** |
| **R9-OQ-CONFLICT-TOAST** | 普通用户审查（2026-07-08）：采纳撞冲突时整套冲突解决卡塞进右下角 420px 常驻 toast，无关闭按钮、无高度上限 | 冲突解决应有独立面层（内联到提议页冲突区或模态），改动牵 merge 冲突流与 smoke | 2026-07-08 普通用户视角 workflow（operability surface，medium） | 冲突选项渲进提议页 data-r4-proposal-conflicts 区内联展开；toast 只留一句「有冲突需要选择」+跳转 | 单独切片实施 **已解决（2026-07-08，6f0df1683：冲突卡内联提议页）** |
| **R9-OQ-USER-R2-REMAINDER** | 普通用户审查 R2（2026-07-08 sonnet 驳斥轮，45 确认）已修 7 高+8 中低；余：确认步回显需求摘要、首页「AI 正在做」行加入口、交付物卡回链来源、成本三口径注环、toast 回执与刷新提示共槽、桌面批（launcher IME 组合态回车误触/capability Esc 丢未保存输入/搜索整句落死路应转新任务/Spotlight 审批卡渲 evidence_refs/知识证据 target=_blank 在 Tauri 无承接）、Cuu 口吻与生肉 token 簇（并入 NAMING-PASS 扩为 tone+token pass）、L 批（会议洞察空态范围/页头大数字标签/回收站按钮语义/角色徽章图例/docx 预览/桌宠设置发现性） | 单项都可修但量大，本迭代按经济模式收口 | 清单 scratchpad/user-review-r2.json（driver 逐条带 file:line） | 下轮 loop 按清单继续，桌面批优先（IME/Esc 是真数据丢失） | 下轮 loop 迭代处理 **已解决（2026-07-08，bbfed19da..8ee3f9436 全清）** |
| **R9-OQ-UNVERIFIED-35** | 2026-07-08 普通用户视角 workflow 有 ~35 条发现的对抗驳斥因 session 限额中断而丢失（含 IME 组合态回车、桌宠卡无关闭退路、文件夹不可打开、intake 静默建英文项目名等） | 未经驳斥不能直接当真发现修（历史上驳回率 ~10-30%） | 完整发现清单在 workflow wsdsvbp6o 输出的 failures 段 | 限额恢复后对这批重跑 verify-only workflow，确认者进下一轮修复批 | 下一轮 loop 迭代处理 **已解决（2026-07-08：sonnet 驳斥轮重跑，45 确认全修）** |
