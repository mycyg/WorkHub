...
>
> **2026-06-10 R1.40 修正**：任务子记录多计划/目标 plan 选择 UI 已落最小切片。`task_items` 写回支持 `task_plan_scope.target_plan_id`，Proposal 可渲染目标 plan 按钮，DB 在多 plan 无 scope 时 fail closed。详见 [`r1-task-plan-scope-ui.md`](./r1-task-plan-scope-ui.md)。
>
...
>
> **2026-06-10 R3.13.1 修正**：Cuu 真实 Tauri `pet` window 已补 zh-CN / en-US run-failure terminal capture。failed AgentRun 会通过 SSE connected + REST fallback refresh 进入 `worried` trace card，并保留 `view_replay`；同轮修复 trace card 文本/预算区超框，最终帧已确认中英长文本与 Budget 均在卡片边界内。详见 [`cuu-r3-agent-entry.md`](./cuu-r3-agent-entry.md)。
>
> **2026-06-10 R3.13.2 修正**：Cuu 真实 Tauri `pet` window 已补 zh-CN / en-US 401、403 与 stream offline capture。权限错误进入 `permission/worried` 轻卡，网络/服务不可用进入 `offline` card，六个 capture 均通过 `right_edge_clip_gate`，最终帧右边缘亮色像素为 0。详见 [`cuu-r3-agent-entry.md`](./cuu-r3-agent-entry.md)。
>
> **2026-06-10 R3.13.3 修正**：Cuu `pet` webview boot 已补 session/run 恢复。刷新后可恢复当前 session question card、active AgentRun card 或 terminal AgentRun card；AgentRun 恢复通过 typed API `GET /api/agent-runs/:id` 重建，不让 Rust shell 拥有业务状态机。详见 [`cuu-r3-agent-entry.md`](./cuu-r3-agent-entry.md)。
>
...
>
> **2026-06-10 R1.44 修正**：Proposal route line editor 已落最小产品化切片。文本冲突可在严肃 Proposal 页按文件 tab 切换、搜索长 patch 行、逐段点选 current / incoming / AI fusion，并提交覆盖全部 hunk 的完整 `text_hunk_overrides` payload；Web 与 desktop browser runtime 会更新选择状态和 apply payload。详见 [`r1-route-line-editor.md`](./r1-route-line-editor.md)。
>
> **2026-06-10 R3.20b 修正**：Windows 物理 OS 托盘菜单点击恢复已落真实 Tauri/UIA 证据。`pass-through-recovery-tray-physical` 右键系统托盘 `WorkHub - Cuu is ready`、左键原生菜单 `Restore Cuu interaction`，且 `command_fallback_used=false`；同轮把 run failure / run stream 卡片文本超框纳入 `pet_card_text_overflow_gate`。详见 [`cuu-r3-agent-entry.md`](./cuu-r3-agent-entry.md)。
...
| Desktop 主窗 | Tauri webview 加载、surface 分流、Rust bridge、`/settings` pet 恢复面板、pet settings event bridge、右键 hover -> main settings 同步截图、Windows 物理托盘恢复后 main settings 中英截图 | 安装包、设备令牌门、生产更新、跨平台 smoke |
| Rust shell | 窗口、SSE、通知、deep-link、pet geometry、cursor sample、托盘 `restore-pet-interaction` 源码门、command-backed handler 恢复证据、Windows 物理 OS tray icon/menu item 点击恢复证据 | 私有 SSE 重连、动态托盘状态、本地同步、跨平台截图/权限策略 |
| Cuu 桌宠 | 独立 pet window、黑猫/白猫 Live2D registry、偏好二选项、QA 合同、概念图已同步真实模型帧、pass-through 源码恢复门、P1.6 behavior manifest 源码合同、P1.7 业务录屏入口与黑猫 approval smoke、R3.10 真实 launcher/en-US capture、R3.12 真实 zh-CN/en-US run-stream completion capture、R3.13.1 真实 zh-CN/en-US run-failure terminal capture、R3.13.2 真实 zh-CN/en-US 401/403/offline capture、R3.13.3 webview boot session/run restore、R3.15 真实 reload capture、R3.17 settings/menu matrix、R3.18 主窗 pass-through 恢复、R3.19 tray handler 恢复、R3.20a 右键 hover -> 主窗 settings 截图、R3.20b 物理托盘恢复和 run card 文本 overflow 自动门 | 长期性能、授权或原创替换 |
...
| Cuu pet surface | `apps/desktop-webview/src/pet-surface.ts` | 独立 pet window，Live2D cat + 轻气泡；idle tick patch behavior attrs，不重建 iframe；R3.13.3 已补 session/run card boot 恢复；R3.19 已补 menu/tray settings event bridge 与 transient bubble 收起 |
| Cuu preferences | `apps/desktop-webview/src/cuu-preferences.ts` | 模型选择只显示黑猫/白猫；desktop pet settings payload 支持主窗/pet/托盘同步 |
...
| Cuu 是会动的小猫桌宠 | 黑/白 Live2D registry 已落，概念图已同步真实模型帧，P1.6 behavior manifest 源码合同已落，P1.7 可录真实业务场景且已有黑猫 approval smoke，R3.12 已有真实 Tauri run-stream completion 中英双语证据，R3.13.1 已有真实 Tauri run-failure 中英双语证据且 trace card 文本/预算区无超框，R3.13.2 已有真实 Tauri 401/403/offline 中英双语证据且右缘无裁切，R3.13.3 已有 webview boot session/run 恢复单测，R3.15 已有真实 reload capture，R3.17 已有 settings/menu matrix，R3.18 已有 `/settings` pass-through 恢复证据，R3.19 已有 tray handler 恢复与中英短提示文本边界证据，R3.20a 已有右键菜单 hover -> 主窗 settings 中英截图，R3.20b 已有物理 OS 托盘点击恢复与 run card 文本 overflow 自动门 | 长期性能 |
| Cuu 不在 Web/主窗里 | 当前文档和代码收束中 | 截图审查确认无主窗 Cuu 本体 |
| Rust 客户端哲学是轻、气泡、少打扰 | Tauri shell、托盘 settings、pass-through 恢复源码门、tray handler command-backed 恢复证据、Windows 物理 OS 托盘点击恢复证据已对齐 | 通知、deep-link、跨平台 smoke、安装包 |
...
| CUX-L2D-05 | 恢复策略 | `/settings` pass-through 恢复、tray handler 恢复、Windows 物理 OS 托盘点击恢复证据已落；仍需 full hide、多屏与跨平台恢复 |
...
| WEB-P3 | `/proposals/:id` | 多类型交付物变更说明、预览、结构化字段落点、标量字段高级编辑、子记录逐项 diff/editor、任务 plan 选择、折叠多冲突批量检查；R1.39 已补真实 route 富 preview、长 patch、移动端与截图门，R1.40 已补多 plan `task_items` 写回前点选目标 plan，R1.44 已补 route line editor 文件 tab、搜索、逐段 payload 与键盘焦点；后续迁移完整 React SPA route |
| WEB-P4 | `/knowledge` / Cuu search | Web 做兜底，Cuu 气泡做默认入口 |
| WEB-P5 | `/dashboard/cost` | P-COST Usage / BudgetNotice / CostSummaryVM 接真实数据 |
| WEB-P6 | `/agent-runs/:id/replay` | 真实 trace、accepted deliverables、merge timeline、字段写回审计与 eval/replay 对齐；R1.39 已补 Web/Desktop route 富 viewer 截图门；后续扩全量状态与多场景数据 |
...
| PET-R2 | tray recovery | Cuu 显隐、主窗、收件箱、设置已落；`restore-pet-interaction` 已落且有 command-backed handler 恢复证据；R3.20b 已补 Windows 物理 OS 托盘菜单点击恢复 |
...
| Cuu motion | P1.10 以前的黑/白 contact sheet、GIF/MP4、diff report只作为回归证据；R3.10 launcher/en-US、R3.12 zh-CN/en-US run-stream completion、R3.13.1 zh-CN/en-US run-failure terminal、R3.13.2 zh-CN/en-US 401/403/offline、R3.15 reload restore、R3.17 settings/menu matrix、R3.18 `/settings` pass-through 恢复、R3.19 tray handler 恢复、R3.20a hover sync、R3.20b physical tray recovery 与 card text overflow gate 是当前业务功能视觉证据；R3.13.3 是 webview boot 恢复源码/单测证据；R3.20b 起 run-failure/run-stream 要求 `pet_card_text_overflow_gate.passed=true`，R3.13.2 要求 `right_edge_clip_gate` 通过，R3.17-R3.20b 要求 settings/menu/recovery layout gate 与 `overflow.offenders=[]`；不新增外观矩阵 |
...
| R3 | Cuu 自然语言 / option-first 出站 Agent 入口，不新增外观；R3.12 已补真实 Tauri run-stream completion，中英双语证据；R3.13.1 已补真实 Tauri run-failure terminal，中英双语证据；R3.13.2 已补真实 Tauri 401/403/offline，中英双语证据；R3.13.3 已补 webview boot session/run 恢复；R3.15 已补真实 reload capture；R3.17 已补 settings/menu matrix；R3.18 已补 `/settings` pass-through 恢复；R3.19 已补 tray handler 恢复与文本边界；R3.20a 已补 hover sync 截图与主窗 overflow gate；R3.20b 已补 Windows physical OS tray recovery 与 card text overflow gate；下一步 Linux/macOS smoke |
