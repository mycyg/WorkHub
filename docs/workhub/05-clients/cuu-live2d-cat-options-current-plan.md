...
2. 当前工程主线：继续 R1/R2 后端缺口；真实 `sessions/workitems/knowledge/page workitem` service、CostLedger 默认 store、merge accepted deliverable ledger 与 persistent merge audit 已接入，下一批优先 ProjectDrive/object storage 真实文件搬运、AI 冲突调解、BudgetPolicy 持久化、PG queue claim 与多 worker。
